...
_nczzyspapa = {'started': None, 'text': None}
_ksappfbyoo = 24000
_uwoixniqub = 290.0
_pfjhqcfqxd = 250.0

def _fgpabspjwm() -> float:
    started = _nczzyspapa['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _muptthoupp() -> float:
    return _uwoixniqub - _fgpabspjwm()
_slvkebavsc = _hsapi.llm_chat
_asyvotnqmi = _hsapi.search_web
_oeqyvbziju = _hsapi.fetch_page
_jzhphxfbtq = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _eidckjuutm(*args, **kwargs):
    if _fgpabspjwm() >= _pfjhqcfqxd:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _jzhphxfbtq})
            kwargs['messages'] = steered
    _result = await _slvkebavsc(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _mjtouzkkky(_result)
    return _result

async def _wffmeohqsy(*args, **kwargs):
    if _fgpabspjwm() >= _pfjhqcfqxd:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _asyvotnqmi(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _jzrbfdniir(*args, **kwargs):
    if _fgpabspjwm() >= _pfjhqcfqxd:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _oeqyvbziju(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _eidckjuutm
_hsapi.search_web = _wffmeohqsy
_hsapi.fetch_page = _jzrbfdniir
_oaylmhdwgs = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_ppcizvzecb = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_lmqbhiwhku = 900
_qoswqshoeh = 2

def _xyacmfmzhh(query: Query) -> int:
...
def _hrgxeisxbk(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _ckxbofuxhf(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _xyacmfmzhh(query)
    if fields >= 3:
        return 2
    if _hrgxeisxbk(lowered, _oaylmhdwgs):
        return 1
    if fields <= _qoswqshoeh and len(text) <= _lmqbhiwhku:
        return 0
    if _hrgxeisxbk(lowered, _ppcizvzecb):
        return 0
    return 1

def _mjtouzkkky(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _nczzyspapa['text'] = text.strip()[:_ksappfbyoo]
    except Exception:
        pass

def _rfsxtruogy(text: str):
...
def _ukqfgkuxbm(query: Query) -> Response:
    text = _nczzyspapa['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_ksappfbyoo]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rfsxtruogy(text)
...
def _knwindggvn():
    ...
    def _repoint(answer: str, slot_pos: dict[int, int]) -> str:
        ...
        def sub(m: 're.Match[str]') -> str:
            ...
            return ''.join(('[[%d]]' % pos for pos in out))
        return _REPOINT_RE.sub(sub, answer)
    _SEG_PTR_RE = re.compile('\\[\\[(\\d{1,4})\\]\\]')
    _SEG_ANCHOR_RE = re.compile("[A-Z][A-Za-z0-9]*(?:[.,'&/\\-][A-Za-z0-9]+)*")
    _SEG_MAX_SPLITS = 5

    def _seg_locate_anchor(pre: str, note: str) -> tuple[str, int]:
        cands = [m.group(0).strip() for m in _SEG_ANCHOR_RE.finditer(pre)]
    ...
    def _segment_citations(text: str, ledger: 'EvidenceLedger', citations: list, slot_pos: dict[int, int]) -> tuple[str, list]:
        if not text or not citations:
            return (text, citations)
        pos_to_row = {p: n for n, p in slot_pos.items()}
        cites = list(citations)
        spent = 0
        for c in cites:
            for sl in getattr(c, 'slices', None) or []:
                spent += max(0, sl.end - sl.start)
        matches = list(_SEG_PTR_RE.finditer(text))
        seen: dict[int, int] = {}
        ...
        for m in matches:
            ...
            if seen[p] == 1 or splits >= _SEG_MAX_SPLITS or len(cites) >= CITATION_CAP:
                continue
            n = pos_to_row.get(p)
            if not n or not 1 <= n <= len(ledger.rows):
                continue
            row = ledger.rows[n - 1]
            note = row.get('text') or ''
            rid = row.get('receipt_id')
            res = row.get('result_id')
            if not (note and rid and res):
                continue
            anchor, idx = _seg_locate_anchor(text[max(0, m.start() - 90):m.start()], note)
            if not anchor or idx == -1:
                continue
            half = CITATION_ANCHORED_SPAN_CHARS // 2
            s = max(0, idx - half)
            e = min(len(note), idx + len(anchor) + half)
            if spent + (e - s) > EVIDENCE_CHAR_BUDGET:
                break
            try:
                newref = CitationRef(receipt_id=rid, result_id=res, slices=[CitationSlice(start=s, end=e)])
            except Exception:
                continue
            cites.append(newref)
            spent += e - s
            splits += 1
            repls.append((m.start(), m.end(), '[[%d]]' % len(cites)))
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        answer = _strip_lead_narration(answer)
        answer = _answer_line_only(answer, question)
        repointed = _repoint(answer, _slot_pos)
        if not fast:
            try:
                repointed, citations = _segment_citations(repointed, ledger, citations, _slot_pos)
            except Exception:
                pass
        text = _cap(repointed) or f'Best-effort answer unavailable for: {question[:400]}'
...
def _wurimhkhxv():
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            return Response(text=text)
    return query
_rktlbnjbol = _xikgsdrtrb()
_qvgbktnoxa = _knwindggvn()
_dyucfanzyz = _obwvlzcrfb()
_fahidjhgur = _wurimhkhxv()
_uwoixniqub = 290.0
_yszrxugweu = 250.0
_lxmymvujzi = 90.0

async def _sgxkrsnyjf(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _uwoixniqub - (time.monotonic() - started)
        if first:
            budget = _yszrxugweu if _yszrxugweu < remaining else remaining
            first = False
        else:
            if remaining < _lxmymvujzi:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ukqfgkuxbm(query)
_fskmraemtm = 235.0

async def _hbbwsehmmi(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _ghzqgykujl(query, agents):
    if len(agents) < 2:
        return await _sgxkrsnyjf(query, agents)
    r0, r1 = await asyncio.gather(_hbbwsehmmi(agents[0], query, _fskmraemtm), _hbbwsehmmi(agents[1], query, _fskmraemtm))
    ok0 = isinstance(r0, Response) and _qoqdhcaabg(r0) != ''
    ok1 = isinstance(r1, Response) and _qoqdhcaabg(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_slvkebavsc(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _nczzyspapa['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _sgxkrsnyjf(query, (_dyucfanzyz, _rktlbnjbol, _qvgbktnoxa, _fahidjhgur))
        index = _ckxbofuxhf(query)
        if index == 0:
            agents = (_rktlbnjbol, _qvgbktnoxa, _dyucfanzyz, _fahidjhgur)
        elif index == 1:
            agents = (_qvgbktnoxa, _dyucfanzyz, _fahidjhgur, _rktlbnjbol)
        elif index == 2:
            agents = (_dyucfanzyz, _fahidjhgur, _rktlbnjbol, _qvgbktnoxa)
        elif index == 3:
            agents = (_fahidjhgur, _rktlbnjbol, _qvgbktnoxa, _dyucfanzyz)
        else:
            agents = (_rktlbnjbol, _qvgbktnoxa, _dyucfanzyz, _fahidjhgur)
        return await _ghzqgykujl(query, agents)
    except Exception:
        return _ukqfgkuxbm(query)
_BUILDJ_TAG_m30 = "sn45-60743faff11f"
