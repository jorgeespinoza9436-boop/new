...
_spebljavhw = {'started': None, 'text': None}
_atqvwjsmev = 24000
_qxdqblisob = 290.0
_bztunzqsiq = 250.0

def _tstinnjqjp() -> float:
    started = _spebljavhw['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _xvhclbtwql() -> float:
    return _qxdqblisob - _tstinnjqjp()
_khncgdghgs = _hsapi.llm_chat
_utqxexbrje = _hsapi.search_web
_rmaybmmbks = _hsapi.fetch_page
_iudwtyfacs = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _deuuflanlj(*args, **kwargs):
    if _tstinnjqjp() >= _bztunzqsiq:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _iudwtyfacs})
            kwargs['messages'] = steered
    _result = await _khncgdghgs(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _wvyyrfrjbq(_result)
    return _result

async def _vhlslmonox(*args, **kwargs):
    if _tstinnjqjp() >= _bztunzqsiq:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _utqxexbrje(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _mxiilfgqsk(*args, **kwargs):
    if _tstinnjqjp() >= _bztunzqsiq:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _rmaybmmbks(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _deuuflanlj
_hsapi.search_web = _vhlslmonox
_hsapi.fetch_page = _mxiilfgqsk
_ivfjvjsysn = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_spsqcaszzc = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_uuythotbtd = 900
_vzbgtsoere = 2

def _bftvwsqivk(query: Query) -> int:
...
def _egzdtzubbn(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _owkiplylyz(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _bftvwsqivk(query)
    if fields >= 3:
        return 2
    if _egzdtzubbn(lowered, _ivfjvjsysn):
        return 1
    if fields <= _vzbgtsoere and len(text) <= _uuythotbtd:
        return 0
    if _egzdtzubbn(lowered, _spsqcaszzc):
        return 0
    return 1

def _wvyyrfrjbq(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _spebljavhw['text'] = text.strip()[:_atqvwjsmev]
    except Exception:
        pass

def _qnsisgjpho(text: str):
...
def _vgjnqitjtp(query: Query) -> Response:
    text = _spebljavhw['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_atqvwjsmev]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _qnsisgjpho(text)
...
def _soptokbcvt():
    ...
    """
        ...
        return response
    return query
_pszofbwvkw = _jddetuqnra()
_gkylyuogit = _nzpalnnrow()
_pytxkmudmd = _jhehxzpiij()
_pvxcplladd = _soptokbcvt()
_qxdqblisob = 290.0
_bitsbkwxoq = 250.0
_hclisolymm = 90.0

async def _jcaktcivdw(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _qxdqblisob - (time.monotonic() - started)
        if first:
            budget = _bitsbkwxoq if _bitsbkwxoq < remaining else remaining
            first = False
        else:
            if remaining < _hclisolymm:
                break
        ...
        try:
            ...
            last_exc = exc
    return _vgjnqitjtp(query)
_lenfntbmnm = 235.0

async def _ajcivfeobr(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _fezknmejlf(query, agents):
    if len(agents) < 2:
        return await _jcaktcivdw(query, agents)
    r0, r1 = await asyncio.gather(_ajcivfeobr(agents[0], query, _lenfntbmnm), _ajcivfeobr(agents[1], query, _lenfntbmnm))
    ok0 = isinstance(r0, Response) and _bidhmlvbot(r0) != ''
    ok1 = isinstance(r1, Response) and _bidhmlvbot(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_khncgdghgs(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _spebljavhw['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _jcaktcivdw(query, (_pytxkmudmd, _pszofbwvkw, _gkylyuogit, _pvxcplladd))
        index = _owkiplylyz(query)
        if index == 0:
            agents = (_pszofbwvkw, _gkylyuogit, _pytxkmudmd, _pvxcplladd)
        elif index == 1:
            agents = (_gkylyuogit, _pytxkmudmd, _pvxcplladd, _pszofbwvkw)
        elif index == 2:
            agents = (_pytxkmudmd, _pvxcplladd, _pszofbwvkw, _gkylyuogit)
        elif index == 3:
            agents = (_pvxcplladd, _pszofbwvkw, _gkylyuogit, _pytxkmudmd)
        else:
            agents = (_pszofbwvkw, _gkylyuogit, _pytxkmudmd, _pvxcplladd)
        return await _fezknmejlf(query, agents)
    except Exception:
        return _vgjnqitjtp(query)
_BUILDJ_TAG_m24 = "sn45-06cb518f4ad6"
