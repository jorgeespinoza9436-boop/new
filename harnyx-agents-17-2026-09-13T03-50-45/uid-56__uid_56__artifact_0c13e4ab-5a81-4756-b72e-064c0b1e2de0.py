...
_mwczxceona = {'started': None, 'text': None}
_dwhxkcoldt = 24000
_ykuarrjgja = 290.0
_yunjvyjdmi = 250.0

def _oatqaapxni() -> float:
    started = _mwczxceona['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _qdguuhhtwv() -> float:
    return _ykuarrjgja - _oatqaapxni()
_rnxtoxefpi = _hsapi.llm_chat
_jvfipazyun = _hsapi.search_web
_qnkegzumxc = _hsapi.fetch_page
_comytpkagf = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _cxsetwpgla(*args, **kwargs):
    if _oatqaapxni() >= _yunjvyjdmi:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _comytpkagf})
            kwargs['messages'] = steered
    _result = await _rnxtoxefpi(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _yrjsdghotd(_result)
    return _result

async def _zbgunktdld(*args, **kwargs):
    if _oatqaapxni() >= _yunjvyjdmi:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _jvfipazyun(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _yjezzidglm(*args, **kwargs):
    if _oatqaapxni() >= _yunjvyjdmi:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qnkegzumxc(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _cxsetwpgla
_hsapi.search_web = _zbgunktdld
_hsapi.fetch_page = _yjezzidglm
_zsrataamtp = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_uoowyqltoh = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_mmkoaryajk = 900
_twqtvnaqhg = 2

def _dpnrerlxok(query: Query) -> int:
...
def _jhcyxgibxz(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _hdcmuxheok(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _dpnrerlxok(query)
    if fields >= 3:
        return 2
    if _jhcyxgibxz(lowered, _zsrataamtp):
        return 1
    if fields <= _twqtvnaqhg and len(text) <= _mmkoaryajk:
        return 0
    if _jhcyxgibxz(lowered, _uoowyqltoh):
        return 0
    return 1

def _yrjsdghotd(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _mwczxceona['text'] = text.strip()[:_dwhxkcoldt]
    except Exception:
        pass

def _qgyleksngs(text: str):
...
def _mxxygryaxp(query: Query) -> Response:
    text = _mwczxceona['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_dwhxkcoldt]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _qgyleksngs(text)
...
def _kxvqkwytbx():
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return response
    return query
_bglhjusxoz = _ixhcdrgcbn()
_xoyvqxcqce = _qyokdxlqor()
_zzsrlvvubp = _emasuaffll()
_wjnaakuldq = _kxvqkwytbx()
_ykuarrjgja = 290.0
_kfwzygizzl = 250.0
_vatpuijzyj = 90.0

async def _wtqqttgrws(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ykuarrjgja - (time.monotonic() - started)
        if first:
            budget = _kfwzygizzl if _kfwzygizzl < remaining else remaining
            first = False
        else:
            if remaining < _vatpuijzyj:
                break
        ...
        try:
            ...
            last_exc = exc
    return _mxxygryaxp(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _mwczxceona['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _wtqqttgrws(query, (_zzsrlvvubp, _bglhjusxoz, _xoyvqxcqce, _wjnaakuldq))
        index = _hdcmuxheok(query)
        if index == 0:
            agents = (_bglhjusxoz, _xoyvqxcqce, _zzsrlvvubp, _wjnaakuldq)
        elif index == 1:
            agents = (_xoyvqxcqce, _zzsrlvvubp, _wjnaakuldq, _bglhjusxoz)
        elif index == 2:
            agents = (_zzsrlvvubp, _wjnaakuldq, _bglhjusxoz, _xoyvqxcqce)
        elif index == 3:
            agents = (_wjnaakuldq, _bglhjusxoz, _xoyvqxcqce, _zzsrlvvubp)
        else:
            agents = (_bglhjusxoz, _xoyvqxcqce, _zzsrlvvubp, _wjnaakuldq)
        return await _wtqqttgrws(query, agents)
    except Exception:
        return _mxxygryaxp(query)
_BUILDJ_TAG_m23 = "sn45-b18c397eeaa8"
