...
_pwuuapkcfl = {'started': None, 'text': None}
_mdboegzmga = 24000
_lhviqbkozh = 290.0
_xvwqibskhf = 250.0

def _ghitnlbmpl() -> float:
    started = _pwuuapkcfl['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _ydodtxboyp() -> float:
    return _lhviqbkozh - _ghitnlbmpl()
_schyjhnajm = _hsapi.llm_chat
_zcvsnubpef = _hsapi.search_web
_rjcsqeqfhx = _hsapi.fetch_page
_robninqkpe = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _fckmsjdbfd(*args, **kwargs):
    if _ghitnlbmpl() >= _xvwqibskhf:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _robninqkpe})
            kwargs['messages'] = steered
    _result = await _schyjhnajm(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ubabiesbgg(_result)
    return _result

async def _zqqaxlocca(*args, **kwargs):
    if _ghitnlbmpl() >= _xvwqibskhf:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zcvsnubpef(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _aaoikwaxzw(*args, **kwargs):
    if _ghitnlbmpl() >= _xvwqibskhf:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _rjcsqeqfhx(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _fckmsjdbfd
_hsapi.search_web = _zqqaxlocca
_hsapi.fetch_page = _aaoikwaxzw
_gqenvmegzj = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_mlewrrtskc = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_oysjiwxmgq = 900
_izadbcgfoy = 2

def _jeoruobequ(query: Query) -> int:
...
def _ribpydjbmy(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _otvltviycj(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _jeoruobequ(query)
    if fields >= 3:
        return 2
    if _ribpydjbmy(lowered, _gqenvmegzj):
        return 1
    if fields <= _izadbcgfoy and len(text) <= _oysjiwxmgq:
        return 0
    if _ribpydjbmy(lowered, _mlewrrtskc):
        return 0
    return 1

def _ubabiesbgg(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _pwuuapkcfl['text'] = text.strip()[:_mdboegzmga]
    except Exception:
        pass

def _kfyayyealp(text: str):
...
def _hmsxncybiw(query: Query) -> Response:
    text = _pwuuapkcfl['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_mdboegzmga]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _kfyayyealp(text)
...
"""
    ...
    async def query(query: _DrvQuery) -> _DrvResponse:
        ...
        try:
            ...
            return draft
    return query
_bkahwjnmcb = _ljgjygjycl()
_pgccebgxrm = _vbkdgigmcf()
_jurxevhsuc = _wfzatnvqpw()
_bjiowytadj = _gfalahblmm()
_lhviqbkozh = 290.0
_kdsrjovrbg = 250.0
_xcqvlzaoiw = 90.0

async def _pqzlvyqyne(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _lhviqbkozh - (time.monotonic() - started)
        if first:
            budget = _kdsrjovrbg if _kdsrjovrbg < remaining else remaining
            first = False
        else:
            if remaining < _xcqvlzaoiw:
                break
        ...
        try:
            ...
            last_exc = exc
    return _hmsxncybiw(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _pwuuapkcfl['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _pqzlvyqyne(query, (_jurxevhsuc, _bkahwjnmcb, _pgccebgxrm, _bjiowytadj))
        index = _otvltviycj(query)
        if index == 0:
            agents = (_bkahwjnmcb, _pgccebgxrm, _jurxevhsuc, _bjiowytadj)
        elif index == 1:
            agents = (_pgccebgxrm, _jurxevhsuc, _bjiowytadj, _bkahwjnmcb)
        elif index == 2:
            agents = (_jurxevhsuc, _bjiowytadj, _bkahwjnmcb, _pgccebgxrm)
        elif index == 3:
            agents = (_bjiowytadj, _bkahwjnmcb, _pgccebgxrm, _jurxevhsuc)
        else:
            agents = (_bkahwjnmcb, _pgccebgxrm, _jurxevhsuc, _bjiowytadj)
        return await _pqzlvyqyne(query, agents)
    except Exception:
        return _hmsxncybiw(query)
_BUILDJ_TAG_m29 = "sn45-c4ed7e0dcefc"
