...
_damljcqygt = {'started': None, 'text': None}
_wdcdvdfgvs = 24000
_cofyvmvfgt = 290.0
_mmatbloqfx = 250.0

def _ntnaxirjyi() -> float:
    started = _damljcqygt['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _fmwyyfxxva() -> float:
    return _cofyvmvfgt - _ntnaxirjyi()
_uxdacrlpvz = _hsapi.llm_chat
_vpplvsfxlp = _hsapi.search_web
_zlbocwoehg = _hsapi.fetch_page
...
async def _hqwsymvjzl(*args, **kwargs):
    if _ntnaxirjyi() >= _mmatbloqfx:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _bgaqwgfpse})
            kwargs['messages'] = steered
    _result = await _uxdacrlpvz(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ibeutqlfjv(_result)
    return _result

async def _rzvbaswatr(*args, **kwargs):
    if _ntnaxirjyi() >= _mmatbloqfx:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _vpplvsfxlp(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _upxfybviyx(*args, **kwargs):
    if _ntnaxirjyi() >= _mmatbloqfx:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zlbocwoehg(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _hqwsymvjzl
_hsapi.search_web = _rzvbaswatr
_hsapi.fetch_page = _upxfybviyx
_lbsieowzoj = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_fninhliepn = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_krmdcqyzfr = 900
_vgoyqiuvxy = 2

def _onijeuntnk(query: Query) -> int:
...
def _oivfqvqlpb(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _nwyzztwwqp(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _onijeuntnk(query)
    if fields >= 3:
        return 2
    if _oivfqvqlpb(lowered, _lbsieowzoj):
        return 1
    if fields <= _vgoyqiuvxy and len(text) <= _krmdcqyzfr:
        return 0
    if _oivfqvqlpb(lowered, _fninhliepn):
        return 0
    return 1

def _ibeutqlfjv(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _damljcqygt['text'] = text.strip()[:_wdcdvdfgvs]
    except Exception:
        pass

def _uwlzmpegnw(text: str):
...
def _jmekxyxgfi(query: Query) -> Response:
    text = _damljcqygt['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_wdcdvdfgvs]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _uwlzmpegnw(text)
...
def _qunwkowmjj():
    ...
    return query
_nvvhfcpkxe = _iawyxltdrv()
_jbqivxrlsn = _jpyniocdba()
_vlgehvdgck = _syflaspwsv()
_kryfswmdia = _qunwkowmjj()
_cofyvmvfgt = 290.0
_arnjaavysq = 250.0
_xlafeeqyxl = 90.0

async def _dfcmyzbzkm(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _cofyvmvfgt - (time.monotonic() - started)
        if first:
            budget = _arnjaavysq if _arnjaavysq < remaining else remaining
            first = False
        else:
            if remaining < _xlafeeqyxl:
                break
        ...
        try:
            ...
            last_exc = exc
    return _jmekxyxgfi(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _damljcqygt['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _dfcmyzbzkm(query, (_vlgehvdgck, _nvvhfcpkxe, _jbqivxrlsn, _kryfswmdia))
        index = _nwyzztwwqp(query)
        if index == 0:
            agents = (_nvvhfcpkxe, _jbqivxrlsn, _vlgehvdgck, _kryfswmdia)
        elif index == 1:
            agents = (_jbqivxrlsn, _vlgehvdgck, _kryfswmdia, _nvvhfcpkxe)
        elif index == 2:
            agents = (_vlgehvdgck, _kryfswmdia, _nvvhfcpkxe, _jbqivxrlsn)
        elif index == 3:
            agents = (_kryfswmdia, _nvvhfcpkxe, _jbqivxrlsn, _vlgehvdgck)
        else:
            agents = (_nvvhfcpkxe, _jbqivxrlsn, _vlgehvdgck, _kryfswmdia)
        return await _dfcmyzbzkm(query, agents)
    except Exception:
        return _jmekxyxgfi(query)
_BUILDJ_TAG_m20 = "sn45-a927bba6203c"
