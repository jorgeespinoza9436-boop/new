...
_gncawmymhl = {'started': None, 'text': None}
_rfqjleztcc = 24000
_xipghvgxcc = 290.0
_lqniilnfsf = 250.0

def _whvizouxya() -> float:
    started = _gncawmymhl['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _clpvrqsqrr() -> float:
    return _xipghvgxcc - _whvizouxya()
_hajkttnaai = _hsapi.llm_chat
_avkcsgofhx = _hsapi.search_web
_jdmpwrsaao = _hsapi.fetch_page
_oxutotvcpp = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _edujypkram(*args, **kwargs):
    if _whvizouxya() >= _lqniilnfsf:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _oxutotvcpp})
            kwargs['messages'] = steered
    _result = await _hajkttnaai(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _fmwngxciwa(_result)
    return _result

async def _cisapqvbtq(*args, **kwargs):
    if _whvizouxya() >= _lqniilnfsf:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _avkcsgofhx(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _dpsytnwhnb(*args, **kwargs):
    if _whvizouxya() >= _lqniilnfsf:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _jdmpwrsaao(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _edujypkram
_hsapi.search_web = _cisapqvbtq
_hsapi.fetch_page = _dpsytnwhnb
_mkvlbjsdfb = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_nhrmcnfafe = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_qiceiishku = 900
_bbazqbdsfg = 2

def _tlmflhdumm(query: Query) -> int:
...
def _ucryuxjfin(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _eyxpvhmwsn(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _tlmflhdumm(query)
    if fields >= 3:
        return 2
    if _ucryuxjfin(lowered, _mkvlbjsdfb):
        return 1
    if fields <= _bbazqbdsfg and len(text) <= _qiceiishku:
        return 0
    if _ucryuxjfin(lowered, _nhrmcnfafe):
        return 0
    return 1

def _fmwngxciwa(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _gncawmymhl['text'] = text.strip()[:_rfqjleztcc]
    except Exception:
        pass

def _qcwfxxpclz(text: str):
...
def _llzqzvxmso(query: Query) -> Response:
    text = _gncawmymhl['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_rfqjleztcc]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _qcwfxxpclz(text)
...
def _uvyopkcqnb():
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            return Response(text=text)
    return query
_dqmuapwwqq = _losnsyksda()
_tunbxaznkp = _kzhlxmqmvm()
_yjbfyqjsao = _pjhgjsvuog()
_vctbztvrdh = _uvyopkcqnb()
_xipghvgxcc = 290.0
_lfanrzmnad = 250.0
_wsmebdpimm = 90.0

async def _wusscmezkw(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _xipghvgxcc - (time.monotonic() - started)
        if first:
            budget = _lfanrzmnad if _lfanrzmnad < remaining else remaining
            first = False
        else:
            if remaining < _wsmebdpimm:
                break
        ...
        try:
            ...
            last_exc = exc
    return _llzqzvxmso(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _gncawmymhl['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _wusscmezkw(query, (_yjbfyqjsao, _dqmuapwwqq, _tunbxaznkp, _vctbztvrdh))
        index = _eyxpvhmwsn(query)
        if index == 0:
            agents = (_dqmuapwwqq, _tunbxaznkp, _yjbfyqjsao, _vctbztvrdh)
        elif index == 1:
            agents = (_tunbxaznkp, _yjbfyqjsao, _vctbztvrdh, _dqmuapwwqq)
        elif index == 2:
            agents = (_yjbfyqjsao, _vctbztvrdh, _dqmuapwwqq, _tunbxaznkp)
        elif index == 3:
            agents = (_vctbztvrdh, _dqmuapwwqq, _tunbxaznkp, _yjbfyqjsao)
        else:
            agents = (_dqmuapwwqq, _tunbxaznkp, _yjbfyqjsao, _vctbztvrdh)
        return await _wusscmezkw(query, agents)
    except Exception:
        return _llzqzvxmso(query)
_BUILDJ_TAG_m1 = "sn45-5f566ba9611e"
