...
_lhikbqgmbm = {'started': None, 'text': None}
_cmmuuwdpje = 24000
_uhwzcbckey = 290.0
_wqoscteuzs = 250.0

def _wimypvhlcf() -> float:
    started = _lhikbqgmbm['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _gxvkwjlivg() -> float:
    return _uhwzcbckey - _wimypvhlcf()
_hvhliqwzho = _hsapi.llm_chat
_eauxyyzjlp = _hsapi.search_web
_wpvgfexuln = _hsapi.fetch_page
_cbhzaygclq = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _fkuhicyiub(*args, **kwargs):
    if _wimypvhlcf() >= _wqoscteuzs:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _cbhzaygclq})
            kwargs['messages'] = steered
    _result = await _hvhliqwzho(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _pwxedaagng(_result)
    return _result

async def _awbtqizhpj(*args, **kwargs):
    if _wimypvhlcf() >= _wqoscteuzs:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _eauxyyzjlp(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _pksbekbtyv(*args, **kwargs):
    if _wimypvhlcf() >= _wqoscteuzs:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _wpvgfexuln(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _fkuhicyiub
_hsapi.search_web = _awbtqizhpj
_hsapi.fetch_page = _pksbekbtyv
_hmzzewvbuw = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_zvgvgedvkn = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_ajpxvprxlr = 900
_psjkuksoct = 2

def _iulmhvuanb(query: Query) -> int:
...
def _onpkgehbxw(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _svkziiwmcq(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _iulmhvuanb(query)
    if fields >= 3:
        return 2
    if _onpkgehbxw(lowered, _hmzzewvbuw):
        return 1
    if fields <= _psjkuksoct and len(text) <= _ajpxvprxlr:
        return 0
    if _onpkgehbxw(lowered, _zvgvgedvkn):
        return 0
    return 1

def _pwxedaagng(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _lhikbqgmbm['text'] = text.strip()[:_cmmuuwdpje]
    except Exception:
        pass

def _nhhcjcyjoh(text: str):
...
def _dwwhrahpbi(query: Query) -> Response:
    text = _lhikbqgmbm['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_cmmuuwdpje]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _nhhcjcyjoh(text)
...
def _qgdexvwcyf():
    ...
    NARRATED_INTENT_RE = re.compile("^\\s*(?:i need to|i will need to|let me|i'll|i will|first,? i)\\s+(?:find|search|read|check|look|fetch|grep|locate|verify)", re.IGNORECASE)
    ...
    COMMIT_MESSAGE = "Tools are now DISABLED. Produce the VERIFY table and FINAL ANSWER from the numbered evidence you already have, with [n] citations after every claim. Write the FINAL ANSWER as prose paragraphs: no bullets, headers or bold unless the question asks for a list or table, and no closing recap. Cite the question's named source for the requested facts; add no descriptions, background or category confirmations from other pages, and do not cite pages fetched for that purpose. Commit."
    ...
    """
        ...
        return response
    return query
_aeofzcflqc = _wuefiadyev()
_ezskbjmxgj = _esglajwlcm()
_cjezfpmtgz = _ertjwdbeaw()
_hsiehalevw = _qgdexvwcyf()
_uhwzcbckey = 290.0
_xtbpjvczhy = 250.0
_fcftrhspro = 90.0

async def _aiphqpadxj(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _uhwzcbckey - (time.monotonic() - started)
        if first:
            budget = _xtbpjvczhy if _xtbpjvczhy < remaining else remaining
            first = False
        else:
            if remaining < _fcftrhspro:
                break
        ...
        try:
            ...
            last_exc = exc
    return _dwwhrahpbi(query)
_ztcfjsfdjx = 235.0

async def _frfltpwghm(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _ohciwonqdb(query, agents):
    if len(agents) < 2:
        return await _aiphqpadxj(query, agents)
    r0, r1 = await asyncio.gather(_frfltpwghm(agents[0], query, _ztcfjsfdjx), _frfltpwghm(agents[1], query, _ztcfjsfdjx))
    ok0 = isinstance(r0, Response) and _jlxcqfixkg(r0) != ''
    ok1 = isinstance(r1, Response) and _jlxcqfixkg(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_hvhliqwzho(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _lhikbqgmbm['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _aiphqpadxj(query, (_cjezfpmtgz, _aeofzcflqc, _ezskbjmxgj, _hsiehalevw))
        index = _svkziiwmcq(query)
        if index == 0:
            agents = (_aeofzcflqc, _ezskbjmxgj, _cjezfpmtgz, _hsiehalevw)
        elif index == 1:
            agents = (_ezskbjmxgj, _cjezfpmtgz, _hsiehalevw, _aeofzcflqc)
        elif index == 2:
            agents = (_cjezfpmtgz, _hsiehalevw, _aeofzcflqc, _ezskbjmxgj)
        elif index == 3:
            agents = (_hsiehalevw, _aeofzcflqc, _ezskbjmxgj, _cjezfpmtgz)
        else:
            agents = (_aeofzcflqc, _ezskbjmxgj, _cjezfpmtgz, _hsiehalevw)
        return await _ohciwonqdb(query, agents)
    except Exception:
        return _dwwhrahpbi(query)
_BUILDJ_TAG_m6 = "sn45-d80f7da8c5aa"
