...
_fqwkfnoaxn = {'started': None, 'text': None}
_esxkcqthrv = 24000
_xzetvqzzjc = 290.0
_yhmxjbanxp = 250.0

def _qjgutxxvbj() -> float:
    started = _fqwkfnoaxn['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _nwjwvjuwuv() -> float:
    return _xzetvqzzjc - _qjgutxxvbj()
_hfglegpnin = _hsapi.llm_chat
_ndkqsywgye = _hsapi.search_web
_zgoiwodaza = _hsapi.fetch_page
...
async def _vjkewgabzm(*args, **kwargs):
    if _qjgutxxvbj() >= _yhmxjbanxp:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _xkfvpmpwrf})
            kwargs['messages'] = steered
    _result = await _hfglegpnin(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ipqyddaahn(_result)
    return _result

async def _rhzbzmqgcl(*args, **kwargs):
    if _qjgutxxvbj() >= _yhmxjbanxp:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ndkqsywgye(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _ixguqmpehg(*args, **kwargs):
    if _qjgutxxvbj() >= _yhmxjbanxp:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zgoiwodaza(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _vjkewgabzm
_hsapi.search_web = _rhzbzmqgcl
_hsapi.fetch_page = _ixguqmpehg
_byysinrrhr = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_ybnhavqblm = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_hvrxxrmfpo = 900
_jxgfxfvtex = 2

def _xthnnxrdfw(query: Query) -> int:
...
def _wkhzlykhzv(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _rapiisrknx(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _xthnnxrdfw(query)
    if fields >= 3:
        return 2
    if _wkhzlykhzv(lowered, _byysinrrhr):
        return 1
    if fields <= _jxgfxfvtex and len(text) <= _hvrxxrmfpo:
        return 0
    if _wkhzlykhzv(lowered, _ybnhavqblm):
        return 0
    return 1

def _ipqyddaahn(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _fqwkfnoaxn['text'] = text.strip()[:_esxkcqthrv]
    except Exception:
        pass

def _qswrhsmndb(text: str):
...
def _ptowzkmlbz(query: Query) -> Response:
    text = _fqwkfnoaxn['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_esxkcqthrv]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _qswrhsmndb(text)
...
def _fimolwbdpr():
    _S111SUCCESS_QUERY_TAG = 's111success-hk6722'
    import asyncio
    import json
    import re
    from time import monotonic
    ...
    async def _element_chat(system: str, user: str, max_tokens: int, timeout: float) -> str:
        return await _chat_simple(LLM_LANE_A, ELEMENT_MODEL, system, user, max_tokens=max_tokens, timeout=timeout)
    ...
    """
        ...
        try:
            raw = await _element_chat('Deep-research planner. Name the evidence to gather. JSON list only.', probe, ELEMENT_MAX_TOKENS, max(6.0, min(ELEMENT_TIMEOUT_S, deadline - monotonic() - ELEMENT_MIN_SECONDS + 40.0)))
            raw = re.sub('^```(?:json)?\\s*|\\s*```$', '', raw.strip(), flags=re.I | re.M)
...
"""
    ...
    return query
_hqmqgwbrlh = _wuiikdqwkj()
_wezhomjaxi = _fimolwbdpr()
_layalzjcyo = _yhdjhdprrm()
_ihrnxngiis = _riptotepfr()
_xzetvqzzjc = 290.0
_qicycsioyb = 250.0
_tocbbzywwi = 90.0

async def _hslyhkqvzw(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _xzetvqzzjc - (time.monotonic() - started)
        if first:
            budget = _qicycsioyb if _qicycsioyb < remaining else remaining
            first = False
        else:
            if remaining < _tocbbzywwi:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ptowzkmlbz(query)
_zsgwjqqvze = 235.0

async def _jiomqfrzuj(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _mzniaktifk(query, agents):
    if len(agents) < 2:
        return await _hslyhkqvzw(query, agents)
    r0, r1 = await asyncio.gather(_jiomqfrzuj(agents[0], query, _zsgwjqqvze), _jiomqfrzuj(agents[1], query, _zsgwjqqvze))
    ok0 = isinstance(r0, Response) and _vsirqgprff(r0) != ''
    ok1 = isinstance(r1, Response) and _vsirqgprff(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_hfglegpnin(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _fqwkfnoaxn['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _hslyhkqvzw(query, (_layalzjcyo, _hqmqgwbrlh, _wezhomjaxi, _ihrnxngiis))
        index = _rapiisrknx(query)
        if index == 0:
            agents = (_hqmqgwbrlh, _wezhomjaxi, _layalzjcyo, _ihrnxngiis)
        elif index == 1:
            agents = (_wezhomjaxi, _layalzjcyo, _ihrnxngiis, _hqmqgwbrlh)
        elif index == 2:
            agents = (_layalzjcyo, _ihrnxngiis, _hqmqgwbrlh, _wezhomjaxi)
        elif index == 3:
            agents = (_ihrnxngiis, _hqmqgwbrlh, _wezhomjaxi, _layalzjcyo)
        else:
            agents = (_hqmqgwbrlh, _wezhomjaxi, _layalzjcyo, _ihrnxngiis)
        return await _mzniaktifk(query, agents)
    except Exception:
        return _ptowzkmlbz(query)
_BUILDJ_TAG_m3 = "sn45-16a9f28ec135"
