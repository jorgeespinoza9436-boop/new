...
_twgkggtxma = {'started': None, 'text': None}
_qbiwajbolp = 24000
_rrxqyxilkz = 290.0
_xmbnbdpmoj = 250.0

def _bwhcfiffeg() -> float:
    started = _twgkggtxma['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _dpoqqiqcao() -> float:
    return _rrxqyxilkz - _bwhcfiffeg()
_aecjxjkolv = _hsapi.llm_chat
_qcwvkqdxba = _hsapi.search_web
_yehqnfpcsx = _hsapi.fetch_page
_vnndzltjpj = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _ssxoogvazs(*args, **kwargs):
    if _bwhcfiffeg() >= _xmbnbdpmoj:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _vnndzltjpj})
            kwargs['messages'] = steered
    _result = await _aecjxjkolv(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _itmhdklbdn(_result)
    return _result

async def _hnvyfnzuat(*args, **kwargs):
    if _bwhcfiffeg() >= _xmbnbdpmoj:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qcwvkqdxba(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _jfqpibhpol(*args, **kwargs):
    if _bwhcfiffeg() >= _xmbnbdpmoj:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _yehqnfpcsx(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _ssxoogvazs
_hsapi.search_web = _hnvyfnzuat
_hsapi.fetch_page = _jfqpibhpol
_hmldkzcldo = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_vxujpodsmx = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_mmztxjtvqc = 900
_czrufcngel = 2

def _zxhdehlbpx(query: Query) -> int:
...
def _srrdkwnoci(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _dpyawqduqc(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _zxhdehlbpx(query)
    if fields >= 3:
        return 2
    if _srrdkwnoci(lowered, _hmldkzcldo):
        return 1
    if fields <= _czrufcngel and len(text) <= _mmztxjtvqc:
        return 0
    if _srrdkwnoci(lowered, _vxujpodsmx):
        return 0
    return 1

def _itmhdklbdn(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _twgkggtxma['text'] = text.strip()[:_qbiwajbolp]
    except Exception:
        pass

def _rcpqvfvanl(text: str):
...
def _fyazsynvtq(query: Query) -> Response:
    text = _twgkggtxma['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_qbiwajbolp]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rcpqvfvanl(text)
...
"""
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            return Response(text=text)
    return query
_qdkoajmoyv = _vmbnexgboz()
_vuppzfzvhs = _osqdqdandg()
_kbtzfcoxdt = _pgzflvmwlp()
_rrxqyxilkz = 290.0
_qhdenltjxb = 250.0
_juoawzjain = 90.0

async def _uipepqhrtk(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _rrxqyxilkz - (time.monotonic() - started)
        if first:
            budget = _qhdenltjxb if _qhdenltjxb < remaining else remaining
            first = False
        else:
            if remaining < _juoawzjain:
                break
        ...
        try:
            ...
            last_exc = exc
    return _fyazsynvtq(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _twgkggtxma['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _uipepqhrtk(query, (_kbtzfcoxdt, _qdkoajmoyv, _vuppzfzvhs))
        index = _dpyawqduqc(query)
        if index == 0:
            agents = (_qdkoajmoyv, _vuppzfzvhs, _kbtzfcoxdt)
        elif index == 1:
            agents = (_vuppzfzvhs, _kbtzfcoxdt, _qdkoajmoyv)
        elif index == 2:
            agents = (_kbtzfcoxdt, _qdkoajmoyv, _vuppzfzvhs)
        else:
            agents = (_qdkoajmoyv, _vuppzfzvhs, _kbtzfcoxdt)
        return await _uipepqhrtk(query, agents)
    except Exception:
        return _fyazsynvtq(query)
_BUILDJ_TAG_m16 = "sn45-97b90f90d9a3"
