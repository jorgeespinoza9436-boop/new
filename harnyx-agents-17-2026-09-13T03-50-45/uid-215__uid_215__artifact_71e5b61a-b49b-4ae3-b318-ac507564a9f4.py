...
_wixkdlcdsh = {'started': None, 'text': None}
_yuxlylefik = 24000
_eqvlkglpiz = 290.0
_vepyyagunx = 250.0

def _mxneedfguo() -> float:
    started = _wixkdlcdsh['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _jpmwiejfnb() -> float:
    return _eqvlkglpiz - _mxneedfguo()
_wrxtgszyec = _hsapi.llm_chat
_obbeaessdd = _hsapi.search_web
_mneumxrouc = _hsapi.fetch_page
_udepwrsxma = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _yszafevzee(*args, **kwargs):
    if _mxneedfguo() >= _vepyyagunx:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _udepwrsxma})
            kwargs['messages'] = steered
    _result = await _wrxtgszyec(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _rboxizkikn(_result)
    return _result

async def _rgljhiymnc(*args, **kwargs):
    if _mxneedfguo() >= _vepyyagunx:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _obbeaessdd(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _ihhlztagzm(*args, **kwargs):
    if _mxneedfguo() >= _vepyyagunx:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _mneumxrouc(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _yszafevzee
_hsapi.search_web = _rgljhiymnc
_hsapi.fetch_page = _ihhlztagzm
_ssbpspemll = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_psdcajecrd = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_iyxwrdcwsm = 900
_avzaqilqxx = 2

def _jdmxmezhlw(query: Query) -> int:
...
def _wtvaiteoss(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _osrsoxsiur(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _jdmxmezhlw(query)
    if fields >= 3:
        return 2
    if _wtvaiteoss(lowered, _ssbpspemll):
        return 1
    if fields <= _avzaqilqxx and len(text) <= _iyxwrdcwsm:
        return 0
    if _wtvaiteoss(lowered, _psdcajecrd):
        return 0
    return 1

def _rboxizkikn(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _wixkdlcdsh['text'] = text.strip()[:_yuxlylefik]
    except Exception:
        pass

def _qjkobhrknu(text: str):
...
def _feiombshoi(query: Query) -> Response:
    text = _wixkdlcdsh['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_yuxlylefik]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _qjkobhrknu(text)
    ...
    *3,100 (footnoted); (12.4) negative; 2,567 lb with a unit; =WR and CR marks;
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        try:
            ...
            return Response(text=text)
    return query
_xtxzhortmz = _rrsvgxqsuk()
_jvsmqieshs = _fzajdnouru()
_jxydgdnojp = _xwvfgymhnb()
_eqvlkglpiz = 290.0
_zjgaytdpcj = 250.0
_ybwbtycwet = 90.0

async def _ysuspbotky(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _eqvlkglpiz - (time.monotonic() - started)
        if first:
            budget = _zjgaytdpcj if _zjgaytdpcj < remaining else remaining
            first = False
        else:
            if remaining < _ybwbtycwet:
                break
        ...
        try:
            ...
            last_exc = exc
    return _feiombshoi(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _wixkdlcdsh['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ysuspbotky(query, (_jxydgdnojp, _xtxzhortmz, _jvsmqieshs))
        index = _osrsoxsiur(query)
        if index == 0:
            agents = (_xtxzhortmz, _jvsmqieshs, _jxydgdnojp)
        elif index == 1:
            agents = (_jvsmqieshs, _jxydgdnojp, _xtxzhortmz)
        elif index == 2:
            agents = (_jxydgdnojp, _xtxzhortmz, _jvsmqieshs)
        else:
            agents = (_xtxzhortmz, _jvsmqieshs, _jxydgdnojp)
        return await _ysuspbotky(query, agents)
    except Exception:
        return _feiombshoi(query)
_BUILDJ_TAG_m31 = "sn45-181067dee56e"
