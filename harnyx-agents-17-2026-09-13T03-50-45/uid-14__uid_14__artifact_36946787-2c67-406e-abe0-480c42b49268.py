...
_smixytzhht = {'started': None, 'text': None}
_moszhfvigl = 24000
_qzcqzrlxlz = 290.0
_ggeaqvlvsc = 250.0

def _fxebitqnxz() -> float:
    started = _smixytzhht['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _mgrguvqhct() -> float:
    return _qzcqzrlxlz - _fxebitqnxz()
_jskjooqbtm = _hsapi.llm_chat
_fcijujknsu = _hsapi.search_web
_zbuuagboch = _hsapi.fetch_page
_drbetamlzd = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _kymrjmrgyq(*args, **kwargs):
    if _fxebitqnxz() >= _ggeaqvlvsc:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _drbetamlzd})
            kwargs['messages'] = steered
    _result = await _jskjooqbtm(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _koseoylxgd(_result)
    return _result

async def _jdvfmqmshj(*args, **kwargs):
    if _fxebitqnxz() >= _ggeaqvlvsc:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _fcijujknsu(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _igllkbfckh(*args, **kwargs):
    if _fxebitqnxz() >= _ggeaqvlvsc:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zbuuagboch(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _kymrjmrgyq
_hsapi.search_web = _jdvfmqmshj
_hsapi.fetch_page = _igllkbfckh
_diywvoebhf = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_qpdrowvgqp = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_xzdjiklnmd = 900
_trjztzonpb = 2

def _xbtncuvkiw(query: Query) -> int:
...
def _nqhyrwnzio(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _jrqkdtiykp(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _xbtncuvkiw(query)
    if fields >= 3:
        return 2
    if _nqhyrwnzio(lowered, _diywvoebhf):
        return 1
    if fields <= _trjztzonpb and len(text) <= _xzdjiklnmd:
        return 0
    if _nqhyrwnzio(lowered, _qpdrowvgqp):
        return 0
    return 1

def _koseoylxgd(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _smixytzhht['text'] = text.strip()[:_moszhfvigl]
    except Exception:
        pass

def _sdlynlcdht(text: str):
...
def _lmpfthvadc(query: Query) -> Response:
    text = _smixytzhht['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_moszhfvigl]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _sdlynlcdht(text)
...
"""
    ...
    *3,100 (footnoted); (12.4) negative; 2,567 lb with a unit; =WR and CR marks;
    ...
    """
        ...
        return response
    return query
_xclqbezcoj = _liruczhyvm()
_xhbstqeszp = _jxkzzmppcv()
_efaivvvtmi = _fwogxdqzhk()
_qzcqzrlxlz = 290.0
_cvilgkjrhu = 250.0
_ytdqodifki = 90.0

async def _auzbvkunxc(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _qzcqzrlxlz - (time.monotonic() - started)
        if first:
            budget = _cvilgkjrhu if _cvilgkjrhu < remaining else remaining
            first = False
        else:
            if remaining < _ytdqodifki:
                break
        ...
        try:
            ...
            last_exc = exc
    return _lmpfthvadc(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _smixytzhht['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _auzbvkunxc(query, (_efaivvvtmi, _xclqbezcoj, _xhbstqeszp))
        index = _jrqkdtiykp(query)
        if index == 0:
            agents = (_xclqbezcoj, _xhbstqeszp, _efaivvvtmi)
        elif index == 1:
            agents = (_xhbstqeszp, _efaivvvtmi, _xclqbezcoj)
        elif index == 2:
            agents = (_efaivvvtmi, _xclqbezcoj, _xhbstqeszp)
        else:
            agents = (_xclqbezcoj, _xhbstqeszp, _efaivvvtmi)
        return await _auzbvkunxc(query, agents)
    except Exception:
        return _lmpfthvadc(query)
_BUILDJ_TAG_m32 = "sn45-25006867d839"
