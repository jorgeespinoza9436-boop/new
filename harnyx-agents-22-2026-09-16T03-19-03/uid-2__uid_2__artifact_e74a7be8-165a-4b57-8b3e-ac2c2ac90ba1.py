...
def _compose_branch34534862_a_a95_entry():
...
_branch34534862_a_a95_query_entry = _compose_branch34534862_a_a95_entry()


def _compose_branch3_c44203_c_f6_a6_entry():
...
_branch3_c44203_c_f6_a6_query_entry = _compose_branch3_c44203_c_f6_a6_entry()


def _compose_branch5_d7_f5_c4_e324_a_entry():
    ...
    def _rt_prose_entry():
        ...
        _IM67_SEARCH_PROVIDERS = ("parallel",)
...
_branch5_d7_f5_c4_e324_a_query_entry = _compose_branch5_d7_f5_c4_e324_a_entry()
...
class Branch34534862AA95:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch34534862_a_a95_query_entry(query=query)


class Branch3C44203CF6A6:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch3_c44203_c_f6_a6_query_entry(query=query)


class Branch5D7F5C4E324A:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch5_d7_f5_c4_e324_a_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = Branch34534862AA95()
_MODE_SHAPE_SECONDARY_AGENT = Branch3C44203CF6A6()
_MODE_SHAPE_TERTIARY_AGENT = Branch5D7F5C4E324A()
_CANDIDATE_BRANCH_CLASS_NAMES = ("Branch34534862AA95", "Branch3C44203CF6A6", "Branch5D7F5C4E324A")
_CANDIDATE_ROUTE_FUNCTION = "_mode_shape_route_index"


async def _gb050_base_query(query: Query, context: ContextSnapshot) -> Response:
...
#   2. Rename its entrypoint `query` -> `_gb050_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _gb050_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _Gb050_TAG below to a value unique to this agent.
#
# TUNE: _Gb050_SKIP_AFTER_S = 252.0 assumes a ~300s wall. Lower it if the host
...
import asyncio as _gb050_asyncio
import json as _gb050_json
import re as _gb050_re
from time import monotonic as _gb050_monotonic

from harnyx_miner_sdk.api import fetch_page as _gb050_fetch_page
from harnyx_miner_sdk.api import llm_chat as _gb050_llm_chat
from harnyx_miner_sdk.api import search_web as _gb050_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as __Gb050Cite
from harnyx_miner_sdk.query import CitationSlice as __Gb050Cite
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_Gb050_PROVIDER = "openrouter"
_Gb050_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_Gb050_SEARCH_PROVIDERS = ("parallel",)
_Gb050_CHAT_TIMEOUT_S = 12.0
_Gb050_SEARCH_TIMEOUT_S = 12.0
_Gb050_FETCH_TIMEOUT_S = 14.0
_Gb050_SKIP_AFTER_S = 252.0
_Gb050_ANSWER_CAP = 60000
_Gb050_NOTE_CAP = 8000
_Gb050_MAX_CITES = 32
_Gb050_FENCE_RE = _gb050_re.compile(r"^```(?:json)?\s*|\s*```$", _gb050_re.I | _gb050_re.M)
_Gb050_POINTER_RE = _gb050_re.compile(r"\[\[(\d+)\]\]")


def _gb050_parse_json(text):
    if not text:
        return None
    body = _Gb050_FENCE_RE.sub("", text).strip()
    try:
        value = _gb050_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _gb050_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _gb050_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _gb050_choice_text(content):
    if isinstance(content, str):
        return content
    if isinstance(content, list):
        parts = []
...
def _gb050_chat_text(payload):
    ...
    if not choices:
        return ""
    message = getattr(choices[0], "message", None)
    return _gb050_choice_text(getattr(message, "content", None)).strip()


async def _gb050_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _Gb050_MODELS:
        try:
            payload = await _gb050_asyncio.wait_for(
                _gb050_llm_chat(
                    provider=_Gb050_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _gb050_chat_text(payload)
        if text:
            return text
    return ""


async def _gb050_search(query_text):
    for provider in _Gb050_SEARCH_PROVIDERS:
        try:
            payload = await _gb050_asyncio.wait_for(
                _gb050_search_web(provider=provider, query=query_text),
                timeout=_Gb050_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _gb050_fetch(url):
    try:
        return await _gb050_asyncio.wait_for(
            _gb050_fetch_page(provider="parallel", url=url),
            timeout=_Gb050_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _gb050_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _gb050_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        ...
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(__Gb050Cite(start=start, end=end))
        out.append(__Gb050Cite(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _Gb050_MAX_CITES:
            break
    return out


def _gb050_merge_row(citations, row):
    for position, ref in enumerate(citations):
        ...
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _Gb050_MAX_CITES:
        return None
    note = row["note"]
    citations.append(__Gb050Cite(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[__Gb050Cite(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _gb050_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _gb050_merge_row(citations, row)
        if pointer is None:
            continue
...
def _gb050_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_Gb050_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _gb050_json.dumps(output)[:_Gb050_ANSWER_CAP]
    except Exception:
        return str(output)[:_Gb050_ANSWER_CAP]


def _gb050_normalize_pointers(text, n_cites):
    ...
    return _Gb050_POINTER_RE.sub(_keep, text)


def _gb050_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_gb050_normalize_pointers(text, n)[:_Gb050_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
...
def _gb050_pointer_only(response):
    citations = _gb050_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_gb050_normalize_pointers(text, len(citations))[:_Gb050_ANSWER_CAP],
            citations=citations,
        )
    return response
...
async def _gb050_audit(question, draft):
    text = await _gb050_chat(
        _Gb050_AUDIT_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:12000],
        900,
        _Gb050_CHAT_TIMEOUT_S,
    )
    parsed = _gb050_parse_json(text)
    if parsed is None:
        return {}
    return {
        "missing_elements": _gb050_str_list(parsed.get("missing_elements"), 6),
        "unsupported_claims": _gb050_str_list(parsed.get("unsupported_claims"), 6),
        "pool_incomplete": parsed.get("pool_incomplete") is True,
        "probes": _gb050_str_list(parsed.get("probes"), 3),
    }


async def _gb050_retrieve(question, ledger):
    probes = ledger.get("probes") or []
    if not probes:
        probes = [question[:220]]
    payloads = await _gb050_asyncio.gather(
        *[_gb050_search(probe) for probe in probes[:3]],
    ...
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_gb050_rows_from_payload(payload, "probe"))
    return rows[:18]


async def _gb050_regenerate(question, draft, ledger, rows, citations):
    board = _gb050_board(rows, citations)
    if not board:
        return None
    system = (
    ...
    user = (
        "QUESTION:\n" + question
        + "\n\nOPEN ITEMS:\n" + _gb050_json.dumps(ledger.get("missing_elements") or [])
        + "\n\nDRAFT:\n" + draft[:8000]
        + "\n\nBOARD:\n" + board[:22000]
    )
    parsed = _gb050_parse_json(await _gb050_chat(system, user, 2400, 28.0))
    if parsed is None:
        return None
    text = parsed.get("text")
    if not isinstance(text, str) or len(text.strip()) < 8:
        return None
    return _gb050_rebuild(None, text.strip(), None, citations)


async def _gb051_base_query(query: Query, context=None) -> Response:
    started = _gb050_monotonic()
    draft = await _gb050_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        if _gb050_monotonic() - started >= _Gb050_SKIP_AFTER_S:
            return _gb050_pointer_only(draft)
        citations = _gb050_copy_citations(draft)
        ledger = await _gb050_audit(question, _gb050_draft_text(draft))
        if _gb050_ledger_open(ledger):
            rows = await _gb050_retrieve(question, ledger)
            if rows:
                rewritten = await _gb050_regenerate(
                    question, _gb050_draft_text(draft), ledger, rows, citations
                )
...
# ============================================================================
# SN67 OVERLAY FAMILY F7 -- anchor coverage
# ----------------------------------------------------------------------------
# Mechanism : post-draft claim audit -> conditional fresh retrieval -> regenerate
# Gate reads: draft claims
# Gate does : re-retrieve + regenerate
...
# GRAFT IN 4 STEPS
#   1. Take the host agent file.
#   2. Rename its entrypoint `query` -> `_gb051_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _gb051_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _Gb051_TAG below to a value unique to this agent.
#
# TUNE: _Gb051_SKIP_AFTER_S = 252.0 assumes a ~300s wall. Lower it if the host
# lane is slow, or you will hit session_budget_exhausted.
#
# query.fast is deliberately skipped (except F8, which returns base directly).
# Fast is scored correctness-only F1, so extra components lower precision.
...
import asyncio as _gb051_asyncio
import json as _gb051_json
import re as _gb051_re
from time import monotonic as _gb051_monotonic

from harnyx_miner_sdk.api import fetch_page as _gb051_fetch_page
from harnyx_miner_sdk.api import llm_chat as _gb051_llm_chat
from harnyx_miner_sdk.api import search_web as _gb051_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as __Gb051Cite
from harnyx_miner_sdk.query import CitationSlice as __Gb051Cite
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_Gb051_PROVIDER = "openrouter"
_Gb051_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_Gb051_SEARCH_PROVIDERS = ("parallel",)
_Gb051_CHAT_TIMEOUT_S = 12.0
_Gb051_SEARCH_TIMEOUT_S = 12.0
_Gb051_FETCH_TIMEOUT_S = 14.0
_Gb051_SKIP_AFTER_S = 252.0
_Gb051_ANSWER_CAP = 60000
_Gb051_NOTE_CAP = 8000
_Gb051_MAX_CITES = 32
_Gb051_FENCE_RE = _gb051_re.compile(r"^```(?:json)?\s*|\s*```$", _gb051_re.I | _gb051_re.M)
_Gb051_POINTER_RE = _gb051_re.compile(r"\[\[(\d+)\]\]")


def _gb051_parse_json(text):
    if not text:
        return None
    body = _Gb051_FENCE_RE.sub("", text).strip()
    try:
        value = _gb051_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _gb051_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _gb051_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _gb051_choice_text(content):
    if isinstance(content, str):
        return content
    if isinstance(content, list):
        parts = []
...
def _gb051_chat_text(payload):
    ...
    if not choices:
        return ""
    message = getattr(choices[0], "message", None)
    return _gb051_choice_text(getattr(message, "content", None)).strip()


async def _gb051_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _Gb051_MODELS:
        try:
            payload = await _gb051_asyncio.wait_for(
                _gb051_llm_chat(
                    provider=_Gb051_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _gb051_chat_text(payload)
        if text:
            return text
    return ""


async def _gb051_search(query_text):
    for provider in _Gb051_SEARCH_PROVIDERS:
        try:
            payload = await _gb051_asyncio.wait_for(
                _gb051_search_web(provider=provider, query=query_text),
                timeout=_Gb051_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _gb051_fetch(url):
    try:
        return await _gb051_asyncio.wait_for(
            _gb051_fetch_page(provider="parallel", url=url),
            timeout=_Gb051_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _gb051_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _gb051_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        slices = []
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(__Gb051Cite(start=start, end=end))
        out.append(__Gb051Cite(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _Gb051_MAX_CITES:
            break
    return out


def _gb051_merge_row(citations, row):
    for position, ref in enumerate(citations):
        same_receipt = str(getattr(ref, "receipt_id", "") or "") == row["receipt_id"]
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _Gb051_MAX_CITES:
        return None
    note = row["note"]
    citations.append(__Gb051Cite(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[__Gb051Cite(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _gb051_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _gb051_merge_row(citations, row)
        if pointer is None:
            continue
        lines.append("[[" + str(pointer) + "]] " + row["title"] + " :: " + row["note"][:1200])
    return "\n".join(lines)
...
def _gb051_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_Gb051_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _gb051_json.dumps(output)[:_Gb051_ANSWER_CAP]
    except Exception:
        return str(output)[:_Gb051_ANSWER_CAP]


def _gb051_normalize_pointers(text, n_cites):
    if not isinstance(text, str):
        return text
    ...
    return _Gb051_POINTER_RE.sub(_keep, text)


def _gb051_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_gb051_normalize_pointers(text, n)[:_Gb051_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
        return Response(output=output, citations=citations)
...
def _gb051_pointer_only(response):
    citations = _gb051_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_gb051_normalize_pointers(text, len(citations))[:_Gb051_ANSWER_CAP],
            citations=citations,
        )
    return response

_Gb051_AUDIT_SYSTEM = (
    "You audit a research draft for anchor coverage. Anchors are the proper "
    "nouns, quoted strings, identifiers, figures and dates named in the "
    "question. Return JSON only with keys missing_elements (string array of "
    "anchors the question names that the draft never addresses), "
    "unsupported_claims (string array of anchors the draft mentions but "
    "attaches to no cited evidence), pool_incomplete (boolean, true when an "
    "anchor is addressed only by inference) and probes (string array of at "
    "most three search queries naming the uncovered anchors verbatim)."
)


def _gb051_ledger_open(ledger):
    """The research-role gate: is a question anchor still uncovered?"""
    if not isinstance(ledger, dict):
        return False
    if ledger.get("pool_incomplete") is True:
        return True
    uncovered = ledger.get("missing_elements") or []
    uncited = ledger.get("unsupported_claims") or []
    return len(uncovered) > 0 or len(uncited) > 0


async def _gb051_audit(question, draft):
    text = await _gb051_chat(
        _Gb051_AUDIT_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:12000],
        900,
        _Gb051_CHAT_TIMEOUT_S,
    )
    parsed = _gb051_parse_json(text)
    if parsed is None:
        return {}
    return {
        "missing_elements": _gb051_str_list(parsed.get("missing_elements"), 6),
        "unsupported_claims": _gb051_str_list(parsed.get("unsupported_claims"), 6),
        "pool_incomplete": parsed.get("pool_incomplete") is True,
        "probes": _gb051_str_list(parsed.get("probes"), 3),
    }


async def _gb051_retrieve(question, ledger):
    probes = ledger.get("probes") or []
    if not probes:
        probes = [question[:220]]
    payloads = await _gb051_asyncio.gather(
        *[_gb051_search(probe) for probe in probes[:3]],
        return_exceptions=True,
    )
    rows = []
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_gb051_rows_from_payload(payload, "probe"))
    return rows[:18]


async def _gb051_regenerate(question, draft, ledger, rows, citations):
    board = _gb051_board(rows, citations)
    if not board:
        return None
    system = (
        "Rewrite the answer so every anchor the question names is addressed "
        "explicitly and carries evidence from the numbered board. Name each "
        "anchor in the same form the question used. If an anchor cannot be "
        "evidenced, say so in one clause rather than omitting it. Cite with "
        "[[n]] pointers. Return JSON only with key text."
    )
    user = (
        "QUESTION:\n" + question
        + "\n\nOPEN ITEMS:\n" + _gb051_json.dumps(ledger.get("missing_elements") or [])
        + "\n\nDRAFT:\n" + draft[:8000]
        + "\n\nBOARD:\n" + board[:22000]
    )
    parsed = _gb051_parse_json(await _gb051_chat(system, user, 2400, 28.0))
    if parsed is None:
        return None
    text = parsed.get("text")
    if not isinstance(text, str) or len(text.strip()) < 8:
        return None
    return _gb051_rebuild(None, text.strip(), None, citations)


async def _zb05_entry(query: Query, context=None) -> Response:
    started = _gb051_monotonic()
    draft = await _gb051_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        if _gb051_monotonic() - started >= _Gb051_SKIP_AFTER_S:
            return _gb051_pointer_only(draft)
        citations = _gb051_copy_citations(draft)
        ledger = await _gb051_audit(question, _gb051_draft_text(draft))
        if _gb051_ledger_open(ledger):
            rows = await _gb051_retrieve(question, ledger)
            if rows:
                rewritten = await _gb051_regenerate(
                    question, _gb051_draft_text(draft), ledger, rows, citations
                )
                if rewritten is not None:
                    return rewritten
        return _gb051_pointer_only(draft)
...
try:
    from harnyx_miner_sdk.api import tooling_info as _zb05_tooling_info
except ImportError:
    _zb05_tooling_info = None


class _ZB05TimeBudget:
    def __init__(self, limit_seconds):
        self.limit_seconds = limit_seconds
        self.remaining_seconds = limit_seconds


class _ZB05CostBudget:
...
class _ZB05Context:
    ...
    def __init__(self, seconds, usd):
        self.time_budget = _ZB05TimeBudget(seconds)
        self.cost_budget = _ZB05CostBudget(usd)


def _zb05_fields(info):
...
def _zb05_pick(fields, names, fallback):
    for name in names:
...
async def _zb05_context():
    seconds = 300.0
    usd = 0.5
    if _zb05_tooling_info is not None:
        try:
            fields = _zb05_fields(await _zb05_tooling_info())
            seconds = _zb05_pick(
                fields,
            ...
            pass
    return _ZB05Context(seconds, usd)


@entrypoint("query")
async def query(query: Query) -> Response:
    try:
        context = await _zb05_context()
    except Exception:
        context = _ZB05Context(300.0, 0.5)
    return await _zb05_entry(query, context)
