...
def _compose_branch95_b2_c_a_d1_f_f_d1_entry():
...
_branch95_b2_c_a_d1_f_f_d1_query_entry = _compose_branch95_b2_c_a_d1_f_f_d1_entry()
...
def _compose_branch_b_f_b7_e_a_b47_c_a_a_entry():
...
_branch_b_f_b7_e_a_b47_c_a_a_query_entry = _compose_branch_b_f_b7_e_a_b47_c_a_a_entry()


def _core_first_mode_key(query: object) -> str:
    speed = "fast" if getattr(query, "fast", False) else "normal"
    shape = "structured" if getattr(query, "output_schema", None) is not None else "text"
    return speed + "_" + shape

def _core_first_specialist_bucket(query: object) -> int:
    if getattr(query, "output_schema", None) is not None:
        return 1
    return 2


_CORE_FIRST_SPECIALIST_MODES = {'normal_text': True, 'normal_structured': True, 'fast_text': True, 'fast_structured': False}


def _core_first_route_index(query: Query) -> int:
    if _CORE_FIRST_SPECIALIST_MODES[_core_first_mode_key(query)]:
        return _core_first_specialist_bucket(query)
    return 0



class Branch95B2CAD1FFD1:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch95_b2_c_a_d1_f_f_d1_query_entry(query=query)
...
class BranchBFB7EAB47CAA:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_b_f_b7_e_a_b47_c_a_a_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = Branch95B2CAD1FFD1()
_MODE_SHAPE_SECONDARY_AGENT = BranchB31669D8E370()
_MODE_SHAPE_TERTIARY_AGENT = BranchBFB7EAB47CAA()
_CANDIDATE_BRANCH_CLASS_NAMES = ("Branch95B2CAD1FFD1", "BranchB31669D8E370", "BranchBFB7EAB47CAA")
_CANDIDATE_ROUTE_FUNCTION = "_core_first_route_index"


@entrypoint("query")
async def query(query: Query, context: ContextSnapshot) -> Response:
    route_index = _core_first_route_index(query)
    if route_index == 0:
