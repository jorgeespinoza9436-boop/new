...
def _compose_branch_b31669_d8_e370_entry():
    ...
    def _gx_pct_sum_off(question: str, answer: str) -> list:
        if not re.search(r"\b(?:breakdown|share|shares|split|proportion|percentage)s?\b",
                         question, re.I):
            return []
        if not re.search(r"percent|%", question, re.I):
            return []
        vals = [float(v) for v in re.findall(r"(\d{1,3}(?:\.\d+)?)\s*(?:%|percent)", answer)]
        if len(vals) < 3:
            return []
    ...
    def _gx_missing_scale(question: str, answer: str) -> list:
        m = re.search(r"\bin (millions|billions|thousands)\b", question, re.I)
        if not m:
            return []
        word = m.group(1).lower()
        stem = word[:-1] if word.endswith("s") else word
        if re.search(r"\b" + stem + r"s?\b", answer, re.I):
            return []
        return [word]
    ...
    def _gx_defects(question: str, answer: str) -> list:
        ...
        if scale:
            notes.append("The question fixes the numeric scale and the answer never "
                         "states it: " + ", ".join(scale))
        pct = _gx_pct_sum_off(question, answer)
        if pct:
            notes.append("The question asks for a percentage breakdown but the shares "
                         "do not add up: " + ", ".join(pct))
...
_branch_b31669_d8_e370_query_entry = _compose_branch_b31669_d8_e370_entry()


def _compose_branch0_b07_e402547_a_entry():
...
_branch0_b07_e402547_a_query_entry = _compose_branch0_b07_e402547_a_entry()
...
class BranchB31669D8E370:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_b31669_d8_e370_query_entry(query=query)


class Branch0B07E402547A:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch0_b07_e402547_a_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = Branch34534862AA95()
_MODE_SHAPE_SECONDARY_AGENT = BranchB31669D8E370()
_MODE_SHAPE_TERTIARY_AGENT = Branch0B07E402547A()
_CANDIDATE_BRANCH_CLASS_NAMES = ("Branch34534862AA95", "BranchB31669D8E370", "Branch0B07E402547A")
_CANDIDATE_ROUTE_FUNCTION = "_mode_shape_route_index"


async def _gb010_base_query(query: Query, context: ContextSnapshot) -> Response:
...
#   2. Rename its entrypoint `query` -> `_gb010_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _gb010_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _Gb010_TAG below to a value unique to this agent.
#
# TUNE: _Gb010_SKIP_AFTER_S = 252.0 assumes a ~300s wall. Lower it if the host
...
import asyncio as _gb010_asyncio
import json as _gb010_json
import re as _gb010_re
from time import monotonic as _gb010_monotonic

from harnyx_miner_sdk.api import fetch_page as _gb010_fetch_page
from harnyx_miner_sdk.api import llm_chat as _gb010_llm_chat
from harnyx_miner_sdk.api import search_web as _gb010_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as __Gb010Cite
from harnyx_miner_sdk.query import CitationSlice as __Gb010Cite
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_Gb010_PROVIDER = "openrouter"
_Gb010_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_Gb010_SEARCH_PROVIDERS = ("parallel",)
_Gb010_CHAT_TIMEOUT_S = 12.0
_Gb010_SEARCH_TIMEOUT_S = 12.0
_Gb010_FETCH_TIMEOUT_S = 14.0
_Gb010_SKIP_AFTER_S = 252.0
_Gb010_ANSWER_CAP = 60000
_Gb010_NOTE_CAP = 8000
_Gb010_MAX_CITES = 32
_Gb010_FENCE_RE = _gb010_re.compile(r"^```(?:json)?\s*|\s*```$", _gb010_re.I | _gb010_re.M)
_Gb010_POINTER_RE = _gb010_re.compile(r"\[\[(\d+)\]\]")


def _gb010_parse_json(text):
    if not text:
        return None
    body = _Gb010_FENCE_RE.sub("", text).strip()
    try:
        value = _gb010_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _gb010_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _gb010_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _gb010_choice_text(content):
    if isinstance(content, str):
        return content
...
def _gb010_chat_text(payload):
    ...
    return _gb010_choice_text(getattr(message, "content", None)).strip()


async def _gb010_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _Gb010_MODELS:
        try:
            payload = await _gb010_asyncio.wait_for(
                _gb010_llm_chat(
                    provider=_Gb010_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _gb010_chat_text(payload)
        if text:
            return text
    return ""


async def _gb010_search(query_text):
    for provider in _Gb010_SEARCH_PROVIDERS:
        try:
            payload = await _gb010_asyncio.wait_for(
                _gb010_search_web(provider=provider, query=query_text),
                timeout=_Gb010_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _gb010_fetch(url):
    try:
        return await _gb010_asyncio.wait_for(
            _gb010_fetch_page(provider="parallel", url=url),
            timeout=_Gb010_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _gb010_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _gb010_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        ...
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(__Gb010Cite(start=start, end=end))
        out.append(__Gb010Cite(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _Gb010_MAX_CITES:
            break
    return out


def _gb010_merge_row(citations, row):
    for position, ref in enumerate(citations):
        ...
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _Gb010_MAX_CITES:
        return None
    note = row["note"]
    citations.append(__Gb010Cite(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[__Gb010Cite(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _gb010_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _gb010_merge_row(citations, row)
        if pointer is None:
            continue
...
def _gb010_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_Gb010_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _gb010_json.dumps(output)[:_Gb010_ANSWER_CAP]
    except Exception:
        return str(output)[:_Gb010_ANSWER_CAP]


def _gb010_normalize_pointers(text, n_cites):
    ...
    return _Gb010_POINTER_RE.sub(_keep, text)


def _gb010_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_gb010_normalize_pointers(text, n)[:_Gb010_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
...
def _gb010_pointer_only(response):
    citations = _gb010_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_gb010_normalize_pointers(text, len(citations))[:_Gb010_ANSWER_CAP],
            citations=citations,
        )
    return response
...
async def _gb010_audit(question, draft):
    text = await _gb010_chat(
        _Gb010_AUDIT_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:12000],
        900,
        _Gb010_CHAT_TIMEOUT_S,
    )
    parsed = _gb010_parse_json(text)
    if parsed is None:
        return {}
    return {
        "missing_elements": _gb010_str_list(parsed.get("missing_elements"), 6),
        "unsupported_claims": _gb010_str_list(parsed.get("unsupported_claims"), 6),
        "pool_incomplete": parsed.get("pool_incomplete") is True,
        "probes": _gb010_str_list(parsed.get("probes"), 3),
    }


async def _gb010_retrieve(question, ledger):
    probes = ledger.get("probes") or []
    if not probes:
        probes = [question[:220]]
    payloads = await _gb010_asyncio.gather(
        *[_gb010_search(probe) for probe in probes[:3]],
    ...
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_gb010_rows_from_payload(payload, "probe"))
    return rows[:18]


async def _gb010_regenerate(question, draft, ledger, rows, citations):
    board = _gb010_board(rows, citations)
    if not board:
        return None
    system = (
    ...
    user = (
        "QUESTION:\n" + question
        + "\n\nOPEN ITEMS:\n" + _gb010_json.dumps(ledger.get("missing_elements") or [])
        + "\n\nDRAFT:\n" + draft[:8000]
        + "\n\nBOARD:\n" + board[:22000]
    )
    parsed = _gb010_parse_json(await _gb010_chat(system, user, 2400, 28.0))
    if parsed is None:
        return None
    text = parsed.get("text")
    if not isinstance(text, str) or len(text.strip()) < 8:
        return None
    return _gb010_rebuild(None, text.strip(), None, citations)


async def _gb011_base_query(query: Query, context=None) -> Response:
    started = _gb010_monotonic()
    draft = await _gb010_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        if _gb010_monotonic() - started >= _Gb010_SKIP_AFTER_S:
            return _gb010_pointer_only(draft)
        citations = _gb010_copy_citations(draft)
        ledger = await _gb010_audit(question, _gb010_draft_text(draft))
        if _gb010_ledger_open(ledger):
            rows = await _gb010_retrieve(question, ledger)
            if rows:
                rewritten = await _gb010_regenerate(
                    question, _gb010_draft_text(draft), ledger, rows, citations
                )
...
# GRAFT IN 4 STEPS
#   1. Take the host agent file.
#   2. Rename its entrypoint `query` -> `_gb011_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _gb011_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _Gb011_TAG below to a value unique to this agent.
#
# TUNE: _Gb011_SKIP_AFTER_S = 245.0 assumes a ~300s wall. Lower it if the host
# lane is slow, or you will hit session_budget_exhausted.
#
# query.fast is deliberately skipped (except F8, which returns base directly).
# Fast is scored correctness-only F1, so extra components lower precision.
...
import asyncio as _gb011_asyncio
import json as _gb011_json
import re as _gb011_re
from time import monotonic as _gb011_monotonic

from harnyx_miner_sdk.api import fetch_page as _gb011_fetch_page
from harnyx_miner_sdk.api import llm_chat as _gb011_llm_chat
from harnyx_miner_sdk.api import search_web as _gb011_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as __Gb011Cite
from harnyx_miner_sdk.query import CitationSlice as __Gb011Cite
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_Gb011_PROVIDER = "openrouter"
_Gb011_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_Gb011_SEARCH_PROVIDERS = ("parallel",)
_Gb011_CHAT_TIMEOUT_S = 12.0
_Gb011_SEARCH_TIMEOUT_S = 12.0
_Gb011_FETCH_TIMEOUT_S = 14.0
_Gb011_SKIP_AFTER_S = 245.0
_Gb011_ANSWER_CAP = 60000
_Gb011_NOTE_CAP = 8000
_Gb011_MAX_CITES = 32
_Gb011_FENCE_RE = _gb011_re.compile(r"^```(?:json)?\s*|\s*```$", _gb011_re.I | _gb011_re.M)
_Gb011_POINTER_RE = _gb011_re.compile(r"\[\[(\d+)\]\]")


def _gb011_parse_json(text):
    if not text:
        return None
    body = _Gb011_FENCE_RE.sub("", text).strip()
    try:
        value = _gb011_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _gb011_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _gb011_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _gb011_choice_text(content):
    if isinstance(content, str):
        return content
    if isinstance(content, list):
        parts = []
...
def _gb011_chat_text(payload):
    ...
    if not choices:
        return ""
    message = getattr(choices[0], "message", None)
    return _gb011_choice_text(getattr(message, "content", None)).strip()


async def _gb011_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _Gb011_MODELS:
        try:
            payload = await _gb011_asyncio.wait_for(
                _gb011_llm_chat(
                    provider=_Gb011_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _gb011_chat_text(payload)
        if text:
            return text
    return ""


async def _gb011_search(query_text):
    for provider in _Gb011_SEARCH_PROVIDERS:
        try:
            payload = await _gb011_asyncio.wait_for(
                _gb011_search_web(provider=provider, query=query_text),
                timeout=_Gb011_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _gb011_fetch(url):
    try:
        return await _gb011_asyncio.wait_for(
            _gb011_fetch_page(provider="parallel", url=url),
            timeout=_Gb011_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _gb011_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _gb011_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        slices = []
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(__Gb011Cite(start=start, end=end))
        out.append(__Gb011Cite(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _Gb011_MAX_CITES:
            break
    return out


def _gb011_merge_row(citations, row):
    for position, ref in enumerate(citations):
        same_receipt = str(getattr(ref, "receipt_id", "") or "") == row["receipt_id"]
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _Gb011_MAX_CITES:
        return None
    note = row["note"]
    citations.append(__Gb011Cite(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[__Gb011Cite(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _gb011_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _gb011_merge_row(citations, row)
        if pointer is None:
            continue
        lines.append("[[" + str(pointer) + "]] " + row["title"] + " :: " + row["note"][:1200])
    return "\n".join(lines)
...
def _gb011_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_Gb011_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _gb011_json.dumps(output)[:_Gb011_ANSWER_CAP]
    except Exception:
        return str(output)[:_Gb011_ANSWER_CAP]


def _gb011_normalize_pointers(text, n_cites):
    if not isinstance(text, str):
        return text
    ...
    return _Gb011_POINTER_RE.sub(_keep, text)


def _gb011_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_gb011_normalize_pointers(text, n)[:_Gb011_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
        return Response(output=output, citations=citations)
...
def _gb011_pointer_only(response):
    citations = _gb011_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_gb011_normalize_pointers(text, len(citations))[:_Gb011_ANSWER_CAP],
            citations=citations,
        )
    return response
...
_Gb011_POOL_ROUNDS = 2


def _gb011_pool_short(state):
...
async def _gb011_assess(question, draft):
    parsed = _gb011_parse_json(await _gb011_chat(
        _Gb011_POOL_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:10000],
        700,
        _Gb011_CHAT_TIMEOUT_S,
    ))
    if parsed is None:
    ...
    return {
        ...
        "probes": _gb011_str_list(parsed.get("probes"), 2),
    }


async def _gb011_widen(state, question):
    probes = state.get("probes") or [question[:200]]
    payloads = await _gb011_asyncio.gather(
        *[_gb011_search(probe) for probe in probes[:2]],
        return_exceptions=True,
    )
    rows = []
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_gb011_rows_from_payload(payload, "widen"))
    return rows[:14]


async def _gb011_render(question, draft, rows, citations):
    board = _gb011_board(rows, citations)
    if not board:
        return None
    system = (
    ...
    parsed = _gb011_parse_json(await _gb011_chat(system, user, 2400, 26.0))
    if parsed is None:
        return None
    text = parsed.get("text")
    if not isinstance(text, str) or len(text.strip()) < 8:
...
async def _zb01_entry(query: Query, context=None) -> Response:
    started = _gb011_monotonic()
    draft = await _gb011_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        citations = _gb011_copy_citations(draft)
        body = _gb011_draft_text(draft)
        improved = None
        for _round in range(_Gb011_POOL_ROUNDS):
            if _gb011_monotonic() - started >= _Gb011_SKIP_AFTER_S:
                break
            state = await _gb011_assess(question, body)
            if not _gb011_pool_short(state):
                break
            rows = await _gb011_widen(state, question)
            if not rows:
                break
            text = await _gb011_render(question, body, rows, citations)
            if text is None or text == body:
                break
            body = text
            improved = text
        if improved is not None:
            return _gb011_rebuild(draft, improved, None, citations)
        return _gb011_pointer_only(draft)
    except Exception:
        return draft
...
try:
    from harnyx_miner_sdk.api import tooling_info as _zb01_tooling_info
except ImportError:
    _zb01_tooling_info = None


class _ZB01TimeBudget:
    def __init__(self, limit_seconds):
        self.limit_seconds = limit_seconds
        self.remaining_seconds = limit_seconds


class _ZB01CostBudget:
...
class _ZB01Context:
    ...
    def __init__(self, seconds, usd):
        self.time_budget = _ZB01TimeBudget(seconds)
        self.cost_budget = _ZB01CostBudget(usd)


def _zb01_fields(info):
...
def _zb01_pick(fields, names, fallback):
    for name in names:
...
async def _zb01_context():
    seconds = 300.0
    usd = 0.5
    if _zb01_tooling_info is not None:
        try:
            fields = _zb01_fields(await _zb01_tooling_info())
            seconds = _zb01_pick(
                fields,
            ...
            pass
    return _ZB01Context(seconds, usd)


@entrypoint("query")
async def query(query: Query) -> Response:
    try:
        context = await _zb01_context()
    except Exception:
        context = _ZB01Context(300.0, 0.5)
    return await _zb01_entry(query, context)
