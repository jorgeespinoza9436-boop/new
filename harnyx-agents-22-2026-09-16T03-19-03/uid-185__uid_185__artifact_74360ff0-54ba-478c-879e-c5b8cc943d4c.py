...
def _compose_branch3_e037444_f_a_a_e_entry():
    ...
    LANE_A = "openrouter"
    LANE_B = "ai_gateway"
    MODEL_A = "z-ai/glm-5.3-flash"
    MODEL_B = "zai/glm-5.3-flash"
    AUDIT_MODEL = "openai/gpt-oss-120b"
    SCHEMA_MODEL = "openai/gpt-oss-120b"
    REPAIR_MODEL = "deepseek/deepseek-v3.2"
    SEARCH_LANES = ("parallel", "exa", "tavily")
    FETCH_LANES = ("parallel", "exa", "firecrawl")
    ...
    _QUOTE_RE = re.compile(r'"([^"]{6,160})"|“([^”]{6,160})”')
    _YEAR_RE = re.compile(r"\b((?:19|20)\d{2})\b")
    _URL_RE = re.compile(r"https?://[^\s\]\)\"']+")
    _SITE_RE = re.compile(r"\bsite:\S+", re.I)
    _FENCE_RE = re.compile(r"^```(?:json)?\s*|\s*```$", re.I | re.M)
    _STUB_RE = re.compile(
        r"no verifiable source-backed|best-effort answer unavailable|"
        r"i (?:cannot|can't|am unable)|unable to (?:verify|find|answer)|"
        r"(?:sources|excerpts|board|evidence|documents|notes) do not (?:contain|include)|"
        r"the complete (?:list|table|roster) (?:is|was) (?:not|missing)",
        re.I,
    )
    _DUMP_RE = re.compile(
        r"jump to main|page \d+ of \d+|table of contents|click here to|"
        r"medicare state operations manual",
        re.I,
    )
    _SET_RE = re.compile(
        r"\b(?:list|name|which|what|how many)\b(?:\s+\w+){0,5}\s+"
        r"(?:all|every|each|both)\b|\bcomplete (?:list|roster|set)\b|\bhow many\b",
        re.I,
    )
    _PEAK_RE = re.compile(
        r"\b(?:highest|lowest|largest|smallest|most|least|first|last|only|"
        r"maximum|minimum|oldest|newest)\b",
        re.I,
    )
    _ROW_KEY_RE = re.compile(
        r"(?:\*|asterisk|\bdagger\b).{0,80}(?:denot|indicat|mark|mean)|"
        r"(?:denot|indicat|mark).{0,80}(?:\*|asterisk)",
        re.I | re.S,
    )
    _DATAFILE_RE = re.compile(r"\bdatafiles?\b|\bfiletype:json\b|\bmachine[- ]readable\b", re.I)
    _STOP = frozenset(
        "the and for with from that this have has was were are been its their "
        "which what when where who how many much also into over under between "
        "during against about after before while other more most than not "
        "please according identify name list compare using only".split()
    )
    ...
    DESK_TOOLS = [
        {
            ...
            "function": {
                "name": "web_search",
                "description": "Search the web. Returns numbered results with title, URL, excerpt.",
                "parameters": {
                    "type": "object",
        ...
        },
        {
            ...
            "function": {
                "name": "read_page",
                "description": "Fetch a URL. Long pages show the head plus focused windows.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "url": {"type": "string"},
        ...
        },
        {
            ...
            "function": {
                "name": "page_grep",
                "description": "Search inside an already fetched page. Repeat to walk remaining matches.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "url": {"type": "string"},
        ...
        },
        {
            ...
            "function": {
                "name": "page_read",
                "description": "Read a character range of an already fetched page.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "url": {"type": "string"},
        ...
        },
        {
            ...
            "function": {
                "name": "keep_quote",
                "description": "Pin a verbatim quote from a numbered result so the citation contains it.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "source": {"type": "string"},
        ...
        },
        {
            ...
            "function": {
                "name": "sec_filing",
                "description": "Resolve a company form-year to the EDGAR primary document URL.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "company": {"type": "string"},
    ...
    HOP_RULES = (
        "You are HopDesk, a cited research worker. A judge scores only claims whose "
        "citation slice contains the supporting words.\n"
        "Lead with the asked entities. Cite [n] on every load-bearing sentence.\n"
        "Primary sources beat encyclopedias. Fetch the named publication, then grep "
        "its tables until the whole requested population is in view.\n"
        "page_grep reports a match count. If it says more matches exist, keep grepping "
        "or page_read around offsets before stating a complete list or a count.\n"
        "The moment a figure, name, or row proves a claim, call keep_quote with the "
        "verbatim words. Also pin quotes that confirm the question's own premises.\n"
        "Respect table keys, footnotes, and row marks. A star or dagger binds only "
        "the row it sits on. Compare only the requested year, edition, and column.\n"
        "If two named documents must be joined, fetch both, then keep only members "
        "that survive every stated condition in both.\n"
        "Independent lookups belong in the same turn so they run together.\n"
        "Never refuse. A cited partial answer scores; 'the sources do not contain it' "
        "scores zero. Never mix tool calls with the final answer."
    )

    SET_RULE = (
        "SET / ROSTER: name the complete candidate pool from the requested table, "
        "then give a verdict for every member. A truncated list is a wrong answer."
    )
    PEAK_RULE = (
        "SUPERLATIVE: show the comparison table before naming a winner. A count "
        "without the enumerated pool is a guessed tally."
    )
    KEY_RULE = (
        "ROW MARK: a leading asterisk, dagger, or named footnote binds only that row. "
        "Unmarked neighbours do not inherit it."
    )
    FILE_RULE = (
        "DATAFILE: prefer the machine index (json / datafile URL) over an HTML wrapper "
        "of the same title."
    )
    ...
    def _spend_note(payload: object) -> None:
        budget = getattr(payload, "budget", None)
        left = getattr(budget, "session_remaining_budget_usd", None)
        if isinstance(left, (int, float)):
            _SPEND["left"] = float(left)
    ...
    def _quoted_titles(question: str) -> list[str]:
        found: list[str] = []
        for match in _QUOTE_RE.finditer(question or ""):
    ...
    def _named_hosts(question: str) -> list[str]:
        low = (question or "").lower()
        hosts: list[str] = []
        mapping = (
            ("booker", "thebookerprizes.com"),
            ("safer seas", "ntsb.gov"),
            ("ntsb", "ntsb.gov"),
            ("american alpine", "publications.americanalpineclub.org"),
            ("volcanic activity", "volcano.si.edu"),
            ("smithsonian", "volcano.si.edu"),
            ("mineral commodity", "pubs.usgs.gov"),
            ("net import reliance", "usgs.gov"),
            ("rfc editor", "rfc-editor.org"),
            ("rfc 7231", "rfc-editor.org"),
            ("canada gazette", "gazette.gc.ca"),
            ("statistical abstract", "nps.gov"),
            ("jwst", "jwst-docs.stsci.edu"),
            ("herald chancellor", "gazette.gc.ca"),
            ("world athletics", "worldathletics.org"),
            ("noaa fisheries", "fisheries.noaa.gov"),
            ("certification officer", "gov.uk"),
            ("fhwa", "fhwa.dot.gov"),
        )
        for needle, host in mapping:
            if needle in low:
                hosts.append(host)
        return _unique(hosts, 4)


    def _hop_queries(question: str) -> list[str]:
        queries: list[str] = []
        titles = _quoted_titles(question)
        hosts = _named_hosts(question)
        years = _YEAR_RE.findall(question or "")
        for title in titles[:4]:
            queries.append(f'"{title}"')
            if hosts:
                queries.append(f'"{title}" site:{hosts[0]}')
            if years:
                queries.append(f'"{title}" {years[-1]}')
        if hosts:
            core = " ".join(_tokens(question)[:8])
            queries.append(f"{core} site:{hosts[0]}")
        if _DATAFILE_RE.search(question or ""):
            if titles:
                queries.append(f'"{titles[0]}" datafile filetype:json')
        for url in _URL_RE.findall(question or ""):
            queries.append(url)
        if not queries:
            queries.append(" ".join(_tokens(question)[:12]))
        return _unique(queries, MAX_SEED)


    def _windows(note: str, terms: list[str], width: int, k: int) -> list[tuple[int, int]]:
        text = note or ""
        if len(text) <= width:
            return [(0, len(text))]
        low = text.lower()
        ...
        for term in terms:
            ...
            while len(hits) < 80:
                ...
                if pos < 0:
                    break
                hits.append(pos)
                start = pos + max(3, len(term))
        if not hits:
            return [(0, min(width, len(text)))]
        scored: list[tuple[int, int, int]] = []
        step = max(400, width // 3)
        for start in range(0, max(1, len(text) - width + 1), step):
            end = min(len(text), start + width)
            score = sum(1 for pos in hits if start <= pos < end)
            scored.append((score, -start, start))
        ...
        for score, _, start in scored:
            ...
            if len(out) >= k:
                break
        if not out:
            out.append((0, min(width, len(text))))
        return out


    def _snap_lines(text: str, start: int, end: int, window: int) -> tuple[int, int]:
        a = max(0, start)
        b = min(len(text), end)
        nl = text.rfind("\n", max(0, a - window), a)
        if nl != -1 and a - nl <= window:
            a = nl + 1
        nl = text.find("\n", b, min(len(text), b + window))
        if nl != -1 and nl - b <= window:
            b = nl
        if b - a > 3 * window:
            b = a + 3 * window
        return a, max(a + 1, b)
    ...
    def _grow_span(start: int, end: int, size: int, floor: int) -> tuple[int, int]:
        start = max(0, min(start, max(0, size)))
        end = max(start, min(end, size))
        need = max(floor, end - start)
        if end - start >= need or size <= 0:
            return start, max(start + 1, end)
        extra = need - (end - start)
        left = min(extra // 2, start)
        start -= left
        rest = extra - left
        right = min(rest, size - end)
        end += right
        start = max(0, start - (rest - right))
        end = min(size, max(end, start + min(need, size)))
        if end - start < min(floor, size):
            end = min(size, start + min(floor, size))
            if end - start < min(floor, size):
                start = max(0, end - min(floor, size))
        return start, max(start + 1, end)


    def _merge_spans(spans: list[tuple[int, int]]) -> list[tuple[int, int]]:
        ordered = sorted((max(0, a), max(a + 1, b)) for a, b in spans if b > a)
        merged: list[list[int]] = []
        for a, b in ordered:
            if merged and a <= merged[-1][1]:
    ...
    def _desk_find(desk: Desk, url: str) -> tuple[int, dict] | None:
        target = (url or "").rstrip("/").lower()
        if target.isdigit():
            n = int(target)
            if 1 <= n <= len(desk.rows):
                return n, desk.rows[n - 1]
        for index, row in enumerate(desk.rows, start=1):
            if (row.get("url") or "").rstrip("/").lower() == target:
                return index, row
        return None


    def _commit(desk: Desk, payload: object, kind: str, preview: int) -> str:
        ...
        for item in results:
            ...
            spans = [(0, min(len(note), max(SEARCH_PREVIEW, CITE_FLOOR)))] if kind == "search" else [(0, min(len(note), HEAD_CHARS))]
            n = desk.add(receipt, rid, str(note), kind, spans, title or url, url)
            if n >= MAX_ROWS:
                break
            snippet = str(note)[:preview].replace("\n", " ")
            lines.append(f"[{n}] {title or url} — {url}\n    {snippet}")
        return "\n".join(lines) if lines else "# no citable rows"


    async def _search(query_text: str, desk: Desk) -> str:
        q = " ".join((query_text or "").split())[:280]
        if len(q) < 4:
            return "# web_search: empty"
        memo_key = "s:" + q.lower()
        hit = _MEMO.get(memo_key)
        if hit:
            return hit
        degraded = _SITE_RE.sub("", q).replace('"', " ")
        attempts = _unique([q, " ".join(degraded.split())], 3)
        payload = None
        for attempt in attempts:
            for provider in SEARCH_LANES:
                try:
                    payload = await search_web(attempt, provider=provider, num=8, timeout=SEARCH_S)
                    _spend_note(payload)
                    if getattr(payload, "results", None):
                        break
                except Exception:
        ...
        body = _commit(desk, payload, "search", SEARCH_PREVIEW)
        if body.startswith("["):
            _MEMO[memo_key] = body
        return f"# web_search({q!r})\n{body}"


    async def _fetch(url: str, focus: str, question: str, desk: Desk) -> str:
        url = (url or "").strip()
        if not url.startswith("http"):
            return "# read_page: empty url"
        key = url.rstrip("/").lower()
        if key in _DEAD:
            return f"# read_page({url!r}): previously empty"
        existing = _desk_find(desk, url)
        if existing is not None and len((existing[1].get("note") or "")) > 800:
            n, row = existing
            return f"# read_page({url!r}) already [{n}] {len(row.get('note') or '')} chars"
        payload = None
        for provider in FETCH_LANES:
            try:
                payload = await fetch_page(url, provider=provider, timeout=FETCH_S)
                _spend_note(payload)
                if getattr(payload, "results", None):
                    break
            except Exception:
        ...
        if not isinstance(rid, str) or not rid or not str(note).strip():
            return f"# read_page({url!r}): no content"
        note_s = str(note)
        terms = _tokens(question) + _tokens(focus)
        if len(note_s) <= PLAIN_CHARS:
            spans = [(0, len(note_s))]
            shown = note_s
        else:
            windows = _windows(note_s, terms[:18], FOCUS_CHARS, 3)
            spans = [(0, min(HEAD_CHARS, len(note_s)))] + windows
            chunks = [note_s[:HEAD_CHARS]]
            for a, b in windows:
                chunks.append(f"\n--- @{a}:{b} ---\n{note_s[a:b]}")
            shown = "\n".join(chunks)
        n = desk.add(receipt, rid, note_s, "fetch", spans, url, url)
        return f"# read_page({url!r}) -> [{n}] {len(note_s)} chars\n{shown[:12000]}"


    def _page_grep(url: str, pattern: str, desk: Desk) -> str:
        hit = _desk_find(desk, url)
        if hit is None:
            return f"# page_grep: {url!r} not fetched; call read_page first"
        n, row = hit
        ...
        for match in rx.finditer(text):
            c = (match.start() + match.end()) // 2
            if centres and c - centres[-1] < GREP_NARROW // 2:
                continue
            centres.append(c)
            if len(centres) > 4000:
                break
        ...
        key = f"{n}:{pat.casefold()}"
        cursor = min(_GREP_CUR.get(key, 0), len(centres))
        batch = centres[cursor: cursor + GREP_HITS]
        _GREP_CUR[key] = cursor + len(batch)
        win = GREP_WINDOW if len(centres) <= GREP_NARROW_AT else GREP_NARROW
        parts: list[str] = []
        spent = 0
        kept = row.setdefault("kept", [])
        for c in batch:
            a, b = _snap_lines(text, c - win // 2, c + win // 2, win)
            if spent + (b - a) > GREP_CHAR_BUDGET:
                break
            spent += b - a
            parts.append(f"\n--- match @{a} ---\n{text[a:b]}")
            if len(centres) <= GREP_NARROW_AT or len(parts) <= 2:
                kept.append((a, b))
        more = len(centres) - (cursor + len(batch))
        head = (
            f"# page_grep({pat!r}) on [{n}] -> {len(centres)} match(es) of {len(text)} chars; "
            f"showing {len(parts)} from offset {cursor}"
        )
        if more > 0 or cursor + len(batch) < len(centres):
            head += (
                f". {len(centres) - cursor - len(parts)} MORE MATCHES remain — repeat this "
                "same page_grep before stating a complete list or a count."
            )
        return head + "".join(parts)


    def _page_read(url: str, offset: object, length: object, desk: Desk) -> str:
        hit = _desk_find(desk, url)
        if hit is None:
            return f"# page_read: {url!r} not fetched"
        n, row = hit
        text = row.get("note") or ""
        try:
            a = max(0, int(offset or 0))
        except Exception:
            a = 0
        try:
            want = int(length or READ_CHARS)
        except Exception:
            want = READ_CHARS
        b = min(len(text), a + max(1, min(want, READ_CHARS)))
        a = min(a, max(0, len(text) - 1))
        b = max(a + 1, b)
        row.setdefault("kept", []).append((a, b))
        return f"# page_read([{n}] @{a}:{b} of {len(text)})\n{text[a:b]}"


    def _keep_quote(source: str, quote: str, desk: Desk) -> str:
        raw = (source or "").strip().strip("[]")
        try:
            n = int(raw)
        except Exception:
            return f"# keep_quote: need a result number, got {source!r}"
        if not (1 <= n <= len(desk.rows)):
            return f"# keep_quote: no [{n}]"
        row = desk.rows[n - 1]
        text = row.get("note") or ""
        q = (quote or "").strip()
        if len(q) < RETAIN_MIN:
            return f"# keep_quote: quote too short ({len(q)})"
        pos = text.find(q)
        if pos < 0:
            pos = text.lower().find(q.lower())
        if pos < 0:
            return f"# keep_quote: quote not in [{n}] — copy it exactly"
        a = max(0, pos - RETAIN_PAD)
        b = min(len(text), pos + len(q) + RETAIN_PAD)
        a, b = _grow_span(a, b, len(text), CITE_FLOOR)
        kept = row.setdefault("kept", [])
        for index, (ka, kb) in enumerate(kept):
            if a <= kb and ka <= b:
                kept[index] = (min(ka, a), max(kb, b))
                return f"# keep_quote: merged into [{n}] ({kept[index][1] - kept[index][0]} chars)"
        if len(kept) >= RETAIN_MAX:
            return f"# keep_quote: [{n}] already has {len(kept)} pins"
        kept.append((a, b))
        return f"# keep_quote: kept {b - a} chars of [{n}]. Cite [{n}]."


    async def _fetch_json(url: str, deadline: float) -> dict | None:
        cached = _JSON_CACHE.get(url)
        if isinstance(cached, dict):
            return cached
        if _left(deadline) < 12.0:
            return None
        for provider in FETCH_LANES[:3]:
            try:
                payload = await fetch_page(url, provider=provider, timeout=min(FETCH_S, _left(deadline) - 2.0))
                _spend_note(payload)
                results = list(getattr(payload, "results", None) or [])
                note = (getattr(results[0], "note", None) or "") if results else ""
                start = note.find("{")
    ...
    async def _sec_filing(company: str, form: str, year: str, deadline: float) -> str:
        company = (company or "").strip()
        form = " ".join((form or "10-K").upper().replace("FORM", " ").split())
        year = (year or "").strip()[:4]
        if not company or _left(deadline) < 16.0:
            return "# sec_filing: missing company"
        tickers = await _fetch_json(SEC_TICKERS, deadline)
        if not isinstance(tickers, dict):
            return "# sec_filing: ticker index missing"
        want = [tok for tok in re.findall(r"[a-z0-9]+", company.lower()) if tok not in _STOP]
        best = None
        for row in tickers.values():
            if not isinstance(row, dict):
                continue
            ...
            if len(want) == 1 and ticker == want[0]:
                score = 100
            elif want and all(tok in words for tok in want):
                score = 40 + len(want)
            else:
                continue
            cand = (score, -len(title), str(row.get("cik_str", "")).zfill(10))
            if best is None or cand > best:
                best = cand
        if best is None:
            return f"# sec_filing: no CIK for {company!r}"
        cik = best[2]
        subs = await _fetch_json(SEC_SUBS.format(cik=cik), deadline)
        if not isinstance(subs, dict):
            return f"# sec_filing: no submissions for {cik}"
        recent = (subs.get("filings") or {}).get("recent") if isinstance(subs.get("filings"), dict) else None
        if not isinstance(recent, dict):
            return "# sec_filing: no recent filings"
        forms = recent.get("form")
        accs = recent.get("accessionNumber")
        docs = recent.get("primaryDocument")
        rdates = recent.get("reportDate")
        if not (isinstance(forms, list) and isinstance(accs, list) and isinstance(docs, list)):
            return "# sec_filing: malformed index"
        pick = None
        for i in range(min(len(forms), len(accs), len(docs))):
            if str(forms[i]).upper().replace(" ", "") != form.replace(" ", ""):
                continue
            acc = str(accs[i] or "").replace("-", "")
            doc = str(docs[i] or "")
            if not acc or not doc:
                continue
            rd = str(rdates[i]) if isinstance(rdates, list) and i < len(rdates) else ""
            if year and rd[:4] != year:
                continue
            pick = (acc, doc)
            break
        if pick is None:
            return f"# sec_filing: no {form} {year} for {company}"
        acc, doc = pick
        url = SEC_DOC.format(cik=int(cik), acc=acc, doc=doc)
        return f"# sec_filing {company} {form} {year} -> {url}"


    async def _run_tool(call: object, question: str, desk: Desk, deadline: float) -> str:
        try:
            args = json.loads(getattr(call, "arguments", None) or "{}")
        except Exception:
            args = {}
        ...
        name = getattr(call, "name", "") or ""
        if name == "web_search":
            return await _search(str(args.get("query") or ""), desk)
        if name == "read_page":
            return await _fetch(str(args.get("url") or ""), str(args.get("focus") or ""), question, desk)
        if name == "page_grep":
            return _page_grep(str(args.get("url") or ""), str(args.get("pattern") or ""), desk)
        if name == "page_read":
            return _page_read(str(args.get("url") or ""), args.get("offset") or 0, args.get("length") or READ_CHARS, desk)
        if name == "keep_quote":
            return _keep_quote(str(args.get("source") or ""), str(args.get("quote") or ""), desk)
        if name == "sec_filing":
            return await _sec_filing(str(args.get("company") or ""), str(args.get("form") or ""), str(args.get("year") or ""), deadline)
        return f"# unknown tool {name!r}"


    def _think(model: str, finish: bool) -> dict:
    ...
    async def _chat_turn(messages: list, deadline: float, finish: bool) -> object | None:
        payload_chars = 0
        for msg in messages:
            if isinstance(msg, dict):
                payload_chars += len(str(msg.get("content") or ""))
        lanes = ((LANE_A, MODEL_A), (LANE_B, MODEL_B))
        for provider, model in lanes:
            if provider == LANE_B and payload_chars > LANE_B_CAP:
                continue
            timeout = min(TURN_S, max(8.0, _left(deadline) - 6.0))
            if finish:
                timeout = min(timeout, max(18.0, _left(deadline) - 30.0))
            if timeout <= 6.0:
                return None
            try:
                if finish:
    ...
    async def _chat_plain(provider: str, model: str, system: str, user: str, deadline: float, timeout: float, tokens: int) -> str:
        if _left(deadline) < MIN_TAIL_S + 4.0:
            return ""
        wait = min(timeout, max(6.0, _left(deadline) - 4.0))
        try:
            payload = await llm_chat(
                provider=provider,
                model=model,
    ...
    def _usable(text: str, question: str) -> bool:
        body = (text or "").strip()
        if len(body) < 24:
            return False
        if _STUB_RE.search(body[:500]):
            return False
        if _DUMP_RE.search(body[:400]):
            return False
        if body.lower().startswith(question[:80].lower()) and len(body) < 200:
            return False
        return True


    def _json_blob(raw: str) -> object | None:
    ...
    def _schema_errors(value: object, schema: object, path: str = "output", depth: int = 0) -> list[str]:
        ...
        if isinstance(value, dict):
            ...
            for key in schema.get("required") or []:
                if key not in value:
                    errors.append(f"{path}.{key}: missing")
            for key, item in value.items():
                errors.extend(_schema_errors(item, props.get(key) or {}, f"{path}.{key}", depth + 1))
        if isinstance(value, list):
            if len(value) < schema.get("minItems", 0) or len(value) > schema.get("maxItems", 100000):
                errors.append(f"{path}: array length")
            for i, item in enumerate(value[:40]):
                errors.extend(_schema_errors(item, schema.get("items") or {}, f"{path}[{i}]", depth + 1))
        return errors


    def _clamp_schema(value: object, schema: object, evidence: str, depth: int = 0) -> object:
        ...
        if isinstance(enum, list) and enum:
            if value in enum:
                return value
            blob = str(value or "") + " " + evidence
            for opt in enum:
                if isinstance(opt, str) and opt.lower() in blob.lower():
                    return opt
            return enum[0]
        kind = schema.get("type")
        if not kind:
            for key in ("anyOf", "oneOf"):
                ...
                if isinstance(branch, list):
                    for sub in branch:
                        if isinstance(sub, dict) and sub.get("type") != "null":
                            return _clamp_schema(value, sub, evidence, depth + 1)
            kind = "string"
        if kind == "object":
            props = schema.get("properties") or {}
            required = schema.get("required") or list(props.keys())
            ...
            out = {}
            for key in required:
                out[key] = _clamp_schema(src.get(key) if isinstance(src, dict) else value, props.get(key) or {}, evidence, depth + 1)
            return out
        if kind == "array":
            items = schema.get("items") or {}
            src = value if isinstance(value, list) else [value]
            mx = schema.get("maxItems", 40)
            mn = schema.get("minItems", 0)
            out = [_clamp_schema(item, items, evidence, depth + 1) for item in src[:mx]]
            while len(out) < mn:
                out.append(_clamp_schema("", items, evidence, depth + 1))
            return out
        if kind in {"number", "integer"}:
            blob = str(value or "") + " " + evidence[:2000]
            found = re.search(r"-?\d[\d,]*(?:\.\d+)?", blob)
            if found is None:
                return 0
            raw = found.group(0).replace(",", "")
            try:
                return int(raw.split(".")[0]) if kind == "integer" else float(raw)
            except Exception:
                return 0
        if kind == "boolean":
            return not re.match(r"\s*(no\b|false\b|none\b)", str(value or ""), re.I)
        text = str(value or "").strip()
        if _DUMP_RE.search(text[:200]) or len(text) > 2000:
            text = ""
        mx = schema.get("maxLength", 80000)
        mn = schema.get("minLength", 0)
        if not text:
            text = " ".join(evidence.split())[: max(mn, 24)]
        if len(text) > mx:
            text = text[:mx]
        if len(text) < mn:
            pad = " ".join(evidence.split())
            text = (text + " " + pad)[:mx]
            if len(text) < mn:
                text = (text + ("." * mn))[:mn]
                text = text[:mx]
    ...
    def _cited_numbers(answer: str, n_rows: int) -> list[int]:
        found: list[int] = []
        for match in _MARK_RE.finditer(answer or ""):
            blob = match.group(1) or match.group(2) or ""
            if blob.isdigit():
                n = int(blob)
                if 1 <= n <= n_rows and n not in found:
                    found.append(n)
        return found


    def _ref_for(row: dict) -> CitationRef | None:
        receipt = row.get("receipt_id") or ""
        rid = row.get("result_id") or ""
        note = row.get("note") or ""
        if not receipt or not rid or not note:
            return None
        spans = list(row.get("kept") or []) + list(row.get("spans") or [])
        if not spans:
            spans = [(0, min(len(note), max(CITE_FLOOR, min(CITE_GROW, len(note)))))]
        grown: list[tuple[int, int]] = []
        for a, b in spans:
            grown.append(_grow_span(int(a), int(b), len(note), CITE_FLOOR))
        merged = _merge_spans(grown)[:8]
        extra = max(0, CITE_SOFT - sum(b - a for a, b in merged))
        if merged and extra:
            pad = extra // len(merged)
            padded: list[tuple[int, int]] = []
            for a, b in merged:
                want = min(CITE_GROW, b - a + pad)
                padded.append(_grow_span(a, b, len(note), max(CITE_FLOOR, want)))
            merged = _merge_spans(padded)
        slices = []
        for a, b in merged:
            if b - a < 100:
                a, b = _grow_span(a, b, len(note), 100)
            if b > a:
                slices.append(CitationSlice(start=a, end=b))
        if not slices:
            return None
        return CitationRef(receipt_id=receipt, result_id=rid, slices=slices)


    def _citations(answer: str, desk: Desk) -> tuple[list[CitationRef], dict[int, int]]:
        refs: list[CitationRef] = []
        pos: dict[int, int] = {}
        numbers = _cited_numbers(answer, len(desk.rows))
        if not numbers:
            numbers = list(range(1, min(4, len(desk.rows)) + 1))
        for n in numbers:
            if len(refs) >= MAX_CITES:
                break
            ref = _ref_for(desk.rows[n - 1])
            if ref is None:
                continue
            refs.append(ref)
            pos[n] = len(refs)
    ...
    def _repoint(answer: str, pos: dict[int, int]) -> str:
        def repl(match: re.Match[str]) -> str:
            blob = match.group(1) or match.group(2) or ""
            if blob.isdigit() and int(blob) in pos:
                return f"[[{pos[int(blob)]}]]"
            return match.group(0)

        return _MARK_RE.sub(repl, answer or "")


    def _wrapup(seconds: float) -> str:
        return (
            f"TIME LEFT ~{int(seconds)}s. Stop tools. Write the complete cited answer now. "
            "First words are the asked entities. Cite [n] on every claim. A short cited "
            "answer beats a refusal."
        )


    async def _preseed(question: str, desk: Desk, deadline: float) -> str:
        queries = _hop_queries(question)
        if not queries or _left(deadline) < 40.0:
            return ""
        jobs = [_search(q, desk) for q in queries[:MAX_SEED]]
        blocks = await asyncio.gather(*jobs, return_exceptions=True)
        useful = [b for b in blocks if isinstance(b, str) and "[" in b]
        if not useful:
            return ""
        urls = _URL_RE.findall(question or "")
        titles = _quoted_titles(question)
        if titles and _left(deadline) > 70.0:
            host = _named_hosts(question)
            focus = titles[0]
            for row in desk.rows:
                url = row.get("url") or ""
                blob = f"{url} {row.get('title') or ''}".lower()
                title_hit = titles[0].lower()[:18] in blob
                host_hit = bool(host and host[0] in blob)
                if title_hit or host_hit:
                    if url.startswith("http") and _left(deadline) > 55.0:
                        fetched = await _fetch(url, focus, question, desk)
                        useful.append(fetched)
                        if len(row.get("note") or "") > 800:
                            useful.append(_page_grep(url, r"|".join(re.escape(t) for t in _tokens(focus)[:4]), desk))
                        break
        for url in urls[:2]:
            if _left(deadline) < 50.0:
                break
            useful.append(await _fetch(url, question[:80], question, desk))
        return "First-pass hops already numbered:\n" + "\n".join(useful[:8])


    async def _hop_loop(question: str, desk: Desk, deadline: float) -> str:
        messages: list = [{"role": "system", "content": HOP_RULES}]
        if _SET_RE.search(question or ""):
            messages.append({"role": "system", "content": SET_RULE})
        if _PEAK_RE.search(question or ""):
            messages.append({"role": "system", "content": PEAK_RULE})
        if _ROW_KEY_RE.search(question or ""):
            messages.append({"role": "system", "content": KEY_RULE})
        if _DATAFILE_RE.search(question or ""):
            messages.append({"role": "system", "content": FILE_RULE})
        seeded = await _preseed(question, desk, deadline)
        if seeded:
            messages.append({"role": "system", "content": seeded})
        messages.append({"role": "user", "content": question})
        answer = ""
        ...
        for turn in range(1, MAX_TURNS + 1):
            if _left(deadline) <= MIN_TAIL_S:
                break
            finish = _left(deadline) <= WRAPUP_S or _spend_left() <= WRAPUP_USD or turn >= MAX_TURNS
            if finish and not wrapped:
                messages.append({"role": "system", "content": _wrapup(_left(deadline))})
                wrapped = True
            payload = await _chat_turn(messages, deadline, finish)
            if payload is None:
                break
            llm = getattr(payload, "llm", None)
            choices = getattr(llm, "choices", None) or []
            ...
            if not calls:
                ...
                if not candidate:
                    ...
                    if isinstance(content, str):
                        candidate = content.strip()
                if _usable(candidate, question):
                    answer = candidate
                break
            try:
                messages.append(msg.to_input_message())
            except Exception:
                messages.append({"role": "assistant", "content": "continuing hops"})
            run = list(calls)[:MAX_HOPS]
            results = await asyncio.gather(
                *[_run_tool(call, question, desk, deadline) for call in run],
                return_exceptions=True,
            )
            for call, result in zip(run, results):
                body = result if isinstance(result, str) else f"# tool error {result}"
                call_id = getattr(call, "id", "") or "tool"
                messages.append({"role": "tool", "tool_call_id": call_id, "content": body[:18000]})
        return answer


    def _harvest(desk: Desk, question: str) -> str:
        bits: list[str] = []
        terms = _tokens(question)[:10]
        for index, row in enumerate(desk.rows, start=1):
            note = row.get("note") or ""
            if not note:
                continue
            windows = _windows(note, terms, 700, 1)
            a, b = windows[0]
            a, b = _grow_span(a, b, len(note), CITE_FLOOR)
            bits.append(f"[{index}] {(note[a:b]).strip()}")
            if len(bits) >= 6:
                break
        if not bits:
            return ""
        return "From fetched sources:\n" + "\n".join(bits)


    async def _to_schema(question: str, draft: str, schema: object, desk: Desk, deadline: float) -> object:
        rendered = ""
        try:
            rendered = json.dumps(schema, ensure_ascii=False)[:8000]
        except Exception:
            rendered = ""
        evidence = _harvest(desk, question)[:9000]
        raw = await _chat_plain(
            LANE_A,
            SCHEMA_MODEL,
            "Return JSON that matches the schema. Copy names, dates, and figures exactly. "
            "Never fill a string field with a whole page or the question. No markdown.",
            f"Question:\n{question}\n\nSchema:\n{rendered}\n\nDraft:\n{draft[:7000]}\n\nEvidence:\n{evidence}",
            deadline,
            28.0,
            1800,
        ...
        if parsed is None or _schema_errors(parsed, schema):
            parsed = _clamp_schema(parsed if parsed is not None else draft, schema, evidence + " " + draft)
        if _schema_errors(parsed, schema) and _left(deadline) > 20.0:
            repair = await _chat_plain(
                LANE_A,
                REPAIR_MODEL,
                "Repair JSON schema violations. Return only the JSON value.",
                f"Question:\n{question}\nSchema:\n{rendered}\nViolations:\n{_schema_errors(parsed, schema)}\n"
                f"Draft:\n{json.dumps(parsed)[:4000]}\nEvidence:\n{evidence[:5000]}",
                deadline,
                22.0,
                1400,
            )
            fixed = _json_blob(repair)
            if fixed is not None and not _schema_errors(fixed, schema):
                parsed = fixed
            else:
                parsed = _clamp_schema(parsed, schema, evidence)
        return _clamp_schema(parsed, schema, evidence)


    async def _audit(question: str, answer: str, desk: Desk, deadline: float) -> str:
        if not _usable(answer, question) or _left(deadline) < 70.0:
            return answer
        table = _harvest(desk, question)[:7000]
        raw = await _chat_plain(
            LANE_A,
            AUDIT_MODEL,
            "JSON only with keys unanswered_parts, incomplete_roster, missing_named_docs, all lists.",
            f"Question:\n{question}\n\nAnswer:\n{answer[:9000]}\n\nEvidence:\n{table}",
            deadline,
            AUDIT_S,
            900,
        )
        report = _json_blob(raw)
        if not isinstance(report, dict):
            return answer
        gaps: list[str] = []
        for key in ("unanswered_parts", "incomplete_roster", "missing_named_docs"):
            vals = report.get(key)
            if isinstance(vals, list):
                gaps.extend(str(v) for v in vals if str(v).strip())
        if not gaps:
            return answer
        extra = await _search(gaps[0][:180], desk)
        if "[" in extra and _left(deadline) > 50.0:
            rewritten = await _chat_plain(
                LANE_A,
                MODEL_A,
                "Rewrite the answer using the new evidence. Cite [n]. Lead with asked entities.",
                f"Question:\n{question}\nDraft:\n{answer[:7000]}\nGaps:\n{gaps}\nNew:\n{extra[:4000]}",
                deadline,
                24.0,
                1600,
            )
            if _usable(rewritten, question):
                return rewritten
        return answer


    def _ship(text: str, desk: Desk, question: str) -> Response:
        body = (text or "").strip()
        if not _usable(body, question):
            harvested = _harvest(desk, question)
            if harvested:
                body = harvested
        if not body:
            body = "Retrieved sources were incomplete, so the best supported facts are listed with citations."
        body = body[:ANSWER_CAP]
        refs, pos = _citations(body, desk)
        pointed = _repoint(body, pos) if refs else body
        if refs:
            try:
                return Response(text=pointed, citations=refs)
            except Exception:
                return Response(text=pointed)
        return Response(text=pointed)
    ...
    async def _solve(query: Query) -> Response:
        ...
        desk = Desk()
        try:
            draft = await _hop_loop(question, desk, deadline if schema is None else deadline - 42.0)
        except Exception:
            draft = ""
        if not _usable(draft, question):
            harvested = _harvest(desk, question)
            if harvested:
                draft = harvested
        if _usable(draft, question):
            try:
                draft = await _audit(question, draft, desk, deadline if schema is None else deadline - 36.0)
            except Exception:
                pass
        if schema is not None:
            parsed = await _to_schema(question, draft, schema, desk, deadline)
            refs, _pos = _citations(draft, desk)
            note = draft[:NOTE_CAP] if _usable(draft, question) else None
            try:
                if refs and note:
                    return Response(output=parsed, note=note, citations=refs)
                if refs:
                    return Response(output=parsed, citations=refs)
                if note:
                    return Response(output=parsed, note=note)
                return Response(output=parsed)
            except Exception:
                return Response(output=_clamp_schema(question[:200], schema, draft))
        return _ship(draft, desk, question)
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            if schema is not None:
                try:
                    return Response(output=_clamp_schema(question[:400], schema, question))
                except Exception:
                    return Response(output={"answer": question[:200]})
            return Response(text=f"Cited research incomplete for: {question[:400]}" if question else "No question provided.")

    return query

_branch3_e037444_f_a_a_e_query_entry = _compose_branch3_e037444_f_a_a_e_entry()


def _compose_branch_d_e36120_e_a780_entry():


    """Combined deep-research miner agent."""
    ...
    _BUDGET_WRAPUP_FRACTION = 0.70
    _BUDGET_HARD_FRACTION = 0.92
    _BUDGET_BACKUP_MIN_FRACTION = 0.50
    _BUDGET_STATE = {"limit": 0.0, "spent": 0.0, "known": False}

    _COST_IN_PER_TOKEN = {
        "z-ai/glm-5.2": 0.0000008008,
        "z-ai/glm-5": 0.0000008008,
        "z-ai/glm-5.3-flash": 0.0000002000,
        "openai/gpt-oss-120b": 0.0000000370,
        "openai/gpt-oss": 0.0000000370,
        "deepseek/deepseek-v3.2": 0.0000002800,
        "google/gemma-4-31b-it": 0.0000000500,
        "qwen/qwen3.6-27b": 0.0000000600,
    }
    _COST_OUT_PER_TOKEN = {
        "z-ai/glm-5.2": 0.0000025168,
        "z-ai/glm-5": 0.0000025168,
        "z-ai/glm-5.3-flash": 0.0000008000,
        "openai/gpt-oss-120b": 0.0000001700,
        "openai/gpt-oss": 0.0000001700,
        "deepseek/deepseek-v3.2": 0.0000012000,
        "google/gemma-4-31b-it": 0.0000002000,
        "qwen/qwen3.6-27b": 0.0000002500,
    }


    def _budget_reset():
        _BUDGET_STATE["limit"] = 0.0
        _BUDGET_STATE["spent"] = 0.0
        _BUDGET_STATE["known"] = False


    def _budget_candidates(info):
        out = []
        if isinstance(info, dict):
            out.append(info.get("session_budget"))
            out.append(info.get("budget"))
            out.append(info.get("remaining_budget"))
            out.append(info.get("budget_usd"))
            return out
        out.append(getattr(info, "session_budget", None))
        out.append(getattr(info, "budget", None))
        out.append(getattr(info, "remaining_budget", None))
        out.append(getattr(info, "budget_usd", None))
        return out


    async def _budget_prime():
        try:
            info = await tooling_info()
        except Exception:
            return
        limit = 0.0
        for value in _budget_candidates(info):
            if isinstance(value, (int, float)) and value > 0:
                limit = float(value)
                break
        if limit > 0.0:
            _BUDGET_STATE["limit"] = limit
            _BUDGET_STATE["known"] = True


    def _budget_estimate(model, in_chars, out_tokens):
        rate_in = _COST_IN_PER_TOKEN.get(model, 0.0000008008)
        rate_out = _COST_OUT_PER_TOKEN.get(model, 0.0000025168)
        return (in_chars / 4.0) * rate_in + float(out_tokens) * rate_out


    def _budget_fraction():
        if not _BUDGET_STATE["known"] or _BUDGET_STATE["limit"] <= 0.0:
            return 0.0
        return _BUDGET_STATE["spent"] / _BUDGET_STATE["limit"]


    def _budget_should_wrap_up():
        return _budget_fraction() >= _BUDGET_WRAPUP_FRACTION


    def _budget_can_spend(estimate):
        if not _BUDGET_STATE["known"] or _BUDGET_STATE["limit"] <= 0.0:
            return True
        return (_BUDGET_STATE["spent"] + estimate) <= (_BUDGET_STATE["limit"] * _BUDGET_HARD_FRACTION)


    def _budget_note(estimate):
        _BUDGET_STATE["spent"] = _BUDGET_STATE["spent"] + estimate


    def _budget_backup_allowed():
        if not _BUDGET_STATE["known"] or _BUDGET_STATE["limit"] <= 0.0:
            return True
        return _budget_fraction() <= (1.0 - _BUDGET_BACKUP_MIN_FRACTION)

    def _compose_slate_db49af_alpha():
    ...
    def _compose_slate_db49af_beta():
...
""".strip()
    ...
    omitted key falls back to the SDK default)."""
        ...
        async def _repair_llm_chat(
            *,
            ...
            if state is None:
                return await _repair_chat_call(kwargs)
            if kwargs.get("provider") == "openrouter" and kwargs.get("model") == "z-ai/glm-5.2":
                kwargs["model"] = "z-ai/glm-5.2"
            messages = [dict(m) for m in kwargs.get("messages") or []]
    ...
    _LANE_A_DB49AF = _compose_slate_db49af_alpha()
    _LANE_B_DB49AF = _compose_slate_db49af_beta()

    def _shape_schema_fields(query):
        schema = getattr(query, "output_schema", None)
        if not isinstance(schema, dict):
            return 0
        properties = schema.get("properties")
    ...
    _ANALYTICAL_TERMS = (
        "compare", "comparison", "versus", " vs ", "difference", "ratio", "percent",
        "percentage", "how many", "how much", "count", "total", "sum", "average",
        "median", "rank", "ranking", "trend", "growth", "increase", "decrease",
        "more than", "less than", "higher than", "lower than", "year over year",
        "reconcile", "discrepancy", "contradiction",
    )

    _SET_TERMS = (
        "list all", "all of the", "every ", "each of", "roster", "names of",
        "which ones", "enumerate", "complete list",
    )


    def _question_class(query):
        text = getattr(query, "text", "")
        if not isinstance(text, str):
            text = ""
        lowered = " " + text.strip().lower() + " "
        if _shape_schema_fields(query) >= 1:
            return "schema"
        for term in _ANALYTICAL_TERMS:
            if term in lowered:
                return "analytical"
        for term in _SET_TERMS:
            if term in lowered:
                return "set"
        return "default"
    ...
    def _repair_output(query, value):
        skeleton = _schema_skeleton(query)
        if skeleton is None:
            return None
    ...
    def _contract_response(query, response):
        """Emit only text/output/note/citations, so no forbidden field reaches the validator."""
        wants_schema = getattr(query, "output_schema", None) is not None
        citations = _safe_citations(response)
        if wants_schema:
            value = getattr(response, "output", None)
            repaired = _repair_output(query, value)
            if citations:
                return Response(output=repaired, citations=citations)
            return Response(output=repaired)
        text = _answer_text(response)
        if not text:
            text = "No answer could be produced for this request."
        if citations:
            return Response(text=text, citations=citations)
        return Response(text=text)


    def _floor_response(query):
        if getattr(query, "output_schema", None) is not None:
            return Response(output=_schema_skeleton(query))
        return Response(text="No answer could be produced for this request.")
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            pass
        selected = _question_class(query)
        if selected == 'schema':
            try:
                return _contract_response(query, await _LANE_A_DB49AF(query))
            except Exception:
                pass
        elif selected == 'analytical':
            try:
                return _contract_response(query, await _LANE_B_DB49AF(query))
            except Exception:
                pass
        elif selected == 'set':
            try:
        ...
        try:
            ...
            pass
        return _floor_response(query)

    return query

_branch_d_e36120_e_a780_query_entry = _compose_branch_d_e36120_e_a780_entry()
...
class Branch3E037444FAAE:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch3_e037444_f_a_a_e_query_entry(query=query)


class BranchDE36120EA780:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_d_e36120_e_a780_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = Branch357895349B2A()
_MODE_SHAPE_SECONDARY_AGENT = Branch3E037444FAAE()
_MODE_SHAPE_TERTIARY_AGENT = BranchDE36120EA780()
_CANDIDATE_BRANCH_CLASS_NAMES = ("Branch357895349B2A", "Branch3E037444FAAE", "BranchDE36120EA780")
