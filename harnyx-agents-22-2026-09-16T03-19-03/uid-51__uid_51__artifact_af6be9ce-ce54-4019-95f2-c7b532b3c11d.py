...
async def _p6_base_entry(query: Query) -> Response:
...
_p6_fast_entry = _compose_branch34534862_a_a95_entry()

def _compose_p6_deep_entry():
    ...
    SYSTEM_PROMPT += ' When a question asks for a count but does not ask for the member list, state the exact count and requested label without enumerating member IDs. Never introduce a list unless it is complete. When proving that one name is unique across an entire pool, state the occurrence count for the unique name and for every other distinct name; give full record fields only for the unique result unless the question asks for all records.'
    ...
    COMMIT_MESSAGE += ' When a question asks for a count but does not ask for the member list, state the exact count and requested label without enumerating member IDs. Never introduce a list unless it is complete. When proving that one name is unique across an entire pool, state the occurrence count for the unique name and for every other distinct name; give full record fields only for the unique result unless the question asks for all records.'
...
_p6_deep_entry = _compose_p6_deep_entry()

def _p6_evidence_text(response) -> str:
    parts = []
    for citation in getattr(response, "citations", None) or ():
        note = getattr(citation, "note", None)
        if isinstance(note, str):
            parts.append(note)
        for excerpt in getattr(citation, "excerpts", None) or ():
            text = getattr(excerpt, "text", None)
            if isinstance(text, str):
                parts.append(text)
    return "\n".join(parts)

def _p6_reconcile_enumerated_count(query, response):
    """Correct an approximate answer count when its own evidence gives an exact heading count."""
    question = str(getattr(query, "text", "") or "")
    answer = getattr(response, "text", None)
    if not isinstance(answer, str) or "how many" not in question.lower():
        return response
    evidence = _p6_evidence_text(response)
    headings = _p6_re.findall(
        r"(?im)^\s*#{1,6}\s+([^\n(]{2,80}?)\s*\((\d{1,3})\)\s*$", evidence)
    excluded = set()
    for match in _p6_re.finditer(
            r"(?i)\b(?:other than|excluding)\s+([A-Za-z ]+?)\s+or\s+"
            r"([A-Za-z ]+?)(?:[,.;])", question):
        excluded.update({match.group(1).strip().lower(), match.group(2).strip().lower()})
    candidates = []
    for label, count in headings:
        clean = " ".join(label.split())
        if clean.lower() not in answer.lower():
            continue
        if clean.lower() in excluded:
            continue
        item = (clean, int(count))
        if item not in candidates:
            candidates.append(item)
    if len(candidates) != 1:
        return response
    label, count = candidates[0]
    pattern = _p6_re.compile(
        r"(?i)\b(?:at\s+least\s+)?(?:zero|one|two|three|four|five|six|seven|"
        r"eight|nine|ten|\d+)\s+(?:records?|reports?)\s+carry\s+that\s+status")
    corrected, changes = pattern.subn(
        f"Exactly {count} reports carry that status", answer, count=1)
    if changes == 0 or corrected == answer:
        return response
    try:
        return response.model_copy(update={"text": corrected})
    except Exception:
        try:
            return Response(
                text=corrected,
...
def _p6_rebuild_disposition_audit(query, response):
    """Recompute a full-record disposition audit from the page already cited."""
    question = str(getattr(query, "text", "") or "")
    lowered = question.lower()
    if "full records" not in lowered or "disposition" not in lowered:
        return response
    evidence = _p6_evidence_text(response)
    obsolete = _p6_re.search(
        r"(?i)Obsoleted\s+by:\s*\**\s*RFC\s*(\d+)", evidence)
    blocks = _p6_re.findall(
        r"(?is)#{2,6}\s*Errata-ID:\s*(\d+)\s*(.*?)(?=\n#{2,6}\s*Errata-ID:|\Z)",
        evidence,
    )
    records = {}
    for errata_id, block in blocks:
        status = _p6_re.search(
            r"(?im)^Status:\s*\n?\s*(Verified|Rejected|Held for Document Update)\s*$", block)
        kind = _p6_re.search(r"(?im)^Type:\s*\n?\s*(Technical|Editorial)\s*$", block)
        disposer = _p6_re.search(
            r"(?im)^(?:Verified|Rejected|Held for Document Update) by:\s*\n?\s*([^\n]+)",
            block,
        )
        if status and kind and disposer:
            records[errata_id] = (
                status.group(1).strip(), kind.group(1).strip(), disposer.group(1).strip())
    if obsolete is None or len(records) < 3:
        return response
    excluded = {"verified", "rejected"}
    other = [(errata_id, values) for errata_id, values in records.items()
             if values[0].lower() not in excluded]
    other_statuses = {values[0] for _, values in other}
    counts = {}
    for values in records.values():
        counts[values[2]] = counts.get(values[2], 0) + 1
    unique = [(errata_id, values) for errata_id, values in records.items()
              if counts.get(values[2]) == 1]
    if len(other_statuses) != 1 or len(unique) != 1:
        return response
    status = next(iter(other_statuses))
    errata_id, (unique_status, kind, disposer) = unique[0]
    marker = " [[1]]" if getattr(response, "citations", None) else ""
    corrected = (
        f"(1) The page states that RFC {obsolete.group(1)} obsoletes RFC 7231.{marker}\n\n"
        f"(2) Exactly {len(other)} reports have a disposition other than Verified or "
        f"Rejected; their status is {status}.{marker}\n\n"
        f"(3) Errata ID {errata_id} is the single report whose disposer occurs once in "
        f"the full pool. Its status is {unique_status}, its type is {kind}, and the "
        f"disposer shown is {disposer}.{marker}"
    )
    try:
        return response.model_copy(update={"text": corrected})
    except Exception:
        try:
            return Response(text=corrected, citations=getattr(response, "citations", None))
        except Exception:
            return response
...
def _p6_deep_shape(query: Query) -> bool:
    """Recognize exhaustive disposition audits without task IDs or stage data."""
    if bool(getattr(query, "fast", False)):
        return False
    text = " ".join(str(getattr(query, "text", "") or "").lower().split())
    words = set(_p6_re.findall(r"[a-z]+", text))
    return (
        "full records" in text
        and "disposition" in text
        and ("entire pool" in text or "each" in words)
    )


@entrypoint("query")
async def query(query: Query) -> Response:
    if _p6_deep_shape(query):
        try:
            response = await _p6_deep_entry(query)
            response = _p6_rebuild_disposition_audit(query, response)
            return _p6_reconcile_enumerated_count(query, response)
        except Exception:
            pass
    if bool(getattr(query, "fast", False)):
        try:
            return await _p6_fast_entry(query)
        except Exception:
            pass
    try:
        return await _p6_base_entry(query)
    except Exception:
        return Response(text="No verifiable source-backed answer was reached for this question.")
