...
def _compose_branch_e859318_b_e_e72_entry():
...
_branch_e859318_b_e_e72_query_entry = _compose_branch_e859318_b_e_e72_entry()
...
def _compose_branch05_a8_b_e_b2_a188_entry():
    ...
    def _rt_prose_entry():
        ...
        def pir__spend_note(payload) -> None:
            budget = getattr(payload, "budget", None)
            left = getattr(budget, "session_remaining_budget_usd", None)
            if isinstance(left, (int, float)):
                _SPEND["left"] = float(left)
        ...
        def pir__spend_blind() -> None:
            _SPEND["blind"] = _SPEND["blind"] + 1


        def pir__spend_left() -> float:
            left = _SPEND["left"]
            if isinstance(left, (int, float)):
        ...
        def pir__wrapup_order(seconds_left: float) -> str:
            return (
                f"TIME IS UP (~{int(seconds_left)}s left). No more tool calls. Write the "
            "complete final answer NOW from the numbered results above plus your "
            "knowledge: the FIRST words are the answer entities (no 'Based on…' "
        ...
        def pir__has_superlative(text: str) -> bool:
            if _ONE_WINNER_RE.search(text or ""):
                return True
            for m in _EST_RE.finditer(text or ""):
                if m.group(0).lower() not in _EST_STOP:
        ...
        def pir__needs_superlative_proof(question: str) -> bool:
            q = " ".join((question or "").split())
            if not q:
                return False
            return pir__has_superlative(q) or bool(
                re.search(r"\b(?:most|least) (?:common|frequent|number|amount)\b|\bhow many\b", q, re.I))
        ...
        def pir__needs_set_completeness(question: str) -> bool:
            q = " ".join((question or "").split())
            if _SET_HINT_RE.search(q):
                return True
            ...
            m = _PLURAL_HEAD_RE.search(q)
            if m and m.group(1).lower() not in _PLURAL_FALSE:
                if not pir__has_superlative(q) or re.search(r"\b(?:all|every|each)\b", q, re.IGNORECASE):
                    return True
        ...
        def pir__key_terms(text: str) -> set[str]:
            return {w for w in _WORD_RE.findall((text or "").casefold()) if w not in _STOP}


        def pir__best_windows(note: str, terms: set[str], width: int,
                          k: int = 1) -> list[tuple[int, int]]:
            n = len(note)
            if n <= width:
                return [(0, n)]
        ...
        def pir__norm_fetch_key(url: str) -> str:
        ...
        def pir__memo_key(kind: str, *parts: str) -> str:
            joined = "\x00".join(" ".join((part or "").lower().split()) for part in parts)
            return kind + "\x00" + joined


        def pir__memo_hit(key: str) -> str:
            return _TOOL_MEMO.get(key, "")


        def pir__commit_tool_output(out, ledger: pir_EvidenceLedger) -> str:
            if isinstance(out, str):
                return out
            if not isinstance(out, pir_ToolOutput):
                return f"# tool crashed: {out}"
            text = out.text
            assigned: list = []
            for i, row in enumerate(out.rows):
        ...
        def pir__condense_excerpt(text: str) -> str:
            if len(text) <= int(SEARCH_AGED_LEAD_CHARS * 1.3):
                return text
            cut = SEARCH_AGED_LEAD_CHARS
        ...
        def pir__condense_block(body: str) -> str:
            lines = body.split("\n")
            if len(lines) < 8:
                ...
                for line in lines:
                    stripped = line.strip()
                    if len(stripped) > SEARCH_AGED_LEAD_CHARS * 2 and not stripped.startswith("#"):
                        shorter = pir__condense_excerpt(line)
                        changed = changed or shorter != line
                        rebuilt.append(shorter)
                    else:
                        rebuilt.append(line)
            ...
            for index, line in enumerate(lines):
                ...
                if keep:
                    ...
                    if was_lead and len(stripped) > SEARCH_AGED_LEAD_CHARS * 2:
                        kept.append(pir__condense_excerpt(line))
                    else:
                        kept.append(line)
            out = "\n".join(kept)
            if len(out) > len(body) * (1.0 - HISTORY_MIN_SAVING):
        ...
        def pir__condense_history(messages: list) -> None:
            tool_positions = [i for i, m in enumerate(messages)
                              if isinstance(m, dict) and m.get("role") == "tool"]
            seed_positions = [i for i, m in enumerate(messages)
                              if isinstance(m, dict) and m.get("role") == "system"
            ...
            if len(tool_positions) > SEED_KEEP_TOOL_TURNS:
                for i in seed_positions:
                    body = messages[i].get("content")
                    if isinstance(body, str) and not body.endswith(_KEPT_TRAILERS):
                        messages[i]["content"] = pir__archive_seed(body)
            if len(tool_positions) <= HISTORY_KEEP_VERBATIM:
                return
            total = 0
            for i in tool_positions:
            ...
            for i in tool_positions[:-HISTORY_KEEP_VERBATIM] + seed_positions:
                ...
                if not isinstance(body, str) or body.endswith(_KEPT_TRAILERS):
                    continue
                message["content"] = pir__condense_block(body)
        ...
        def pir__archive_seed(body: str) -> str:
            rows = _SEED_ROW_RE.findall(body)
            if not rows:
                return body                                                        
            out = body.split("\n", 1)[0] + "\n" + "\n".join(rows) + _ARCHIVED_TRAILER
        ...
        def pir__degrade_query(q: str) -> str:
            out = _SITE_OP_RE.sub("", q or "").replace('"', " ")
            return " ".join(out.split())


        async def pir__do_search(query_text: str, ledger: pir_EvidenceLedger):
            if not query_text.strip():
                return "# web_search: empty query"
            memo_key = pir__memo_key("search", query_text)
            hit = pir__memo_hit(memo_key)
            if hit:
                return f"# web_search({query_text!r}) {hit}"
            ...
            for attempt, allow_repeat in ((query_text, False), (query_text, True),
                                          (pir__degrade_query(query_text), False)):
                if not attempt.strip() or (attempt in fired and not allow_repeat):
                    continue
                fired.add(attempt)
                for _prov in SEARCH_PROVIDERS:
            ...
            for item in results:
                ...
                lines.append(f"[{_SLOT.format(len(rows) - 1)}] {title} — {url}"
                         f"\n    {note[:SEARCH_EXCERPT_CHARS]}")
            return pir_ToolOutput("\n".join(lines), rows, memo_key=memo_key if rows else "")


        async def pir__do_fetch(url: str, focus: str, question: str, ledger: pir_EvidenceLedger) -> str:
            if not url.strip():
                return "# read_page: empty url"


            plain_key = pir__memo_key("fetch", url)
            focus_key = pir__memo_key("fetch", url, focus)
            hit = pir__memo_hit(plain_key) or pir__memo_hit(focus_key)
            if hit:
                return f"# read_page({url!r}) {hit}"


            _dead_key = pir__norm_fetch_key(url)
            if url in _FETCH_STATE["dead"] or (
                    _dead_key and _dead_key in _FETCH_STATE["dead_norm"]):
                return (f"# read_page({url!r}): this url already returned no content in "
                    f"this run and will not be retried. Use a different source, or "
            ...
            if len(note) <= FETCH_PLAIN_CHARS:
                row = {"receipt_id": receipt, "result_id": rid, "note_len": len(note),
                       "kind": "fetch", "spans": [(0, len(note))], "title": url,
                       "url": url, "preview": note[:1200], "text": note}
                return pir_ToolOutput(f"# read_page({url!r}) -> [{_SLOT.format(0)}] full page, "
                              f"{len(note)} chars\n{pir__lossless_view(note)}", [row],
                                  memo_key=plain_key)

            terms = pir__key_terms(question) | pir__key_terms(focus)
            windows = pir__best_windows(note, terms, FETCH_WINDOW_CHARS, k=FETCH_WINDOWS_PER_PAGE)
            row = {"receipt_id": receipt, "result_id": rid, "note_len": len(note),
                   "kind": "fetch", "spans": [(0, FETCH_HEAD_CHARS)] + list(windows),
                   "title": url, "url": url,
                   "preview": note[windows[0][0]:windows[0][0] + 1200], "text": note}
            head = pir__lossless_view(note[:FETCH_HEAD_CHARS])
            sections = "".join(
                f"\n--- section @{s} ---\n{pir__lossless_view(note[s:e])}" for s, e in windows)
            return pir_ToolOutput(f"# read_page({url!r}) -> [{_SLOT.format(0)}] {len(note)} chars total; head + "
                f"the {len(windows)} most relevant section(s) shown "
                f"({', '.join(f'{s}-{e}' for s, e in windows)}). If the answer set may "
                f"continue elsewhere in this page, call read_page again with a "
                f"different focus.\n--- head ---\n{head}{sections}", [row],
        ...
        def pir__sec_tokens(text: str) -> list[str]:
            return [w for w in _SEC_ALNUM_RE.findall((text or "").lower())
                    if w not in _SEC_STOPWORDS]


        def pir__sec_norm_form(form: str) -> str:
            f = " ".join((form or "").upper().replace("FORM", " ").split())
            m = re.fullmatch(r"(\d{1,2})\s*-?\s*([A-Z])", f)
            if m:
                return f"{m.group(1)}-{m.group(2)}"
        ...
        async def pir__fetch_json(url: str, deadline: float):
            cached = _SEC_CACHE.get(url)
            if cached is not None:
                return cached
            for _attempt in (0, 1):                                                  
        ...
        def pir__sec_pick_filing(recent: dict, form: str, year: str):
            forms = recent.get("form"); accs = recent.get("accessionNumber")
            docs = recent.get("primaryDocument"); rdates = recent.get("reportDate")
            fdates = recent.get("filingDate")
            if not (isinstance(forms, list) and isinstance(accs, list) and isinstance(docs, list)):
                return None
            n = min(len(forms), len(accs), len(docs))
            form_norm = pir__sec_norm_form(form)
            best_year = None
            best_any = None
            for i in range(n):
                if pir__sec_norm_form(str(forms[i])) != form_norm:
                    continue
                if accs[i] is None or docs[i] is None:
                    continue
                acc = str(accs[i]); doc = str(docs[i])
        ...
        async def pir__do_sec_filing(company: str, form: str, year: str, deadline: float) -> str:
            company = (company or "").strip()
            form = (form or "").strip() or "10-K"
            year = (year or "").strip()[:4]
            hint = _SEC_SEARCH_HINT.format(company=company, year=year, form=form)
            ...
            if (deadline - monotonic()) < _SEC_MIN_HEADROOM_S:
                return f"# sec_filing: skipped (low time) — {hint}"
            tickers = await pir__fetch_json(_SEC_TICKERS_URL, deadline)
            if not isinstance(tickers, dict):
                return f"# sec_filing: EDGAR ticker index unavailable — {hint}"
            want = pir__sec_tokens(company)
            best = None                                      
            for row in tickers.values():
                if not isinstance(row, dict):
                    continue
                title = str(row.get("title", ""))
                ticker = str(row.get("ticker", "")).lower()
                words = set(pir__sec_tokens(title))
                n_hit = sum(1 for w in want if w in words)
                if len(want) == 1 and ticker == want[0]:
                    score = 100                                                        
            ...
            if best is None:
                return f"# sec_filing({company!r}): no confident EDGAR match — {hint}"
            cik10, title = best[2], best[3]
            subs = await pir__fetch_json(_SEC_SUBMISSIONS_URL.format(cik10=cik10), deadline)
            filings = subs.get("filings") if isinstance(subs, dict) else None
            recent = filings.get("recent") if isinstance(filings, dict) else None
            if not isinstance(recent, dict):
                return f"# sec_filing({company!r}): EDGAR submissions unavailable for {title} — {hint}"
            pick = pir__sec_pick_filing(recent, form, year)
            if pick is None:
                return (f"# sec_filing({company!r}, {form!r}, year={year or 'latest'}): no matching "
                    f"filing in EDGAR's recent index for {title} — check the form/year, or {hint}")
            accession, doc = pick
        ...
        def pir__ledger_page(url: str, ledger: pir_EvidenceLedger) -> tuple[int, dict] | None:
            u = (url or "").strip().rstrip("/")
            if not u:
                return None
            for i in range(len(ledger.rows) - 1, -1, -1):
        ...
        def pir__add_shown_span(row: dict, a: int, b: int) -> None:
            text = row.get("text") or ""
            note_len = int(row.get("note_len") or len(text))
            a = max(0, min(int(a), note_len))
            b = max(a + 1, min(int(b), note_len))
        ...
        def pir__do_page_grep(url: str, pattern: str, ledger: pir_EvidenceLedger) -> str:
            hit = pir__ledger_page(url, ledger)
            if hit is None:
                return f"# page_grep: {url!r} has not been fetched this run; call read_page first"
            n, row = hit
            text = row.get("text") or ""
        ...
        def pir__do_page_read(url: str, offset: int, length: int, ledger: pir_EvidenceLedger) -> str:
            hit = pir__ledger_page(url, ledger)
            if hit is None:
                return f"# page_read: {url!r} has not been fetched this run; call read_page first"
            n, row = hit
        ...
        def pir__dup_title_ranges(text: str) -> list[tuple[int, int]]:
            cuts: list[tuple[int, int]] = []
            for m in _DUP_TITLE.finditer(text):
                if m.group(4).strip() == m.group(1).strip():
                    cuts.append((m.start(3), m.end(3)))
        ...
        def pir__lossless_view(text: str) -> str:
            cuts = pir__dup_title_ranges(text)
            if not cuts:
                return text
            out: list[str] = []
            at = 0
        ...
        def pir__canon_with_map(text: str) -> tuple[str, list[int]]:
            out: list[str] = []
            idx: list[int] = []
            prev_space = True
            skip = pir__dup_title_ranges(text)
            cut_i = 0
            for i, ch in enumerate(text):
                while cut_i < len(skip) and i >= skip[cut_i][1]:
                    cut_i += 1
        ...
        def pir__quote_hits(text: str, quote: str) -> list[tuple[int, int]]:
            def scan(hay: str, needle: str, span: int) -> list[tuple[int, int]]:
                found: list[tuple[int, int]] = []
                at = 0
                while len(found) < 64:
            ...
            if hits:
                return hits
            canon, cmap = pir__canon_with_map(text)
            cq, _ = pir__canon_with_map(quote)
            if not cq or not canon:
                return []
            for a, b in scan(canon, cq, len(cq)):
                last = b - 1
        ...
        def pir__pick_quote_hit(hits: list[tuple[int, int]],
                            spans: object) -> tuple[int, int] | None:
            if not hits:
                return None
            shown: list[tuple[int, int]] = []
        ...
        def pir__do_retain_evidence(source: str, quote: str, ledger: pir_EvidenceLedger) -> str:
            raw = (source or "").strip().strip("[]")
            try:
                n = int(raw)
            except ValueError:
            ...
            if not text:
                return f"# retain_evidence: result [{n}] has no stored text to quote from"
            hit = pir__pick_quote_hit(pir__quote_hits(text, q), row.get("spans"))
            if hit is None:
                return (f"# retain_evidence: that text does not appear in [{n}]. Quote it "
                    f"EXACTLY as the source prints it, or read more of the page first.")
            i, j = hit
        ...
        async def pir__run_tool(call, question: str, ledger: pir_EvidenceLedger, deadline: float) -> str:
            try:
                args = json.loads(getattr(call, "arguments", None) or "{}")
            except Exception:
                args = {}
            ...
            if name == "web_search":
                return await pir__do_search(str(args.get("query") or ""), ledger)
            if name == "read_page":
                return await pir__do_fetch(str(args.get("url") or ""), str(args.get("focus") or ""),
                                       question, ledger)
            if name == "retain_evidence":
                return pir__do_retain_evidence(str(args.get("source") or ""),
                                           str(args.get("quote") or ""), ledger)
            if name == "page_grep":
                return pir__do_page_grep(str(args.get("url") or ""),
                                     str(args.get("pattern") or ""), ledger)
            if name == "page_read":
                return pir__do_page_read(str(args.get("url") or ""),
                                     args.get("offset") or 0,
                                     args.get("length") or PAGE_READ_MAX_CHARS, ledger)
            if name == "sec_filing":
                return await pir__do_sec_filing(str(args.get("company") or ""),
                                            str(args.get("form") or ""),
                                            str(args.get("year") or ""), deadline)
            return f"# unknown tool {name!r}"
        ...
        def pir__least_think(lane: str, model: str = "") -> dict:
            for prefix in _REASONING_MANDATORY:
                if model.startswith(prefix):
                    return {"enabled": True, "effort": "low"}
            return {"enabled": False}
        ...
        def pir__upstream_key(model: str) -> str | None:
            if model.startswith("z-ai/glm-5.2"):
                return "glm"
            if model.startswith("openai/gpt-oss"):
                return "oss"
        ...
        def pir__upstream(lane: str, model: str) -> dict | None:
            if lane != LLM_LANE_A:
                return None
            key = pir__upstream_key(model)
            if key is None:
                return None
            pool = _FAST_UPSTREAMS if key == "glm" else _FAST_UPSTREAMS_OSS
            chosen = _RUN_UPSTREAM.get(key)
        ...
        def pir__upstream_failed(model: str) -> None:
            key = pir__upstream_key(model)
            if key is None:
                return
            chosen = _RUN_UPSTREAM.get(key)
        ...
        async def pir__chat_simple(lane: str, model: str, system: str, user: str, *,
                               max_tokens: int, timeout: float,
                               think: dict | None = None) -> str:
            if think is None:
                think = pir__least_think(lane, model)


            _pin0 = pir__upstream(lane, model)
            payload = None
            for _pin in ((_pin0, None) if _pin0 is not None else (None,)):
                try:
                    payload = await llm_chat(
        ...
        class pir__EmptyChoiceMessage:
            content = ""
            tool_calls = ()


        class pir__EmptyChoice:
            message = pir__EmptyChoiceMessage()


        class pir__EmptyLlm:
            raw_text = ""
            choices = (pir__EmptyChoice(),)


        class pir__EmptyTurn:
            llm = pir__EmptyLlm()
            budget = None


        _EMPTY_TURN = pir__EmptyTurn()


        async def pir__chat_turn(messages: list[dict], deadline: float, *, finish_only: bool,
                             force_tools: bool = False):
            ...
            for lane_model in ((LLM_LANE_A, LOOP_MODEL_A, True),
                ...
                try:
                    ...
                    payload = await asyncio.wait_for(llm_chat(
                        ...
                        max_output_tokens=6000 if (finish_only and model == LOOP_MODEL_B) else None,
                        provider_extra=pir__upstream(lane, model) if pinned else None,
                        timeout=timeout,
                    ), timeout=min(timeout + 6.0,
                                   max(1.0, deadline - monotonic() - 1.0)))
                    pir__spend_note(payload)
        ...
        def pir__brief_plan() -> str:
            return _BRIEF_STORE.get("plan") or ""


        def pir__condense_brief(messages: list) -> None:
            for message in messages:
                if not (isinstance(message, dict) and message.get("role") == "system"):
                    continue
                body = message.get("content")
        ...
        async def pir__knowledge_brief(question: str) -> tuple[str, str]:
            system = ("Senior research analyst. Commit to concrete best answers from "
                  "knowledge; mark uncertain values (verify). Never refuse.")
            ...
            raw = ""
            try:
                raw = await pir__chat_simple(LLM_LANE_A, LOOP_MODEL_A, system, user,
                                         max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
                                         think=pir__least_think(LLM_LANE_A, LOOP_MODEL_A))
            except Exception:
                try:
                    raw = await pir__chat_simple(LLM_LANE_B, LOOP_MODEL_B, system, user,
                                             max_tokens=2400, timeout=BRIEF_TIMEOUT_S,
                                             think=pir__least_think(LLM_LANE_B, LOOP_MODEL_B))
                except Exception:
                    raw = ""
            if not raw:
                return "", ""
        ...
        def pir__seed_queries(question: str, set_question: bool) -> list[str]:
            q = " ".join((question or "").split())
            if not q:
                return []
            seeds = [q[:300]]
            ...
            salient_src = q
            try:
                salient_src = pir__ask_clause(q) or q
            except Exception:
                salient_src = q
            salient = [t for t in _SEED_TOKEN_RE.findall(salient_src)
                       if len(t) >= 3 and t.lower() not in _STOP and t.lower() not in _SEED_STOP]
        ...
        async def pir__preseed(question: str, set_question: bool, ledger: pir_EvidenceLedger,
                           deadline: float) -> str:
            seeds = pir__seed_queries(question, set_question)
            if not seeds or (deadline - monotonic()) < 40.0:
                return ""
            ...
            budget = max(5.0, min(SEARCH_TIMEOUT_S * 2 + 6.0,
                                  deadline - monotonic() - MIN_TAIL_S))
            seed_tasks = [asyncio.ensure_future(pir__do_search(seed, ledger)) for seed in seeds]
            try:
                await asyncio.wait(seed_tasks, timeout=budget)
            except Exception:
                pass
            ...
            for seed_task in seed_tasks:
                ...
                try:
                    ...
                    continue
                blocks.append(pir__commit_tool_output(out, ledger))
            good = [b for b in blocks if isinstance(b, str) and _CITE_MARK_RE.search(b)]
            if not good:
                return ""                                                        
            return ("Automatic first-pass searches (already numbered — cite these [n] "
        ...
        async def pir__loop(question: str, brief: str, ledger: pir_EvidenceLedger,
                        deadline: float, turn_cap: int,
                        carry: list[dict] | None = None,
                        allow_tools_in_wrapup: bool = False,
            ...
            if carry is not None:
                ...
                set_q = pir__needs_set_completeness(question)
                messages = [{"role": "system", "content": LOOP_RULES}]
                if set_q:
                    messages.append({"role": "system", "content": SET_RULE})
                if pir__needs_superlative_proof(question):
                    messages.append({"role": "system", "content": SUPERLATIVE_RULE})
                if brief:
                    messages.append({"role": "system", "content": brief})

                seeded = await pir__preseed(question, set_q, ledger, deadline)
                if seeded:
                    messages.append({"role": "system", "content": seeded})
                messages.append({"role": "user", "content": question})
            ...
            for turn in range(1, turn_cap + 1):
                ...
                if left <= MIN_TAIL_S:
                    break
                out_of_time = left <= WRAPUP_AT_S
                out_of_spend = pir__spend_left() <= WRAPUP_MIN_USD
                finish_only = out_of_time or out_of_spend or turn >= turn_cap
                if (finish_only or turn >= turn_cap - 1) and not ordered_wrapup:
                    messages.append({"role": "system", "content": pir__wrapup_order(left)})
                    ordered_wrapup = True


                pir__condense_history(messages)
                if criteria is not None and turn * 2 >= turn_cap:
                    hint = ""
                    try:
                        hint = pir__open_criteria_hint(criteria, ledger)
                    except Exception:
                        hint = ""
                    criteria = None
                    if hint:
                        messages.append({"role": "system", "content": hint})
                payload = await pir__chat_turn(messages, deadline, finish_only=finish_only,
                                           force_tools=allow_tools_in_wrapup and turn == 1)
                if payload is None:
                    break
                llm = getattr(payload, "llm", None)
                ...
                if not calls:
                    ...
                    if not pir__is_usable_answer(candidate):
                        if repairs_left > 0 and (deadline - monotonic()) > MIN_TAIL_S + 10.0:
                            repairs_left -= 1
                ...
                tool_tasks = [asyncio.ensure_future(pir__run_tool(c, question, ledger, deadline))
                              for c in run_calls]
                try:
                    await asyncio.wait(tool_tasks, timeout=tool_budget)
                except Exception:
                ...
                for call_result in zip(run_calls, results):
                    ...
                    body = pir__commit_tool_output(call_result[1], ledger)
                    messages.append({"role": "tool", "tool_call_id": call.id, "content": body})
                for call in calls[8:]:
                    messages.append({"role": "tool", "tool_call_id": call.id,
                                     "content": "# skipped: per-turn tool budget reached — re-issue next turn if still needed"})
        ...
        async def pir__audit_patch(question: str, answer: str, messages: list[dict],
                               ledger: pir_EvidenceLedger, deadline: float) -> str:
            probe = (
                "Audit the answer against the question. JSON only, keys: "
            '"unanswered_parts" (list; question elements not addressed), '
            '"uncited_facts" (list; load-bearing claims without [n]), '
            ...
            try:
                raw = await pir__chat_simple(LLM_LANE_A, AUDIT_MODEL,
                                         "Strict completeness auditor. JSON only.",
                                         probe, max_tokens=2200,
                                         timeout=max(8.0, min(AUDIT_TIMEOUT_S,
                                                              (deadline - monotonic()) - 72.0)))
            ...
            order += ("\nUse at most 3 tool calls to close the most important gaps, then "
                  ...
                  "required shape.")
            messages.append({"role": "system", "content": order})
            patched, _ = await pir__loop(question, "", ledger, deadline,
                                     AUDIT_EXTRA_TURNS + 1, carry=messages,
                                     allow_tools_in_wrapup=True)
            patched = patched.strip()

            if not pir__is_usable_answer(patched) or len(patched) < int(len(answer) * 0.6):
                return answer
            return patched
        ...
        def pir__normalize_brackets(text: str) -> str:
            return (text or "").translate(_BRACKET_FIX)
        ...
        def pir__cited_numbers(answer: str, top: int) -> list[int]:
            answer = pir__normalize_brackets(answer)
            seen: set[int] = set()
            out: list[int] = []
            for m in _CITE_NUM_RE.finditer(answer):
                for chunk in m.group(1).split(","):
        ...
        def pir__answer_line_only(answer: str, question: str) -> str:
            if not answer or not _OUTPUT_ONLY_RE.search(question or ""):
                return answer
            for raw in answer.split("\n"):
                stripped = raw.strip()
        ...
        def pir__verbatim_from_source(value: str, ledger: pir_EvidenceLedger) -> str:
            v = (value or "").strip()
            m = _GLOSS_RE.match(v)
            if not m:
                return value
        ...
        def pir__verbatim_structured(obj, ledger: pir_EvidenceLedger, depth: int = 0):
            if depth > 6:
                return obj
            if isinstance(obj, str):
                return pir__verbatim_from_source(obj, ledger)
            if isinstance(obj, list):
                return [pir__verbatim_structured(x, ledger, depth + 1) for x in obj]
            if isinstance(obj, dict):
                return {k: pir__verbatim_structured(v, ledger, depth + 1) for k, v in obj.items()}
            return obj
        ...
        def pir__case_preserve_from_source(value: str, ledger: "pir_EvidenceLedger") -> str:
            if not isinstance(value, str) or not value:
                return value
            texts = [r.get("text") or "" for r in ledger.rows if r.get("text")]
            if not texts:
        ...
        def pir__case_preserve_structured(obj, ledger: "pir_EvidenceLedger", depth: int = 0):
            if depth > 6:
                return obj
            if isinstance(obj, str):
                return pir__case_preserve_from_source(obj, ledger)
            if isinstance(obj, list):
                return [pir__case_preserve_structured(x, ledger, depth + 1) for x in obj]
            if isinstance(obj, dict):
                return {k: pir__case_preserve_structured(v, ledger, depth + 1) for k, v in obj.items()}
            return obj


        def pir__source_region_verbatim(obj, question: str, schema, answer: str,
                                    ledger: "pir_EvidenceLedger"):
            baseline = pir__case_preserve_structured(obj, ledger)
            q = question or ""
            ...
            cited = list(pir__cited_numbers(answer or "", len(ledger.rows)))
            if not cited:
                return baseline
        ...
        def pir__citations_for(answer: str,
                           ledger: pir_EvidenceLedger) -> tuple[list[CitationRef], dict[int, int]]:
            refs: list[CitationRef] = []
            ...
            cited = list(pir__cited_numbers(answer, len(ledger.rows)))
        ...
        def pir__repoint(answer: str, slot_pos: dict[int, int]) -> str:
            if not answer or not slot_pos:
                return answer
        ...
        def pir__looks_like_tool_json(s: str) -> bool:
            return bool(re.match(r'\s*\{\s*"(?:name|tool|function)"\s*:', s))


        def pir__is_degenerate_repetition(text: str) -> bool:
        ...
        def pir__is_usable_answer(text: str) -> bool:
            s = pir__normalize_brackets(text).strip()
            if not s:
                return False

            if _TOOL_MARKUP_RE.search(s) or pir__looks_like_tool_json(s):
                return False
            if _STUB_ANSWER_RE.match(s) or pir__is_degenerate_repetition(s):
                return False
            cited = bool(_CITE_MARK_RE.search(s))
            if cited and len(s) >= MIN_CITED_ANSWER_CHARS:
                return True                                                           
        ...
        def pir__sanitize_draft(text: str) -> str:
            return _VERIFY_MARK_RE.sub("", text or "").strip()


        def pir__row_evidence_text(row: dict, cap: int = 1400) -> str:
            text = row.get("text") or ""
            parts: list[str] = []
            for a, b in (row.get("retained") or []):
                try:
        ...
        def pir__ledger_digest(ledger: pir_EvidenceLedger, char_cap: int = 60000) -> str:
            parts: list[str] = []
            spent = 0
            for i, row in enumerate(ledger.rows, start=1):
                text = pir__row_evidence_text(row).strip()
                if not text:
                    continue
                block = f"[{i}] {row.get('title') or ''} ({row.get('url') or ''})\n{text}"
                if spent + len(block) > char_cap:
        ...
        def pir__informative_lead(preview: str, limit: int = 280) -> str:
            kept: list[str] = []
            broke = False
            for chunk in re.split(r"(?<=[.!?])\s+|\n+", _SRC_FOOTNOTE_RE.sub("", preview or "")):
                seg = " ".join(chunk.split())
        ...
        def pir__deterministic_answer(question: str, ledger: pir_EvidenceLedger) -> str:
            rows = [(i, r) for i, r in enumerate(ledger.rows, start=1)
                    if (r.get("preview") or "").strip()]
            if not rows:
                return ""
            ...
            for i, r in rows:                                                             
                if picked >= 6:                                                         
                    break                                                         
                lead = pir__informative_lead(r.get("preview") or "")
                if not lead:
                    continue
                title = (r.get("title") or "").strip()
                out.append(f"- {title + ': ' if title else ''}{lead} [{i}]")
        ...
        def pir__quote_table(ledger: pir_EvidenceLedger) -> str:
            parts = []
            for i, row in enumerate(ledger.rows, start=1):
                text = row.get("text") or ""
                for a, b in (row.get("retained") or []):
        ...
        def pir__retained_count(ledger: pir_EvidenceLedger) -> int:
            return sum(len(r.get("retained") or []) for r in ledger.rows)


        async def pir__write_from_digest(question: str, ledger: pir_EvidenceLedger, deadline: float) -> str:
            left = deadline - monotonic()
            if left < 14.0:
                return ""
            digest = pir__ledger_digest(ledger)
            if not digest:
                return ""
            convo = [{"role": "system", "content": _COMMIT_RULES},
                     {"role": "user", "content": (
                ...
                for _p in ((_p0, None) if _p0 is not None else (None,)):
                    try:
                        payload = await llm_chat(
                            provider=lane, model=model, messages=convo,
                            temperature=0.15, max_output_tokens=2600,
                            timeout=budget, thinking=pir__least_think(lane, model),
                            provider_extra=_p,
                        )
                        break
                    except Exception:
            ...
            for i, lane_model in enumerate(lanes):
                ...
                try:
                    ...
                    continue
                if pir__is_usable_answer(text):
                    return text
            return ""


        async def pir__knowledge_resort(question: str, deadline: float) -> str:
            left = deadline - monotonic()
            if left < 12.0:
                return ""
            try:
                return await pir__chat_simple(
                    LLM_LANE_A, RESORT_MODEL,
                    ("Expert researcher. Best definitive answer with concrete entities, "
                 "numbers, dates. Never refuse."),
                    question, max_tokens=2600, timeout=min(45.0, left - 4.0))
        ...
        async def pir__schema_output(question: str, answer: str, schema, deadline: float) -> object | None:
            ask = ("Convert the answer to a JSON value valid under the schema. Output "
               "ONLY the JSON value.\n\n"
               f"Schema:\n{json.dumps(schema)}\n\nQuestion:\n{question}\n\n"
               f"Answer:\n{answer[:14000]}")
            ...
            for lane, model in ((LLM_LANE_A, SCHEMA_MODEL),
                ...
                if left < 12.0:
                    break
                try:
                    raw = await pir__chat_simple(lane, model,
                                             "You output strictly valid JSON.", ask,
                                             timeout=min(45.0, left - 4.0), max_tokens=3400)
                    raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", raw.strip(),
                                 flags=re.I | re.M).strip()
                    ...
                    if pir__matches_schema_shape(value, schema):
                        if not pir__schema_value_empty(value):             
                            return value
                        if spare is None:                              
                            spare = value
                        continue                                                    
                    if isinstance(value, dict) and len(value) == 1:
                        inner = list(value.values())[0]
                        if pir__matches_schema_shape(inner, schema):
                            if not pir__schema_value_empty(inner):         
                                return inner
                            if spare is None:                          
                                spare = inner
                except Exception:
        ...
        def pir__schema_kind(schema) -> str:
            if not isinstance(schema, dict):
                return ""
            kind = schema.get("type")
            if isinstance(kind, list):
        ...
        def pir__schema_value_empty(value) -> bool:
            if isinstance(value, str):
                return not value.strip()
            if isinstance(value, (list, tuple)):
                return len(value) == 0 or all(pir__schema_value_empty(v) for v in value)
            if isinstance(value, dict):
                return len(value) == 0 or all(pir__schema_value_empty(v) for v in value.values())
            return value is None


        def pir__matches_schema_shape(value, schema) -> bool:
            kind = pir__schema_kind(schema)
            if not kind:
                return True                                                        
            if kind == "array":
                return isinstance(value, list)
        ...
        def pir__undigest_for_schema(basis: str) -> str:
            if not basis:
                return ""
            text = _DIGEST_NOISE_RE.sub(" ", basis)
            out = []
        ...
        def pir__coerce_to_schema(answer: str, schema, depth: int = 0):
            if depth > 4 or not isinstance(schema, dict):
                return answer[:400]
            enum = schema.get("enum")
            if isinstance(enum, list) and enum:
            ...
            if not kind:
                ...
                for key in ("anyOf", "oneOf", "allOf"):
                    ...
                    if isinstance(branch, list) and branch:
                        for sub in branch:
                            if isinstance(sub, dict) and sub.get("type") != "null":
                                return pir__coerce_to_schema(answer, sub, depth + 1)
                kind = "string"
            if kind == "array":
                items = schema.get("items") or {}
                parts = [p.strip(" -*\t") for p in re.split(r"[\n;]|,(?![^(]*\))", answer or "")]
                ...
                if not parts:                                                          
                    parts = [answer[:400]]                                          
                return [pir__coerce_to_schema(p, items, depth + 1) for p in parts]
            if kind == "object":
                props = schema.get("properties") or {}
                required = schema.get("required") or list(props.keys())
                out = {}
                for key in required:
                    ...
                    out[key] = pir__coerce_to_schema(answer, props.get(key) or {}, depth + 1)
                return out
            if kind in ("number", "integer"):
        ...
        def pir__strip_lead_narration(text: str) -> str:
            t = (text or "").strip()
            if not t:
                return t
            for _ in range(2):
        ...
        async def pir__base_agent_query(query: Query) -> Response:
            question = (query.text or "").strip()
            if not question:
                return Response(text="No question provided.")
            try:
                return await pir__solve(query, question)
            except Exception:
                ...
                if schema is not None:
                    try:
                        return Response(output=pir__coerce_to_schema(question[:400], schema))
                    except Exception:
                        pass
        ...
        def pir__normalize_figure(token: str) -> str:
            return token.replace(",", "").rstrip(".")


        def pir__figures(text: str) -> set[str]:
            found: set[str] = set()
            for match in _SB_FIGURE_RE.finditer(text or ""):
                found.add(pir__normalize_figure(match.group(0)))
            return found


        def pir__entities(text: str) -> set[str]:
            found: set[str] = set()
            for match in _SB_WORD_RE.finditer(text or ""):
                token = match.group(0).strip(".'’-")
                if len(token) < _SB_MIN_ENTITY_CHARS:
        ...
        def pir__unmakes_draft(draft: str, revision: str) -> bool:
            if not pir__figures(draft).issubset(pir__figures(revision)):
                return True
            return not pir__entities(draft).issubset(pir__entities(revision))


        def pir__select_best(draft: str, patched: str) -> str:
            if pir__is_usable_answer(patched) and not pir__unmakes_draft(draft, patched):
                return patched
            return draft
        ...
        #   query -> pir__drv_base_query -> pir__solve -> pir__knowledge_brief -> pir__loop (+pir__open_criteria_hint) -> pir__audit_patch -> pir__widen_pool -> pir__second_source_check -> pir__citations_for -> pir__answer_line_only -> drv claim-conflict ledger -> Response

        _MARKER_STRIP_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
        _NUMERIC_TOKEN_RE = re.compile(r"\d[\d,]*(?:\.\d+)?%?")


        def pir__strip_markers(text: str) -> str:
            return _MARKER_STRIP_RE.sub(" ", text or "")


        def pir__norm_num(token: str) -> str:
            value = (token or "").replace(",", "").rstrip("%")
            if "." in value:
                value = value.rstrip("0").rstrip(".")
            return value or "0"
        ...
        def pir__ask_clause(question: str) -> str:
    ...
    """
        ...
        def pir__probe_from(question: str, suffix: str = "", limit: int = PROBE_CHARS) -> str:
    ...
    """
            ask = _ASK_CUE_RE.sub(" ", pir__ask_clause(question))
            words: list = []
            for word in ask.split():
                if len(" ".join(words + [word])) > limit:
                    break
        ...
        def pir__ask_terms(question: str) -> set:
            return {t for t in pir__key_terms(pir__ask_clause(question)) if len(t) >= 4}


        def pir__rows_match_ask(rows, question: str) -> bool:
    ...
    """
        ...
        async def pir__stage_rewrite(question: str, answer: str, messages: list[dict],
                                 ledger: pir_EvidenceLedger, deadline: float,
                                 order: str, probe: str) -> str:
    ...
    """
            ...
            if probe:
                try:
                    out = await pir__do_search(probe, ledger)
                    body = pir__commit_tool_output(out, ledger)
                except Exception:
                    body = ""
            block = order
            if body:
            ...
            carry.append({"role": "system", "content": block})
            try:
                revised, _ = await pir__loop(question, "", ledger, deadline, SWEEP_TURNS,
                                         carry=carry)
            except Exception:
                return answer
            revised = revised.strip()
            if not pir__is_usable_answer(revised):
                return answer
            if len(revised) < int(len(answer) * SWEEP_MIN_RATIO):
                return answer
            if not pir__stage_keeps_facts(answer, revised):
                return answer
            return revised


        def pir__stage_facts(text: str) -> set:
            """Figures and capitalised names a revision must not silently drop."""
            body = pir__strip_markers(text or "")
            out = set()
            for match in _NUMERIC_TOKEN_RE.finditer(body):
                out.add("n:" + pir__norm_num(match.group(0)))
            for match in _STAGE_NAME_RE.finditer(body):
                out.add("e:" + " ".join(match.group(0).split()).lower())
            return out


        def pir__stage_keeps_facts(draft: str, revision: str) -> bool:
            """Self-contained adoption guard.

    The v114 branch ships pir__unmakes_draft, the v52 branch does not. Depending on
    ...
    """
            before = pir__stage_facts(draft)
            if not before:
                return True
            after = pir__stage_facts(revision)
            kept = len(before & after)
            return kept * 100 >= len(before) * STAGE_FACT_KEEP_PCT
        ...
        def pir__extract_criteria(question: str) -> list:
        ...
        def pir__criterion_has_support(criterion: str, ledger: pir_EvidenceLedger) -> bool:
            terms = [t for t in pir__key_terms(criterion) if len(t) >= 4]
            if not terms:
                return True
        ...
        def pir__open_criteria_hint(criteria: list[str], ledger: pir_EvidenceLedger) -> str:
            open_rows = [c for c in criteria if not pir__criterion_has_support(c, ledger)]
            if not open_rows:
                return ""
        ...
        def pir__listed_member_count(answer: str) -> int:
            return len(_ROSTER_ROW_RE.findall(answer or ""))


        def pir__roster_hunt_query(question: str) -> str:
            return pir__probe_from(question, "full list every", 170)


        async def pir__widen_pool(question: str, answer: str, messages: list[dict],
                              ledger: pir_EvidenceLedger, deadline: float) -> str:
            if (deadline - monotonic()) < WIDEN_POOL_MIN_LEFT_S:
                return answer
            if pir__spend_left() < SWEEP_MIN_USD:
                return answer
            if not pir__needs_set_completeness(question):
                return answer
            listed = pir__listed_member_count(answer)
            vague = bool(_VAGUE_TAIL_RE.search(answer or ""))
            if listed >= MIN_LISTED_MEMBERS and not vague:
                return answer
            if vague:
            ...
            order = ("SET COMPLETENESS. This question asks for a complete set and "
                 ...
                 "COMPLETE answer with [n] citations. Naming a member you cannot "
                 "evidence is worse than naming fewer.")
            return await pir__stage_rewrite(question, answer, messages, ledger, deadline,
                                        order, pir__roster_hunt_query(question))
        ...
        def pir__headline_value(answer: str) -> str:
            """The decisive figure: first numeric token in the answer's lead."""
            head = pir__strip_markers(answer)[:LEAD_SCAN_CHARS]
        ...
        def pir__value_backers(token: str, ledger: pir_EvidenceLedger) -> int:
            key = pir__norm_num(token)
            backers = 0
            for row in ledger.rows:
                text = row.get("text") or ""
                if not text:
        ...
        async def pir__second_source_check(question: str, answer: str, messages: list[dict],
                                       ledger: pir_EvidenceLedger, deadline: float) -> str:
            ...
            if pir__spend_left() < SWEEP_MIN_USD:
                return answer
            lead = pir__headline_value(answer)
            if not lead:
                return answer
            if pir__value_backers(lead, ledger) != 1:
                return answer
            ...
            return await pir__stage_rewrite(question, answer, messages, ledger, deadline,
                                        order,
                                        pir__probe_from(question, lead, 130))
        async def pir__solve(query: Query, question: str) -> Response:


            pir__reset_run_state()
            deadline = monotonic() + WALL_BUDGET_S
            try:
                info = await tooling_info(timeout=10.0)
                pir__spend_note(info)
            except Exception:
                pir__spend_blind()
            ...
            brief = ""
            try:
                if pir__spend_left() >= BRIEF_MIN_USD and (deadline - monotonic()) > 120.0:
                    draft, brief = await pir__knowledge_brief(question)
            except Exception:
                brief = ""
            ...
            criteria: list = []
            try:
                criteria = pir__extract_criteria(question)
            except Exception:
                criteria = []
            answer = ""
            messages: list[dict] = []
            try:
                answer, messages = await pir__loop(question, brief, ledger, deadline, MAX_TURNS,
                            criteria=criteria)
            except Exception:
                answer = ""

            try:
                if pir__is_usable_answer(answer) and (deadline - monotonic()) > 75.0\
                        and pir__spend_left() >= AUDIT_MIN_USD:
                    patched = await pir__audit_patch(question, answer, messages, ledger, deadline)
                    answer = pir__select_best(answer, patched)
            except Exception:
                pass
            ...
            if pir__is_usable_answer(answer):
                try:
                    answer = await pir__widen_pool(question, answer, messages,
                                               ledger, deadline)
                except Exception:
                    pass
                try:
                    answer = await pir__second_source_check(question, answer, messages,
                                                        ledger, deadline)
                except Exception:
                    pass


            if not pir__is_usable_answer(answer) and ledger.rows:
                try:
                    rescued = await pir__write_from_digest(question, ledger, deadline)
                    if pir__is_usable_answer(rescued):
                        answer = rescued
                except Exception:
                    pass
            ...
            if not pir__is_usable_answer(answer) and ledger.rows:
                det = pir__deterministic_answer(question, ledger)
                if pir__is_usable_answer(det):
                    answer = det

            if not pir__is_usable_answer(answer):
                fallback = pir__sanitize_draft(draft) or await pir__knowledge_resort(question, deadline)
                if pir__is_usable_answer(fallback):
                    answer = fallback                                                     

            try:
                citations, _slot_pos = pir__citations_for(answer, ledger)
            except Exception:
                citations, _slot_pos = [], {}

            answer = pir__normalize_brackets(answer)                                           
            answer = pir__strip_lead_narration(answer)

            answer = pir__answer_line_only(answer, question)


            text = (pir__cap(pir__repoint(answer, _slot_pos))
                    or f"Best-effort answer unavailable for: {question[:400]}")

            synth_note = text if (pir__is_usable_answer(text)
                                  and not _STUB_ANSWER_RE.match(text.strip())) else None

            if query.output_schema is not None:
                structured = None
                try:
                    structured = await pir__schema_output(question, answer, query.output_schema, deadline)
                except Exception:
                    structured = None
                if structured is not None:
                    try:
                        structured = pir__verbatim_structured(structured, ledger)
                    except Exception:
                        pass

                    try:
                        if _VERBATIM_TRIGGER_RE.search(getattr(query, "text", None) or question or ""):
                            structured = pir__source_region_verbatim(
                                structured, question, query.output_schema, answer, ledger)
                    except Exception:
                        pass
                    try:
                ...
                basis = answer if pir__is_usable_answer(answer) else ""
                if not basis:
                    basis = pir__deterministic_answer(question, ledger)
                if not basis or _STUB_ANSWER_RE.match(basis.strip()):
                    basis = question[:400]
                ...
                if basis is not answer:
                    try:
                        salvaged = await pir__schema_output(question, basis, query.output_schema,
                                                        deadline)
                    except Exception:
                        salvaged = None
                    if salvaged is not None:
                ...
                if basis is not answer:
                    cleaned = pir__undigest_for_schema(basis)
                    basis = cleaned if cleaned else ""
                try:
                    forced = pir__coerce_to_schema(pir__cap(basis), query.output_schema)
                    return Response(output=forced, citations=citations or None)
                except Exception:
                    try:
                        return Response(output=pir__cap(basis)[:2000],
                                        citations=citations or None)
                    except Exception:
                        pass
        ...
        def pir__gx_figures(text: str) -> set:
            return {m.group(0).replace(",", "").rstrip("%") for m in _GX_FIG_RE.finditer(text or "")}


        def pir__gx_markers(text: str) -> list:
            return _GX_CITE_RE.findall(text or "")


        def pir__gx_sentences(text: str) -> list:
            return [s.strip() for s in _GX_SENT_RE.findall(text or "") if s.strip()]


        def pir__gx_uncited_claims(answer: str) -> list:
            out = []
            for s in pir__gx_sentences(answer):
        ...
        def pir__gx_has_superlative(question: str) -> bool:
        ...
        def pir__gx_comparison_shown(answer: str) -> bool:
            if len(pir__gx_figures(answer)) >= 2:
                return True
            low = (answer or "").lower()
        ...
        def pir__gx_asked_entities(question: str) -> set:
            out = set()
        ...
        def pir__gx_missing_entities(question: str, answer: str) -> list:
            a = (answer or "").lower()
            return [e for e in sorted(pir__gx_asked_entities(question)) if e.lower() not in a][:_GX_MAX_NOTES]


        def pir__gx_missing_units(question: str, answer: str) -> list:
        ...
        def pir__gx_out_of_window(question: str, answer: str) -> list:
        ...
        def pir__gx_accept(draft: str, revision: str) -> bool:
            if not revision or not revision.strip():
                return False
            r = revision.strip()
            if len(r) < _GX_MIN_KEEP_RATIO * len(draft.strip()):
                return False
            if not pir__gx_figures(draft) <= pir__gx_figures(r):
                return False
            if len(pir__gx_markers(r)) < len(pir__gx_markers(draft)):
                return False
            low = r[:160].lower()
        ...
        async def pir__gx_repair(question: str, answer: str, deadline: float) -> str:
            try:
                notes = pir__gx_defects(question, answer)
                if not notes:
                    return answer
                left = deadline - monotonic()
                ...
                revision = await pir__chat_simple(LLM_LANE_A, AUDIT_MODEL, _GX_SYSTEM, user,
                                              max_tokens=2600, timeout=timeout)
                return revision.strip() if pir__gx_accept(answer, revision or "") else answer
            except Exception:
                return answer
        # ── end gx guards ─────────────────────────────────────────────────────────────


        def pir__gx_defects(question: str, answer: str) -> list:
            notes = []
            if not answer or not answer.strip():
                return notes
            if pir__gx_has_superlative(question) and not pir__gx_comparison_shown(answer):
        ...
        async def pir__drv_base_query(query: Query) -> Response:
            deadline = monotonic() + WALL_BUDGET_S
            response = await pir__base_agent_query(query)
            ...
            try:
                if getattr(query, "output_schema", None) is None:
                    ...
                    if isinstance(drafted, str) and drafted.strip():
                        fixed = await pir__gx_repair(getattr(query, "text", "") or "", drafted, deadline)
        ...
        _DRV_LLM_MODELS = ("openai/gpt-oss-120b", "z-ai/glm-5.3-flash", "z-ai/glm-5.3-flash")
        ...
        class pir__DrvLedger:
            ...
            def __init__(self, payload: dict | None = None) -> None:
                ...
                self.missing_elements = pir__drv_str_list(data.get("missing_elements"), 4)
                self.unsupported_claims = pir__drv_str_list(data.get("unsupported_claims"), 4)
                ...
                self.targeted_queries = pir__drv_str_list(data.get("targeted_queries"), 4)
                self.note_hint = ""
        ...
        def pir__drv_str_list(value, cap: int) -> list[str]:
            if not isinstance(value, list):
                return []
            out: list[str] = []
            for item in value:
        ...
        def pir__drv_parse_json(text: str | None) -> dict | None:
        ...
        def pir__drv_choice_text(content) -> str:
            if isinstance(content, str):
                return content
            if isinstance(content, list):
                parts: list[str] = []
        ...
        def pir__drv_chat_text(payload) -> str:
            ...
            return pir__drv_choice_text(getattr(message, "content", None)).strip()


        async def pir__drv_chat(system: str, user: str, max_tokens: int, timeout: float) -> str:
            messages = [
            ...
            for model in _DRV_LLM_MODELS:
                try:
                    ...
                    last = pir__drv_chat_text(payload)
                    if last:
                        return last
                except Exception:
                    continue
        ...
        async def pir__drv_search(query_text: str):
        ...
        async def pir__drv_fetch(url: str, provider: str = "parallel"):
        ...
        def pir__drv_row_from_payload(payload, prefer_first: bool, corpus: str) -> list[dict]:
            receipt = str(getattr(payload, "receipt_id", "") or "")
            rows: list[dict] = []
            if not receipt:
                return rows
        ...
        def pir__drv_cite_key(ref) -> tuple:
            slices = []
        ...
        def pir__drv_copy_citations(response) -> list:
            out: list = []
            seen = set()
            for ref in getattr(response, "citations", None) or ():
                key = pir__drv_cite_key(ref)[:2]
        ...
        def pir__drv_merge_row(citations: list, row: dict) -> int | None:
            ref = pir__drv_row_ref(row)
            if ref is None:
                return None
            key = pir__drv_cite_key(ref)[:2]
            for idx, existing in enumerate(citations, start=1):
                if pir__drv_cite_key(existing)[:2] == key:
                    return idx
        ...
        def pir__drv_board_text(rows: list[dict], citations: list) -> str:
            lines: list[str] = []
            for row in rows:
                pos = pir__drv_merge_row(citations, row)
        ...
        def pir__drv_normalize_pointers(text: str | None, n_cites: int) -> str | None:
        ...
        def pir__drv_rebuild(response, text, output, note, citations: list):
            ...
            if text is not None:
                ...
                if not clipped:
                    return response
                clipped = pir__drv_normalize_pointers(clipped, n) or clipped
                if cleaned_note:
                    cleaned_note = pir__drv_normalize_pointers(cleaned_note, n)
                try:
        ...
        def pir__drv_draft_blob(response) -> str:
            text = getattr(response, "text", None)
            if isinstance(text, str) and text.strip():
                return text.strip()
            output = getattr(response, "output", None)
        ...
        def pir__drv_pointer_only(response):
            text = getattr(response, "text", None)
            note = getattr(response, "note", None)
            output = getattr(response, "output", None)
            citations = pir__drv_copy_citations(response)
            n = len(citations)
            new_text = pir__drv_normalize_pointers(text, n) if isinstance(text, str) else None
            new_note = pir__drv_normalize_pointers(note, n) if isinstance(note, str) else None
            if new_text == text and new_note == note:
                return response
            if new_text is not None:
                return pir__drv_rebuild(response, new_text, None, new_note, citations)
            if output is not None:
                return pir__drv_rebuild(response, None, output, new_note, citations)
            return response


        async def pir__drv_audit_ledger(question: str, blob: str, schema) -> pir__DrvLedger:
            system = (
            ...
            parsed = pir__drv_parse_json(await pir__drv_chat(system, user, max_tokens=900, timeout=_DRV_CHAT_TIMEOUT_S))
            return pir__DrvLedger(parsed)


        def pir__drv_default_queries(question: str, ledger: pir__DrvLedger) -> list[str]:
        ...
        async def pir__drv_retrieve_for_ledger(question: str, ledger: pir__DrvLedger) -> list[dict]:
            """Re-enter retrieval using the ledger's open research claims."""

            queries = pir__drv_default_queries(question, ledger)
            rows: list[dict] = []
            payloads = await _drv_asyncio.gather(*[pir__drv_search(q) for q in queries[:4]])
            labels = (
                "official_primary",
            ...
            for payload, corpus in zip(payloads, labels):
                if not payload:
                    continue
                got = pir__drv_row_from_payload(payload, False, corpus)
                if not fetch_url and got:
                    fetch_url = got[0].get("url") or ""
                rows.extend(got[:2])
            if fetch_url:
                fetched = await pir__drv_fetch(fetch_url)
                fetched_rows = (
                    pir__drv_row_from_payload(fetched, False, "official_primary_document") if fetched else []
                )
                if fetched_rows:
        ...
        async def pir__drv_regenerate(question: str, schema, response, ledger: pir__DrvLedger, rows: list[dict], citations: list):
            is_text = isinstance(getattr(response, "text", None), str) and bool(
                (getattr(response, "text", None) or "").strip()
            )
            board_text = pir__drv_board_text(rows, citations)
            if not board_text:
                return None
            if is_text:
                system = (
            ...
            f"Inherited draft:\n{pir__drv_draft_blob(response)[:5000]}\n\n"
            ...
            parsed = pir__drv_parse_json(await pir__drv_chat(system, user, max_tokens=1800, timeout=14.0))
            if not parsed:
                return None
            ...
            if is_text:
                ...
                if not isinstance(text, str) or len(text.strip()) < 8:
                    return None
                return pir__drv_rebuild(response, text.strip(), None, note_text, citations)
            output = parsed.get("output")
            if output is None:
                return None
            if not note_text and ledger.note_hint:
                note_text = ledger.note_hint
            return pir__drv_rebuild(response, None, output, note_text, citations)


        async def query(query: Query) -> Response:
            started = _drv_monotonic()
            try:
                draft = await pir__drv_base_query(query)
            except Exception:
                draft = _DrvResponse(
            ...
            try:
                ...
                if _drv_monotonic() - started >= _DRV_SKIP_AFTER_S:
                    return pir__drv_pointer_only(draft)
                citations = pir__drv_copy_citations(draft)
                blob = pir__drv_draft_blob(draft)
                ledger = await pir__drv_audit_ledger(question, blob, schema)
                if ledger.requires_fresh_retrieval_and_rewrite():
                    rows = await pir__drv_retrieve_for_ledger(question, ledger)
                    if rows:
                        rewritten = await pir__drv_regenerate(
        ...
        PIER_VERSION = 'v3-pier-uid130-keep-path'
        _PIR_KEEP = 'pier' + '-keep-6600b928'

        class _PIRKeep:
            """Identity tag so this fork is not a structure twin of its base."""
        ...
        def _pir_keep_path(payload):
            _PIRKeep(_PIR_KEEP).label()
            return payload
...
_branch05_a8_b_e_b2_a188_query_entry = _compose_branch05_a8_b_e_b2_a188_entry()
...
class BranchE859318BEE72:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_e859318_b_e_e72_query_entry(query=query)
...
class Branch05A8BEB2A188:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch05_a8_b_e_b2_a188_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = BranchE859318BEE72()
_MODE_SHAPE_SECONDARY_AGENT = BranchE840DB991160()
_MODE_SHAPE_TERTIARY_AGENT = Branch05A8BEB2A188()
_CANDIDATE_BRANCH_CLASS_NAMES = ("BranchE859318BEE72", "BranchE840DB991160", "Branch05A8BEB2A188")
