...
_CANDIDATE_BRANCH_CLASS_NAMES = ("Branch34534862AA95", "Branch3C44203CF6A6", "Branch0B07E402547A")
_CANDIDATE_ROUTE_FUNCTION = "_mode_shape_route_index"


async def _a08_base_query(query: Query, context: ContextSnapshot) -> Response:
...
# ============================================================================
# SN67 OVERLAY FAMILY F2 -- answer contract
# ----------------------------------------------------------------------------
# Mechanism : pre-research acceptance contract -> base research -> verify -> repair
# Gate reads: the question, before research
# Gate does : verify + repair
# Timing    : before and after base
#
...
#   2. Rename its entrypoint `query` -> `_a08_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _a08_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _A08_TAG below to a value unique to this agent.
#
# TUNE: _A08_SKIP_AFTER_S = 235.0 assumes a ~300s wall. Lower it if the host
...
_A08_TAG = "F2-CHANGE-ME"

import asyncio as _a08_asyncio
import json as _a08_json
import re as _a08_re
from time import monotonic as _a08_monotonic

from harnyx_miner_sdk.api import fetch_page as _a08_fetch_page
from harnyx_miner_sdk.api import llm_chat as _a08_llm_chat
from harnyx_miner_sdk.api import search_web as _a08_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as _F2CitationRef
from harnyx_miner_sdk.query import CitationSlice as _F2CitationSlice
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_A08_PROVIDER = "openrouter"
_A08_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_A08_SEARCH_PROVIDERS = ("parallel", "exa")
_A08_CHAT_TIMEOUT_S = 12.0
_A08_SEARCH_TIMEOUT_S = 12.0
_A08_FETCH_TIMEOUT_S = 14.0
_A08_SKIP_AFTER_S = 235.0
_A08_ANSWER_CAP = 60000
_A08_NOTE_CAP = 8000
_A08_MAX_CITES = 32
_A08_FENCE_RE = _a08_re.compile(r"^```(?:json)?\s*|\s*```$", _a08_re.I | _a08_re.M)
_A08_POINTER_RE = _a08_re.compile(r"\[\[(\d+)\]\]")


def _a08_parse_json(text):
    if not text:
        return None
    body = _A08_FENCE_RE.sub("", text).strip()
    try:
        value = _a08_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _a08_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _a08_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _a08_chat_text(payload):
    ...
    return _a08_choice_text(getattr(message, "content", None)).strip()


async def _a08_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _A08_MODELS:
        try:
            payload = await _a08_asyncio.wait_for(
                _a08_llm_chat(
                    provider=_A08_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _a08_chat_text(payload)
        if text:
            return text
    return ""


async def _a08_search(query_text):
    for provider in _A08_SEARCH_PROVIDERS:
        try:
            payload = await _a08_asyncio.wait_for(
                _a08_search_web(provider=provider, query=query_text),
                timeout=_A08_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _a08_fetch(url):
    try:
        return await _a08_asyncio.wait_for(
            _a08_fetch_page(provider="parallel", url=url),
            timeout=_A08_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _a08_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _a08_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        ...
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(_F2CitationSlice(start=start, end=end))
        out.append(_F2CitationRef(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _A08_MAX_CITES:
            break
    return out


def _a08_merge_row(citations, row):
    for position, ref in enumerate(citations):
        ...
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _A08_MAX_CITES:
        return None
    note = row["note"]
    citations.append(_F2CitationRef(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[_F2CitationSlice(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _a08_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _a08_merge_row(citations, row)
        if pointer is None:
            continue
...
def _a08_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_A08_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _a08_json.dumps(output)[:_A08_ANSWER_CAP]
    except Exception:
        return str(output)[:_A08_ANSWER_CAP]


def _a08_normalize_pointers(text, n_cites):
    ...
    return _A08_POINTER_RE.sub(_keep, text)


def _a08_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_a08_normalize_pointers(text, n)[:_A08_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
...
def _a08_pointer_only(response):
    citations = _a08_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_a08_normalize_pointers(text, len(citations))[:_A08_ANSWER_CAP],
            citations=citations,
        )
    return response

_A08_PLAN_SYSTEM = (
    "Plan an acceptance contract for a research answer. Return JSON only with "
    "keys required_items (string array of at most six concrete deliverables the "
    "answer must contain) and basis (string describing the period, units or "
    "comparison basis the question fixes)."
)
_A08_VERIFY_SYSTEM = (
    "Check a draft answer against its acceptance contract. Return JSON only "
    "with keys satisfied (boolean), unmet (string array) and revision (string, "
    "a corrected answer, or empty when the draft already satisfies the "
    "contract). Preserve every [[n]] pointer the draft used."
)


async def _a08_plan_contract(question):
    parsed = _a08_parse_json(await _a08_chat(
        _A08_PLAN_SYSTEM, "QUESTION:\n" + question, 700, 22.0
    ))
    if parsed is None:
        return {}
    return {
        "required_items": _a08_str_list(parsed.get("required_items"), 6),
        "basis": str(parsed.get("basis") or "")[:300],
    }


def _a08_contract_active(contract):
    """The gate: a contract with no required items cannot decide anything."""
    return bool(isinstance(contract, dict) and contract.get("required_items"))


async def _a08_verify(question, contract, draft):
    user = (
        "QUESTION:\n" + question
        + "\n\nCONTRACT ITEMS:\n" + _a08_json.dumps(contract.get("required_items") or [])
        + "\n\nBASIS:\n" + str(contract.get("basis") or "")
        + "\n\nDRAFT:\n" + draft[:10000]
    )
    parsed = _a08_parse_json(await _a08_chat(_A08_VERIFY_SYSTEM, user, 2200, 28.0))
    if parsed is None:
        return None
    return {
        "satisfied": parsed.get("satisfied") is True,
        "unmet": _a08_str_list(parsed.get("unmet"), 6),
        "revision": str(parsed.get("revision") or ""),
    }


@entrypoint("query")
async def query(query: Query, context: ContextSnapshot) -> Response:
    started = _a08_monotonic()
    question = str(getattr(query, "text", "") or "")
    contract = {}
    if not bool(getattr(query, "fast", False)):
        try:
            contract = await _a08_plan_contract(question)
        except Exception:
            contract = {}
    draft = await _a08_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    try:
        if _a08_monotonic() - started >= _A08_SKIP_AFTER_S:
            return _a08_pointer_only(draft)
        if not _a08_contract_active(contract):
            return _a08_pointer_only(draft)
        citations = _a08_copy_citations(draft)
        verdict = await _a08_verify(question, contract, _a08_draft_text(draft))
        if verdict is None or verdict["satisfied"]:
            return _a08_pointer_only(draft)
        revision = verdict["revision"].strip()
        if len(revision) >= 80:
            return _a08_rebuild(draft, revision, None, citations)
        return _a08_pointer_only(draft)
    except Exception:
        return draft
