...
def _compose_branch_b26_a_c_a364_b08_entry():
    ...
    """
        ...
        # --- ccl: coverage-gap retrieval cycle (start) ---
        # Ordinary-path architecture added relative to the baseline agent.
        #
        #   research -> draft -> DETERMINISTIC coverage ledger -> (if any element is
        #   still open) fresh targeted retrieval -> regenerated answer
        #
        # The baseline's own audit (`_audit_patch`) lives INSIDE the research stage and
        # rewrites from evidence the research already holds. This cycle sits OUTSIDE the
        # research stage: it reads the produced draft, decides from the evidence actually
        # retained whether a required element of the question is still unresolved, and on
        # that finding sends the run back into retrieval and rebuilds the answer from the
        # newly fetched sources. The audit itself spends no model call -- it is a
        # structural comparison of question, draft and ledger -- so the cycle costs one
        # regeneration only when it actually fires.

        _CCL_RUN: dict = {"ledger": None}

        _CCL_MAX_GAPS = 3
        _CCL_MAX_SEARCHES = 2
        _CCL_MAX_FETCHES = 2
        _CCL_MIN_LEFT_S = 55.0
        _CCL_REGEN_RESERVE_S = 32.0
        _CCL_MAX_CYCLE_S = 45.0
        _CCL_RETRIEVE_TIMEOUT_S = 25.0
        _CCL_REGEN_TIMEOUT_S = 60.0
        _CCL_MIN_USD = 0.04
        _CCL_EVIDENCE_CHARS = 9000
        _CCL_DRAFT_CHARS = 9000
        _CCL_ROW_CHARS = 1400
        _CCL_MIN_TERM_LEN = 4
        _CCL_MIN_CODE_LEN = 2
        _CCL_MIN_STEM_TERMS = 3
        _CCL_MIN_KEEP_RATIO = 0.6
        _CCL_MIN_SENTENCE_CHARS = 40
        _CCL_VACANT = frozenset(
            "n/a na none null nil unknown unspecified unavailable undetermined "
        "notfound not-found - -- ? 0 0.0 tbd".split())

        _CCL_TOKEN_RE = re.compile(r"[A-Za-z][A-Za-z0-9'\-]{1,}|\d[\d,.]*")
        _CCL_NUM_RE = re.compile(r"\d[\d,.]*")
        _CCL_CITE_RE = re.compile(r"\[\d+\]")
        _CCL_SENT_RE = re.compile(r"(?<=[.!?])\s+")
        _CCL_SENT_START_RE = re.compile(r"(?:^|[.!?:;]\s+|\n)\s*")
        _CCL_MULTIWORD_RE = re.compile(r"\b[A-Z][A-Za-z0-9.\-]+(?:\s+[A-Z][A-Za-z0-9.\-]+)+")
        _CCL_HEDGE_RE = re.compile(
            r"not (?:found|available|stated|specified|listed|reported)"
        r"|unable to (?:determine|find|verify|confirm)"
        r"|could not (?:find|determine|verify|locate)"
        r"|no (?:verifiable|reliable|direct) (?:answer|source|evidence)"
        r"|insufficient (?:evidence|information|data)"
        r"|unclear from the", re.I)

        _CCL_REGEN_SYSTEM = (
            "You rewrite a research answer using newly retrieved sources. Keep every "
        "correct fact and citation the draft already had; add what the new evidence "
        "establishes; drop any claim the new evidence contradicts. Cite every "
        "load-bearing fact with [n] using the bracketed numbers of the evidence "
        "shown. Answer the question that was asked, in the shape it asks for. Never "
        "mention the draft, the gaps, the sources list, or this revision."
        )


        def _ccl_evidence_text(ledger: EvidenceLedger) -> str:
            parts: list[str] = []
            for row in ledger.rows:
                parts.append(row.get("title") or "")
                parts.append(row.get("text") or row.get("preview") or "")
            return "\n".join(parts)


        def _ccl_anchor_terms(question: str) -> list[str]:
            """The question's own anchoring elements, most search-anchoring first.

    Only tokens that identify something are eligible: a token carrying a digit, an
    all-caps code, or a capitalised word that is NOT the first word of its
    sentence. Ordinary vocabulary -- the verbs and common nouns a question is
    phrased with -- is excluded, because an answer is not deficient for omitting
    the word "landed". The test is positional and structural, so no vocabulary is
    baked in and it carries across batches unchanged.
    """
            text = question or ""
            starts = {0}
            for m in _CCL_SENT_START_RE.finditer(text):
                starts.add(m.end())
            scored: list[tuple] = []
            seen: set[str] = set()
            for m in _CCL_TOKEN_RE.finditer(text):
                word = m.group(0)
                key = word.casefold()
                has_digit = any(ch.isdigit() for ch in word)
                all_caps = word.isupper() and word.isalpha() and len(word) >= _CCL_MIN_CODE_LEN
                if key in _STOP or key in seen:
                    continue
                if len(word) < _CCL_MIN_TERM_LEN and not all_caps:
                    continue
                capitalised = word[:1].isupper() and m.start() not in starts
                if not (has_digit or all_caps or capitalised):
                    continue
                seen.add(key)
                scored.append((0 if (has_digit or all_caps) else 1, -len(word), len(scored), word))
            scored.sort()
            return [t[3] for t in scored]


        def _ccl_draft_text(response: object) -> str:
            """A text view of the answer, whether it was returned as prose or structured."""
            text = _w4_response_text(response)
            if text:
                return text
            try:
                output = getattr(response, "output", None)
            ...
            if output is None:
                return ""
            try:
                return json.dumps(output, ensure_ascii=False)[:_CCL_DRAFT_CHARS]
            except Exception:
                return str(output)[:_CCL_DRAFT_CHARS]


        def _ccl_vacant_fields(response: object) -> list[str]:
            """Structured fields the run left empty.

    A schema field answered "N/A", "unknown", 0 or an empty list is the structured
    form of the draft saying it could not resolve something -- the same finding the
    hedge test makes in prose, which only reads sentences. Field names come from
    whatever schema the task supplied, so nothing here is specific to a task.
    """
            try:
                output = getattr(response, "output", None)
            except Exception:
            ...
            for key, value in output.items():
                if isinstance(value, (list, tuple, dict)):
                    ...
                    empty = not text or text in _CCL_VACANT
                if empty:
                    vacant.append(str(key))
            return vacant


        def _ccl_field_probe(field: str) -> str:
            """Turn a schema field name back into words a search can use."""
            words = re.split(r"[_\-.]+|(?<=[a-z0-9])(?=[A-Z])", field or "")
            return " ".join(w for w in words if w and w.casefold() not in _STOP)


        def _ccl_open_gaps(question: str, draft: str, ledger: EvidenceLedger,
                           vacant: list[str] | None = None) -> list[dict]:
            """Decide, without a model call, which required elements are still open.

    Three structural findings, each of which the evidence can actually settle:
    an element of the question that reached neither the answer nor the retained
    evidence; the draft declaring it could not resolve something; and a
    load-bearing sentence asserting a figure or named entity with no citation.
    """
            gaps: list[dict] = []
            seen: set[tuple[str, str]] = set()

            def push(kind: str, probe: str, why: str) -> None:
                probe = " ".join(probe.split())[:200]
                key = (kind, probe.casefold())
                if probe and key not in seen:
                    seen.add(key)
                    gaps.append({"kind": kind, "probe": probe, "why": why})

            anchors = _ccl_anchor_terms(question)
            stem_terms = list(anchors[:6])
            if len(stem_terms) < _CCL_MIN_STEM_TERMS:
                # Some questions name almost nothing in capitals. Fall back to the longest
                # content words so the probe still carries the subject of the question.
                held_out = {t.casefold() for t in stem_terms}
                extra = sorted(_key_terms(question), key=lambda w: (-len(w), w))
                stem_terms.extend(w for w in extra[:_CCL_MIN_STEM_TERMS * 2]
                                  if w not in held_out)
            stem = " ".join(stem_terms[:6])
            if not stem:
                return []
            held = _key_terms(_ccl_evidence_text(ledger))
            stated = _key_terms(draft)

            for term in anchors:
                if len(gaps) >= _CCL_MAX_GAPS:
                    return gaps
                key = term.casefold()
                if key not in stated and key not in held:
                    probe = stem if key in stem.casefold().split() else f"{stem} {term}"
                    push("uncovered", probe,
                         f"the question's {term!r} appears in neither the answer nor the evidence")

            for field in (vacant or []):
                if len(gaps) >= _CCL_MAX_GAPS:
                    return gaps
                words = _ccl_field_probe(field)
                if words:
                    push("vacant", f"{stem} {words}",
                         f"the answer left {field!r} empty")

            if len(gaps) < _CCL_MAX_GAPS and _CCL_HEDGE_RE.search(draft or ""):
                push("declared", stem,
                     "the draft reports it could not resolve part of the question")

            for sentence in _CCL_SENT_RE.split(draft or ""):
                if len(gaps) >= _CCL_MAX_GAPS:
                    break
                line = sentence.strip()
                if len(line) < _CCL_MIN_SENTENCE_CHARS or _CCL_CITE_RE.search(line):
                    continue
                marks = _CCL_MULTIWORD_RE.findall(line)[:2] + _CCL_NUM_RE.findall(line)[:2]
                marks = [m for m in marks if len(m) >= 2]
                if not marks:
                    continue
                push("uncited", f"{stem} {' '.join(marks)}",
                     f"this claim carries no citation: {line[:140]}")

            return gaps[:_CCL_MAX_GAPS]


        async def _ccl_retrieve(question: str, gaps: list[dict], ledger: EvidenceLedger,
                                deadline: float) -> list[tuple[int, dict]]:
            """Re-enter retrieval, aimed at the open elements, and register what comes back."""
            added: list[tuple[int, dict]] = []
            searches = 0
            fetches = 0

            def _affordable(reserve: float) -> bool:
                return _w4_remaining(deadline) >= reserve and _spend_left() >= _CCL_MIN_USD

            def _since(mark: int) -> list[tuple[int, dict]]:
                return [(i + 1, row) for i, row in enumerate(ledger.rows) if i >= mark]

            for gap in gaps:
                if searches >= _CCL_MAX_SEARCHES or not _affordable(_CCL_REGEN_RESERVE_S):
                    break
                searches += 1
                mark = len(ledger.rows)
                budget = min(_CCL_RETRIEVE_TIMEOUT_S, _w4_remaining(deadline))
                if budget <= 0:
                    break
                try:
                    out = await asyncio.wait_for(_do_search(gap["probe"], ledger), timeout=budget)
                except Exception:
                    continue
                ...
                if fetches >= _CCL_MAX_FETCHES or not _affordable(_CCL_REGEN_RESERVE_S):
                    continue
                url = ""
                for _n, row in fresh:
                ...
                if budget <= 0:
                    continue
                try:
                    out = await asyncio.wait_for(_do_fetch(url, gap["probe"], question, ledger),
                                                 timeout=budget)
                except Exception:
                    continue
                _commit_tool_output(out, ledger)
                added.extend(_since(mark))

            return added


        def _ccl_evidence_block(added: list[tuple[int, dict]]) -> str:
            lines: list[str] = []
            total = 0
            for number, row in added:
                if row.get("kind") == "reserved":
                    continue
                body = (row.get("text") or row.get("preview") or "").strip()
                if not body:
                    continue
                line = (f"[{number}] {(row.get('title') or '').strip()} — "
                    f"{(row.get('url') or '').strip()}\n{body[:_CCL_ROW_CHARS]}")
                if total + len(line) > _CCL_EVIDENCE_CHARS:
                    break
                total += len(line)
                lines.append(line)
            return "\n\n".join(lines)
        ...
        async def _ccl_regenerate(question: str, schema: object, draft: str, gaps: list[dict],
                                  added: list[tuple[int, dict]], ledger: EvidenceLedger,
                                  deadline: float) -> Response | None:
            block = _ccl_evidence_block(added)
            if not block:
                return None
            timeout = min(_CCL_REGEN_TIMEOUT_S, _w4_remaining(deadline) - MIN_TAIL_S)
            if timeout <= 5.0:
                return None
            findings = "\n".join(f"- {gap['why']}" for gap in gaps)
            messages = [
                {"role": "system", "content": _CCL_REGEN_SYSTEM},
                {"role": "user", "content": (
                    f"Question:\n{question}\n\n"
                f"Open elements found in the draft:\n{findings}\n\n"
                f"Newly retrieved evidence:\n{block}\n\n"
                f"Draft answer:\n{draft[:_CCL_DRAFT_CHARS]}\n\n"
                "Rewrite the complete answer now."
                )},
            ]
            revised = await _w4_chat(messages, timeout=timeout, temperature=0.0)
            revised = _sanitize_draft(revised) if revised else ""
            if not revised or not _is_usable_answer(revised):
                return None
            if len(revised) < int(len(draft) * _CCL_MIN_KEEP_RATIO):
                return None

            refs, slot_pos = _citations_for(revised, ledger)
            revised = _repoint(revised, slot_pos)
            if schema is not None:
                try:
                    structured = await _schema_output(question, revised, schema, deadline)
                except Exception:
                    structured = None
                ...
                try:
                    ...
                    return None
            try:
                return Response(text=_cap(revised), citations=refs or None)
            except Exception:
                return None


        async def _ccl_cycle(question: str, schema: object, response: object,
                             *, deadline: float) -> Response | None:
            """The cycle: audit the delivered draft, and on an open element research again."""
            ledger = _CCL_RUN.get("ledger")
            if not isinstance(ledger, EvidenceLedger) or not ledger.rows:
                return None
            draft = _ccl_draft_text(response)
            if not draft:
                return None
            gaps = _ccl_open_gaps(question, draft, ledger, _ccl_vacant_fields(response))
            if not gaps:
                return None
            if _w4_remaining(deadline) < _CCL_MIN_LEFT_S or _spend_left() < _CCL_MIN_USD:
                return None
            # One ceiling for the whole stage: whichever of the task deadline and the
            # cycle's own allowance comes first. Every await below is bounded by it, so
            # the stage cannot push the task past the platform's hard kill.
            stop = perf_counter() + _CCL_MAX_CYCLE_S
            cycle_deadline = deadline if deadline < stop else stop
            added = await _ccl_retrieve(question, gaps, ledger, cycle_deadline)
            if not added:
                return None
            return await _ccl_regenerate(question, schema, draft, gaps, added, ledger,
                                         cycle_deadline)
        # --- ccl: coverage-gap retrieval cycle (end) ---


        async def query(query: Query) -> Response:
            """Research, then audit the delivered draft and research again if it is short.

    The research stage produces a cited draft. The coverage cycle then compares
    that draft against the question and against the evidence the run actually
    retained; when a required element is still open it retrieves fresh sources
    aimed at that element and rebuilds the answer from them. Verification that
    cannot reach new evidence can only restate the draft, so the audit is wired
    to retrieval rather than to a rewrite.
    """
            deadline = perf_counter() + _w4_total_budget_seconds()
            question = getattr(query, "text", "") or ""
            schema = getattr(query, "output_schema", None)
            _CCL_RUN["ledger"] = None

            response = await _w4_research_or_salvage(query)

            # Fast mode is correctness-only F1: a rebuilt answer can only add components
            # the scorer penalises, so the cycle is the non-fast path's stage.
            if not bool(getattr(query, "fast", False)):
                try:
                    rebuilt = await _ccl_cycle(question, schema, response, deadline=deadline)
                except Exception:
                    rebuilt = None
...
_branch_b26_a_c_a364_b08_query_entry = _compose_branch_b26_a_c_a364_b08_entry()
...
class BranchB26ACA364B08:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch_b26_a_c_a364_b08_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = BranchE859318BEE72()
_MODE_SHAPE_SECONDARY_AGENT = BranchE840DB991160()
_MODE_SHAPE_TERTIARY_AGENT = BranchB26ACA364B08()
_CANDIDATE_BRANCH_CLASS_NAMES = ("BranchE859318BEE72", "BranchE840DB991160", "BranchB26ACA364B08")
