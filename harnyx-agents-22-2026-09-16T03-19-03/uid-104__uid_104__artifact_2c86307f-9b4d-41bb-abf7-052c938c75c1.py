...
def _compose_branch49_f_d_a7710187_entry():
...
_branch49_f_d_a7710187_query_entry = _compose_branch49_f_d_a7710187_entry()
...
class Branch49FDA7710187:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch49_f_d_a7710187_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = BranchFC2508A5BF96()
_MODE_SHAPE_SECONDARY_AGENT = Branch24ACBE3AF69D()
_MODE_SHAPE_TERTIARY_AGENT = Branch49FDA7710187()
_CANDIDATE_BRANCH_CLASS_NAMES = ("BranchFC2508A5BF96", "Branch24ACBE3AF69D", "Branch49FDA7710187")
_CANDIDATE_ROUTE_FUNCTION = "_mode_shape_route_index"


async def _gb120_base_query(query: Query, context: ContextSnapshot) -> Response:
...
#   2. Rename its entrypoint `query` -> `_gb120_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _gb120_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _Gb120_TAG below to a value unique to this agent.
#
# TUNE: _Gb120_SKIP_AFTER_S = 245.0 assumes a ~300s wall. Lower it if the host
...
import asyncio as _gb120_asyncio
import json as _gb120_json
import re as _gb120_re
from time import monotonic as _gb120_monotonic

from harnyx_miner_sdk.api import fetch_page as _gb120_fetch_page
from harnyx_miner_sdk.api import llm_chat as _gb120_llm_chat
from harnyx_miner_sdk.api import search_web as _gb120_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as __Gb120Cite
from harnyx_miner_sdk.query import CitationSlice as __Gb120Cite
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_Gb120_PROVIDER = "openrouter"
_Gb120_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_Gb120_SEARCH_PROVIDERS = ("parallel",)
_Gb120_CHAT_TIMEOUT_S = 12.0
_Gb120_SEARCH_TIMEOUT_S = 12.0
_Gb120_FETCH_TIMEOUT_S = 14.0
_Gb120_SKIP_AFTER_S = 245.0
_Gb120_ANSWER_CAP = 60000
_Gb120_NOTE_CAP = 8000
_Gb120_MAX_CITES = 32
_Gb120_FENCE_RE = _gb120_re.compile(r"^```(?:json)?\s*|\s*```$", _gb120_re.I | _gb120_re.M)
_Gb120_POINTER_RE = _gb120_re.compile(r"\[\[(\d+)\]\]")


def _gb120_parse_json(text):
    if not text:
        return None
    body = _Gb120_FENCE_RE.sub("", text).strip()
    try:
        value = _gb120_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _gb120_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _gb120_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _gb120_choice_text(content):
    if isinstance(content, str):
        return content
...
def _gb120_chat_text(payload):
    ...
    return _gb120_choice_text(getattr(message, "content", None)).strip()


async def _gb120_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _Gb120_MODELS:
        try:
            payload = await _gb120_asyncio.wait_for(
                _gb120_llm_chat(
                    provider=_Gb120_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _gb120_chat_text(payload)
        if text:
            return text
    return ""


async def _gb120_search(query_text):
    for provider in _Gb120_SEARCH_PROVIDERS:
        try:
            payload = await _gb120_asyncio.wait_for(
                _gb120_search_web(provider=provider, query=query_text),
                timeout=_Gb120_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _gb120_fetch(url):
    try:
        return await _gb120_asyncio.wait_for(
            _gb120_fetch_page(provider="parallel", url=url),
            timeout=_Gb120_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _gb120_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _gb120_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        ...
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(__Gb120Cite(start=start, end=end))
        out.append(__Gb120Cite(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _Gb120_MAX_CITES:
            break
    return out


def _gb120_merge_row(citations, row):
    for position, ref in enumerate(citations):
        ...
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _Gb120_MAX_CITES:
        return None
    note = row["note"]
    citations.append(__Gb120Cite(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[__Gb120Cite(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _gb120_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _gb120_merge_row(citations, row)
        if pointer is None:
            continue
...
def _gb120_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_Gb120_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _gb120_json.dumps(output)[:_Gb120_ANSWER_CAP]
    except Exception:
        return str(output)[:_Gb120_ANSWER_CAP]


def _gb120_normalize_pointers(text, n_cites):
    if not isinstance(text, str):
        return text
    ...
    return _Gb120_POINTER_RE.sub(_keep, text)


def _gb120_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_gb120_normalize_pointers(text, n)[:_Gb120_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
...
def _gb120_pointer_only(response):
    citations = _gb120_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_gb120_normalize_pointers(text, len(citations))[:_Gb120_ANSWER_CAP],
            citations=citations,
        )
    return response
...
_Gb120_POOL_ROUNDS = 2


def _gb120_pool_short(state):
...
async def _gb120_assess(question, draft):
    parsed = _gb120_parse_json(await _gb120_chat(
        _Gb120_POOL_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:10000],
        700,
        _Gb120_CHAT_TIMEOUT_S,
    ))
    if parsed is None:
    ...
    return {
        ...
        "probes": _gb120_str_list(parsed.get("probes"), 2),
    }


async def _gb120_widen(state, question):
    probes = state.get("probes") or [question[:200]]
    payloads = await _gb120_asyncio.gather(
        *[_gb120_search(probe) for probe in probes[:2]],
    ...
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_gb120_rows_from_payload(payload, "widen"))
    return rows[:14]


async def _gb120_render(question, draft, rows, citations):
    board = _gb120_board(rows, citations)
    if not board:
        return None
    system = (
    ...
    parsed = _gb120_parse_json(await _gb120_chat(system, user, 2400, 26.0))
    if parsed is None:
        return None
...
async def _gb121_base_query(query: Query, context=None) -> Response:
    started = _gb120_monotonic()
    draft = await _gb120_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        citations = _gb120_copy_citations(draft)
        body = _gb120_draft_text(draft)
        improved = None
        for _round in range(_Gb120_POOL_ROUNDS):
            if _gb120_monotonic() - started >= _Gb120_SKIP_AFTER_S:
                break
            state = await _gb120_assess(question, body)
            if not _gb120_pool_short(state):
                break
            rows = await _gb120_widen(state, question)
            if not rows:
                break
            text = await _gb120_render(question, body, rows, citations)
            if text is None or text == body:
                break
            body = text
            improved = text
        if improved is not None:
            return _gb120_rebuild(draft, improved, None, citations)
        return _gb120_pointer_only(draft)
    except Exception:
        return draft


# ============================================================================
# SN67 OVERLAY FAMILY F3 -- conflict adjudication
# ----------------------------------------------------------------------------
...
# GRAFT IN 4 STEPS
#   1. Take the host agent file.
#   2. Rename its entrypoint `query` -> `_gb121_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _gb121_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _Gb121_TAG below to a value unique to this agent.
#
# TUNE: _Gb121_SKIP_AFTER_S = 252.0 assumes a ~300s wall. Lower it if the host
# lane is slow, or you will hit session_budget_exhausted.
#
# query.fast is deliberately skipped (except F8, which returns base directly).
# Fast is scored correctness-only F1, so extra components lower precision.
...
import asyncio as _gb121_asyncio
import json as _gb121_json
import re as _gb121_re
from time import monotonic as _gb121_monotonic

from harnyx_miner_sdk.api import fetch_page as _gb121_fetch_page
from harnyx_miner_sdk.api import llm_chat as _gb121_llm_chat
from harnyx_miner_sdk.api import search_web as _gb121_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as __Gb121Cite
from harnyx_miner_sdk.query import CitationSlice as __Gb121Cite
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_Gb121_PROVIDER = "openrouter"
_Gb121_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_Gb121_SEARCH_PROVIDERS = ("parallel",)
_Gb121_CHAT_TIMEOUT_S = 12.0
_Gb121_SEARCH_TIMEOUT_S = 12.0
_Gb121_FETCH_TIMEOUT_S = 14.0
_Gb121_SKIP_AFTER_S = 252.0
_Gb121_ANSWER_CAP = 60000
_Gb121_NOTE_CAP = 8000
_Gb121_MAX_CITES = 32
_Gb121_FENCE_RE = _gb121_re.compile(r"^```(?:json)?\s*|\s*```$", _gb121_re.I | _gb121_re.M)
_Gb121_POINTER_RE = _gb121_re.compile(r"\[\[(\d+)\]\]")


def _gb121_parse_json(text):
    if not text:
        return None
    body = _Gb121_FENCE_RE.sub("", text).strip()
    try:
        value = _gb121_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _gb121_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _gb121_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _gb121_choice_text(content):
    if isinstance(content, str):
        return content
    if isinstance(content, list):
        parts = []
...
def _gb121_chat_text(payload):
    ...
    if not choices:
        return ""
    message = getattr(choices[0], "message", None)
    return _gb121_choice_text(getattr(message, "content", None)).strip()


async def _gb121_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _Gb121_MODELS:
        try:
            payload = await _gb121_asyncio.wait_for(
                _gb121_llm_chat(
                    provider=_Gb121_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _gb121_chat_text(payload)
        if text:
            return text
    return ""


async def _gb121_search(query_text):
    for provider in _Gb121_SEARCH_PROVIDERS:
        try:
            payload = await _gb121_asyncio.wait_for(
                _gb121_search_web(provider=provider, query=query_text),
                timeout=_Gb121_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _gb121_fetch(url):
    try:
        return await _gb121_asyncio.wait_for(
            _gb121_fetch_page(provider="parallel", url=url),
            timeout=_Gb121_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _gb121_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _gb121_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        slices = []
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(__Gb121Cite(start=start, end=end))
        out.append(__Gb121Cite(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _Gb121_MAX_CITES:
            break
    return out


def _gb121_merge_row(citations, row):
    for position, ref in enumerate(citations):
        same_receipt = str(getattr(ref, "receipt_id", "") or "") == row["receipt_id"]
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _Gb121_MAX_CITES:
        return None
    note = row["note"]
    citations.append(__Gb121Cite(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[__Gb121Cite(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _gb121_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _gb121_merge_row(citations, row)
        if pointer is None:
            continue
        lines.append("[[" + str(pointer) + "]] " + row["title"] + " :: " + row["note"][:1200])
    return "\n".join(lines)
...
def _gb121_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_Gb121_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _gb121_json.dumps(output)[:_Gb121_ANSWER_CAP]
    except Exception:
        return str(output)[:_Gb121_ANSWER_CAP]


def _gb121_normalize_pointers(text, n_cites):
    if not isinstance(text, str):
        return text
    ...
    return _Gb121_POINTER_RE.sub(_keep, text)


def _gb121_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_gb121_normalize_pointers(text, n)[:_Gb121_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
        return Response(output=output, citations=citations)
...
def _gb121_pointer_only(response):
    citations = _gb121_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_gb121_normalize_pointers(text, len(citations))[:_Gb121_ANSWER_CAP],
            citations=citations,
        )
    return response

_Gb121_AUDIT_SYSTEM = (
    "You audit a research draft for cross-source disagreement. Return JSON "
    "only with keys missing_elements (string array naming each claim whose "
    "sources conflict), unsupported_claims (string array of claims asserted "
    "without a reconciling source), pool_incomplete (boolean, true when a "
    "contradiction cannot be adjudicated from the evidence shown) and probes "
    "(string array of at most three search queries that would settle the "
    "disagreement). Judge only contradictions between retrieved sources: "
    "differing figures, dates, spellings, definitions or scopes for the same "
    "underlying fact. Ignore stylistic variation."
)


def _gb121_ledger_open(ledger):
    """The research-role gate: does an unadjudicated source conflict remain?"""
    if not isinstance(ledger, dict):
        return False
    if ledger.get("pool_incomplete") is True:
        return True
    conflicts = ledger.get("missing_elements") or []
    unsettled = ledger.get("unsupported_claims") or []
    return len(conflicts) > 0 or len(unsettled) > 0


async def _gb121_audit(question, draft):
    text = await _gb121_chat(
        _Gb121_AUDIT_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:12000],
        900,
        _Gb121_CHAT_TIMEOUT_S,
    )
    parsed = _gb121_parse_json(text)
    if parsed is None:
        return {}
    return {
        "missing_elements": _gb121_str_list(parsed.get("missing_elements"), 6),
        "unsupported_claims": _gb121_str_list(parsed.get("unsupported_claims"), 6),
        "pool_incomplete": parsed.get("pool_incomplete") is True,
        "probes": _gb121_str_list(parsed.get("probes"), 3),
    }


async def _gb121_retrieve(question, ledger):
    probes = ledger.get("probes") or []
    if not probes:
        probes = [question[:220]]
    payloads = await _gb121_asyncio.gather(
        *[_gb121_search(probe) for probe in probes[:3]],
        return_exceptions=True,
    )
    rows = []
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_gb121_rows_from_payload(payload, "probe"))
    return rows[:18]


async def _gb121_regenerate(question, draft, ledger, rows, citations):
    board = _gb121_board(rows, citations)
    if not board:
        return None
    system = (
        "Two or more sources disagree. Adjudicate each conflict from the "
        "numbered board: state the value the better-supported source gives, "
        "say briefly why that source wins, and keep the rejected value visible "
        "as a noted discrepancy. Cite with [[n]] pointers. Return JSON only "
        "with key text."
    )
    user = (
        "QUESTION:\n" + question
        + "\n\nOPEN ITEMS:\n" + _gb121_json.dumps(ledger.get("missing_elements") or [])
        + "\n\nDRAFT:\n" + draft[:8000]
        + "\n\nBOARD:\n" + board[:22000]
    )
    parsed = _gb121_parse_json(await _gb121_chat(system, user, 2400, 28.0))
    if parsed is None:
        return None
    text = parsed.get("text")
    if not isinstance(text, str) or len(text.strip()) < 8:
        return None
    return _gb121_rebuild(None, text.strip(), None, citations)


async def _zb12_entry(query: Query, context=None) -> Response:
    started = _gb121_monotonic()
    draft = await _gb121_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        if _gb121_monotonic() - started >= _Gb121_SKIP_AFTER_S:
            return _gb121_pointer_only(draft)
        citations = _gb121_copy_citations(draft)
        ledger = await _gb121_audit(question, _gb121_draft_text(draft))
        if _gb121_ledger_open(ledger):
            rows = await _gb121_retrieve(question, ledger)
            if rows:
                rewritten = await _gb121_regenerate(
                    question, _gb121_draft_text(draft), ledger, rows, citations
                )
...
try:
    from harnyx_miner_sdk.api import tooling_info as _zb12_tooling_info
except ImportError:
    _zb12_tooling_info = None


class _ZB12TimeBudget:
    def __init__(self, limit_seconds):
        self.limit_seconds = limit_seconds
        self.remaining_seconds = limit_seconds


class _ZB12CostBudget:
...
class _ZB12Context:
    ...
    def __init__(self, seconds, usd):
        self.time_budget = _ZB12TimeBudget(seconds)
        self.cost_budget = _ZB12CostBudget(usd)


def _zb12_fields(info):
...
def _zb12_pick(fields, names, fallback):
    for name in names:
...
async def _zb12_context():
    seconds = 300.0
    usd = 0.5
    if _zb12_tooling_info is not None:
        try:
            fields = _zb12_fields(await _zb12_tooling_info())
            seconds = _zb12_pick(
                fields,
            ...
            pass
    return _ZB12Context(seconds, usd)


@entrypoint("query")
async def query(query: Query) -> Response:
    try:
        context = await _zb12_context()
    except Exception:
        context = _ZB12Context(300.0, 0.5)
    return await _zb12_entry(query, context)
