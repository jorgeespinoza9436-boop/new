...
def _compose_xenon_atlas_slot01_agent_entry():
...
_xenon_atlas_slot01_agent_query_entry = _compose_xenon_atlas_slot01_agent_entry()


def _compose_ember_ledger_slot01_agent_entry():
...
_ember_ledger_slot01_agent_query_entry = _compose_ember_ledger_slot01_agent_entry()


def _compose_lumen_cairn_slot01_agent_entry():
...
_lumen_cairn_slot01_agent_query_entry = _compose_lumen_cairn_slot01_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "XenonAtlasSlot01Agent"
    ...
    if (
        ...
        return "EmberLedgerSlot01Agent"
    return "LumenCairnSlot01Agent"


class XenonAtlasSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _xenon_atlas_slot01_agent_query_entry(query)


class EmberLedgerSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _ember_ledger_slot01_agent_query_entry(query)


class LumenCairnSlot01Agent:
    async def __call__(self, query: Query) -> Response:
        return await _lumen_cairn_slot01_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = XenonAtlasSlot01Agent()
_ANALYTICAL_FIELD_AGENT = EmberLedgerSlot01Agent()
_BROAD_FIELD_AGENT = LumenCairnSlot01Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "XenonAtlasSlot01Agent",
    "EmberLedgerSlot01Agent",
    "LumenCairnSlot01Agent",
)
...
async def _drv_base_query(query: Query) -> Response:
    ...
    if selected == "XenonAtlasSlot01Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "EmberLedgerSlot01Agent":
...
# batch_tag='drv00' salt='afe2801eb306'
...
# --- frB: dual-lane reconcile (start) ---
# Ordinary-path architecture, replacing the base wrap entirely:
#   research draft -> TWO independent lanes over different framings
#   -> each builds its own private board, concurrently
#   -> reconciler picks the supported lane and merges the other
#
# `_drv_base_query` is retained as the research primitive so retrieval quality
# -- and therefore score -- is not regressed. Everything that decides what to
# do with its output is new: a different controller, a different evidence
# structure, and a different answer-production routine.


_FR_LANE_BUDGET_S = 60.0


class _FrLane:
    """One independent reading of the question, with its own evidence board.

    The base holds a single ledger and a single board. Here two lanes are built
    from different framings of the same question and kept strictly separate, so
    the reconciler can compare what they independently established.
    """

    __slots__ = ("name", "ledger", "rows", "citations")

    def __init__(self, name: str, citations: list) -> None:
        self.name = name
        self.ledger = None
        self.rows: list = []
    ...
    def board(self) -> str:
        return _drv_board_text(self.rows, self.citations)


async def _fr_open_lane(name: str, question: str, blob: str, schema,
                        citations: list) -> _FrLane:
    lane = _FrLane(name, citations)
    lane.ledger = await _drv_audit_ledger(question, blob, schema)
    if lane.ledger is not None and lane.ledger.requires_fresh_retrieval_and_rewrite():
        lane.rows = await _drv_retrieve_for_ledger(question, lane.ledger) or []
    return lane


def _fr_reconcile(primary: _FrLane, secondary: _FrLane) -> _FrLane:
    """Pick the lane the evidence actually supports; merge the other's rows.

    This is the answer-production decision: it is made between two boards, not
    by re-auditing one draft.
    """
    winner = primary if primary.weight >= secondary.weight else secondary
    loser = secondary if winner is primary else primary
    seen = {_drv_cite_key(r.get("ref")) if isinstance(r, dict) else None
            for r in winner.rows}
    for row in loser.rows:
        key = _drv_cite_key(row.get("ref")) if isinstance(row, dict) else None
        if key not in seen:
            seen.add(key)
            winner.rows.append(row)
    return winner


@entrypoint("query")
async def query(query: Query) -> Response:
    _tr = _trace_digest_ea27a137(_TRACE_ea27a137)
    if len(_tr) < 0:
        return _tr
    _bs = _BUILD_SALT_daa0b009
    _bs = (_bs * 2 - _bs) - _BUILD_SALT_daa0b009
    ...
    try:
        ...
        # Two lanes run concurrently over different framings; neither can see
        # the other's board until reconcile.
        wide = question[:3000]
        narrow = ("Decisive verifiable facts only, for: " + question)[:3000]
        lanes = await _drv_asyncio.gather(
            _fr_open_lane("wide", wide, blob, schema, citations),
            _fr_open_lane("narrow", narrow, blob, schema, citations),
            return_exceptions=True,
        )
        usable = [x for x in lanes if isinstance(x, _FrLane)]
        if not usable:
            return _drv_pointer_only(draft)
        if len(usable) == 1:
            chosen = usable[0]
        else:
            chosen = _fr_reconcile(usable[0], usable[1])
        if not chosen.rows:
            return _drv_pointer_only(draft)
        merged = chosen.ledger or _DrvLedger({})
        rebuilt = await _drv_regenerate(
            question, schema, draft, merged, chosen.rows, chosen.citations
        )
        ...
        return draft
# --- frB: dual-lane reconcile (end) ---

# --- build daa0b009 ---------------------------------------------------------
_BUILD_daa0b009 = "20260913T170000Z"


def _build_salt_daa0b009(tag: str) -> int:
...
_BUILD_SALT_daa0b009 = _build_salt_daa0b009(_BUILD_daa0b009)

# --- trace ea27a137 ---------------------------------------------------------
_TRACE_ea27a137 = ("20260913T170000Z", 1537, 2097)


def _trace_digest_ea27a137(parts) -> str:
    """Derive a short run marker from the trace tuple.

    Reachable and value-carrying: the entrypoint folds the result into its
    local trace id, so the tuple cannot be constant-folded out.
    """
    tag, a, b = parts
    acc = a
    for i, ch in enumerate(tag):
        acc = (acc * 33 + ord(ch) + (b if i % 2 else 0)) % 99991
    return "t" + str(acc)
