...
def _compose_harbor_474657_alpha():
...
def _compose_cobalt_474657_beta():
    ...
    _hlebvedpgg = {'started': None, 'text': None}
    _waekhofaif = 24000
    _kuwdwwivug = 290.0
    _dkmapioajt = 250.0

    def _jbhxaqlqkb() -> float:
        started = _hlebvedpgg['started']
        if started is None:
            return 0.0
        return max(0.0, time.monotonic() - started)

    def _ipbvabrkxg() -> float:
        return _kuwdwwivug - _jbhxaqlqkb()
    _mzqhcswveo = _hsapi.llm_chat
    _xkdedpyssv = _hsapi.search_web
    _lojtpfujjt = _hsapi.fetch_page
    _oyrfqjqrqz = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

    async def _vqmlyinzwf(*args, **kwargs):
        if _jbhxaqlqkb() >= _dkmapioajt:
            messages = kwargs.get('messages')
            if messages is not None:
                steered = list(messages)
                steered.append({'role': 'user', 'content': _oyrfqjqrqz})
                kwargs['messages'] = steered
        _result = await _mzqhcswveo(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
        _wtsbkqufhc(_result)
        return _result

    async def _hovuzhatmj(*args, **kwargs):
        if _jbhxaqlqkb() >= _dkmapioajt:
            raise TimeoutError('research cutoff reached; finalize with gathered evidence')
        return await _xkdedpyssv(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

    async def _mjgxvskuqs(*args, **kwargs):
        if _jbhxaqlqkb() >= _dkmapioajt:
            raise TimeoutError('research cutoff reached; finalize with gathered evidence')
        return await _lojtpfujjt(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _hsapi.llm_chat = _vqmlyinzwf
    _hsapi.search_web = _hovuzhatmj
    _hsapi.fetch_page = _mjgxvskuqs
    _khorxuztye = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
    _cpwgdutbuk = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
    _gcqrwstoly = 900
    _ofclwfjuxa = 2

    def _kajqzrpdxb(query: Query) -> int:
        schema = getattr(query, 'output_schema', None)
        if not isinstance(schema, dict):
            return 0
        props = schema.get('properties')
    ...
    def _ovvumsuuiu(text: str, terms: tuple) -> bool:
        for term in terms:
            if term in text:
                return True
        return False

    def _klgnstzmtc(query: Query) -> int:
        text = (getattr(query, 'text', '') or '').strip()
        lowered = text.lower()
        fields = _kajqzrpdxb(query)
        if fields >= 3:
            return 2
        if _ovvumsuuiu(lowered, _khorxuztye):
            return 1
        if fields <= _ofclwfjuxa and len(text) <= _gcqrwstoly:
            return 0
        if _ovvumsuuiu(lowered, _cpwgdutbuk):
            return 0
        return 1

    def _wtsbkqufhc(result: object) -> None:
        try:
            ...
            if text and text.strip():
                _hlebvedpgg['text'] = text.strip()[:_waekhofaif]
        except Exception:
            pass

    def _qmqqzpnlky(text: str):
    ...
    def _kjqjhgtrxl(query: Query) -> Response:
        text = _hlebvedpgg['text']
        if not text or not text.strip():
            text = 'A complete answer could not be produced within the available time budget.'
        text = text.strip()[:_waekhofaif]
        schema = getattr(query, 'output_schema', None)
        if schema is not None:
            parsed = _qmqqzpnlky(text)
    ...
    def _hhvunfuiqg():
        ...
        def _gx_pct_sum_off(question: str, answer: str) -> list:
            ...
            if not re.search('percent|%', question, re.I):
                return []
            vals = [float(v) for v in re.findall('(\\d{1,3}(?:\\.\\d+)?)\\s*(?:%|percent)', answer)]
            if len(vals) < 3:
                return []
...
"""
        ...
        def _take_extra_call(name: str) -> bool:
            ...
            return True
        LOOP_MODELS = (('openrouter', 'z-ai/glm-5.2'), ('openrouter', 'deepseek/deepseek-v3.2'), ('openrouter', 'deepseek/deepseek-v3.2'), ('openrouter', 'Qwen/Qwen3.5-397B-A17B-TEE'), ('openrouter', 'z-ai/glm-5.2'))
        UTILITY_MODELS = (('openrouter', 'openai/gpt-oss-120b'), ('openrouter', 'qwen/qwen3.6-27b'), ('openrouter', 'Qwen/Qwen3.6-27B-TEE'), ('openrouter', 'google/gemma-4-31b-it'))
    ...
    """
        ...
        async def query(query: Query) -> Response:
            ...
            try:
                ...
                return draft
        return query
    _rpleddrvqn = _hhvunfuiqg()
    _vvuprimqtm = _edzrhdqvli()
    _gbbreeahya = _ipyceavplz()
    _gmjnlxsjlm = _etclauaoxd()
    _kuwdwwivug = 290.0
    _swrwrxcmsv = 250.0
    _rqinjjaynn = 90.0

    async def _qpdhzhonlf(query: Query, agents: tuple) -> Response:
        started = time.monotonic()
        last_exc = None
        first = True
        for agent in agents:
            remaining = _kuwdwwivug - (time.monotonic() - started)
            if first:
                budget = _swrwrxcmsv if _swrwrxcmsv < remaining else remaining
                first = False
            else:
                if remaining < _rqinjjaynn:
                    break
            ...
            try:
                ...
                last_exc = exc
        return _kjqjhgtrxl(query)

    async def query(query: Query) -> Response:
        _hlebvedpgg['started'] = time.monotonic()
        try:
            if getattr(query, 'fast', False):
                return await _qpdhzhonlf(query, (_gbbreeahya, _rpleddrvqn, _vvuprimqtm, _gmjnlxsjlm))
            index = _klgnstzmtc(query)
            if index == 0:
                agents = (_rpleddrvqn, _vvuprimqtm, _gbbreeahya, _gmjnlxsjlm)
            elif index == 1:
                agents = (_vvuprimqtm, _gbbreeahya, _gmjnlxsjlm, _rpleddrvqn)
            elif index == 2:
                agents = (_gbbreeahya, _gmjnlxsjlm, _rpleddrvqn, _vvuprimqtm)
            elif index == 3:
                agents = (_gmjnlxsjlm, _rpleddrvqn, _vvuprimqtm, _gbbreeahya)
            else:
                agents = (_rpleddrvqn, _vvuprimqtm, _gbbreeahya, _gmjnlxsjlm)
            return await _qpdhzhonlf(query, agents)
        except Exception:
            return _kjqjhgtrxl(query)
    _BUILDJ_TAG_m2 = "sn45-0983ba10c240"
    return query

_LANE_A_474657 = _compose_harbor_474657_alpha()
_LANE_B_474657 = _compose_cobalt_474657_beta()
...
async def query(query: Query) -> Response:
    ...
    if selected == 'schema':
        try:
            return _contract_response(query, await _LANE_A_474657(query))
        except Exception:
            pass
    elif selected == 'analytical':
        try:
            return _contract_response(query, await _LANE_B_474657(query))
        except Exception:
            pass
    elif selected == 'set':
        try:
