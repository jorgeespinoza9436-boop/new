...
_ftsjeqmltp = {'started': None, 'text': None}
_oiauxajqnd = 24000
_nybeaobuno = 290.0
_bbapnjxppj = 250.0

def _smtthbniuh() -> float:
    started = _ftsjeqmltp['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _aznhfrslni() -> float:
    return _nybeaobuno - _smtthbniuh()
_wyanilyaoa = _hsapi.llm_chat
_udjhxjuftn = _hsapi.search_web
_djvedtmskp = _hsapi.fetch_page
_iabomplfff = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _zpwsnpbqlq(*args, **kwargs):
    if _smtthbniuh() >= _bbapnjxppj:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _iabomplfff})
            kwargs['messages'] = steered
    _result = await _wyanilyaoa(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ygyujibgkt(_result)
    return _result

async def _hyqljvlhos(*args, **kwargs):
    if _smtthbniuh() >= _bbapnjxppj:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _udjhxjuftn(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _ixaptcqrtt(*args, **kwargs):
    if _smtthbniuh() >= _bbapnjxppj:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _djvedtmskp(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _zpwsnpbqlq
_hsapi.search_web = _hyqljvlhos
_hsapi.fetch_page = _ixaptcqrtt
_qcrtdkatnl = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_gvevumshvj = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_jtfgrizjdp = 900
_gruljzmizk = 2

def _rlyjrmaxlm(query: Query) -> int:
...
def _ysugmswpng(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _rycrfhknzy(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _rlyjrmaxlm(query)
    if fields >= 3:
        return 2
    if _ysugmswpng(lowered, _qcrtdkatnl):
        return 1
    if fields <= _gruljzmizk and len(text) <= _jtfgrizjdp:
        return 0
    if _ysugmswpng(lowered, _gvevumshvj):
        return 0
    return 1

def _ygyujibgkt(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _ftsjeqmltp['text'] = text.strip()[:_oiauxajqnd]
    except Exception:
        pass

def _rrkijmntbt(text: str):
...
def _beyxgpjcto(query: Query) -> Response:
    text = _ftsjeqmltp['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_oiauxajqnd]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rrkijmntbt(text)
...
"""
    ...
    """
        ...
        return response
    return query
_ziuttcwelv = _pzmcbmgiwh()
_ybaiufjdmh = _twswlifwve()
_bnxknmatxl = _yejufbofga()
_nybeaobuno = 290.0
_zyqxhlqcrs = 250.0
_grgiyokynl = 90.0

async def _xevfugolqv(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _nybeaobuno - (time.monotonic() - started)
        if first:
            budget = _zyqxhlqcrs if _zyqxhlqcrs < remaining else remaining
            first = False
        else:
            if remaining < _grgiyokynl:
                break
        ...
        try:
            ...
            last_exc = exc
    return _beyxgpjcto(query)
_taxbbpjyow = 235.0

async def _veoqnukgzl(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _ryqpsdsiio(query, agents):
    if len(agents) < 2:
        return await _xevfugolqv(query, agents)
    r0, r1 = await asyncio.gather(_veoqnukgzl(agents[0], query, _taxbbpjyow), _veoqnukgzl(agents[1], query, _taxbbpjyow))
    ok0 = isinstance(r0, Response) and _owoqbnzwgw(r0) != ''
    ok1 = isinstance(r1, Response) and _owoqbnzwgw(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_wyanilyaoa(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
async def _drv_base_query(query: Query) -> Response:
    _ftsjeqmltp['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _xevfugolqv(query, (_bnxknmatxl, _ziuttcwelv, _ybaiufjdmh))
        index = _rycrfhknzy(query)
        if index == 0:
            agents = (_ziuttcwelv, _ybaiufjdmh, _bnxknmatxl)
        elif index == 1:
            agents = (_ybaiufjdmh, _bnxknmatxl, _ziuttcwelv)
        elif index == 2:
            agents = (_bnxknmatxl, _ziuttcwelv, _ybaiufjdmh)
        else:
            agents = (_ziuttcwelv, _ybaiufjdmh, _bnxknmatxl)
        return await _ryqpsdsiio(query, agents)
    except Exception:
        return _beyxgpjcto(query)
_BUILDJ_TAG_m15 = "sn45-27a49441d7b3"

# --- drv wrap: claim-conflict ledger (start) ---
# batch_tag='drv121' salt='1f17db79cc35'
