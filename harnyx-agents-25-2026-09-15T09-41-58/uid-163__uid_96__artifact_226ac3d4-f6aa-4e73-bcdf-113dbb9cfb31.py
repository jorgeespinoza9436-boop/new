...
async def _a04_base_query(query: Query, context: ContextSnapshot) -> Response:
...
#   2. Rename its entrypoint `query` -> `_a04_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _a04_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _A04_TAG below to a value unique to this agent.
#
# TUNE: _A04_SKIP_AFTER_S = 252.0 assumes a ~300s wall. Lower it if the host
...
_A04_TAG = "F1-CHANGE-ME"

import asyncio as _a04_asyncio
import json as _a04_json
import re as _a04_re
from time import monotonic as _a04_monotonic

from harnyx_miner_sdk.api import fetch_page as _a04_fetch_page
from harnyx_miner_sdk.api import llm_chat as _a04_llm_chat
from harnyx_miner_sdk.api import search_web as _a04_search_web
from harnyx_miner_sdk.decorators import entrypoint
...
_A04_PROVIDER = "openrouter"
_A04_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_A04_SEARCH_PROVIDERS = ("parallel", "exa")
_A04_CHAT_TIMEOUT_S = 12.0
_A04_SEARCH_TIMEOUT_S = 12.0
_A04_FETCH_TIMEOUT_S = 14.0
_A04_SKIP_AFTER_S = 252.0
_A04_ANSWER_CAP = 60000
_A04_NOTE_CAP = 8000
_A04_MAX_CITES = 32
_A04_FENCE_RE = _a04_re.compile(r"^```(?:json)?\s*|\s*```$", _a04_re.I | _a04_re.M)
_A04_POINTER_RE = _a04_re.compile(r"\[\[(\d+)\]\]")


def _a04_parse_json(text):
    if not text:
        return None
    body = _A04_FENCE_RE.sub("", text).strip()
    try:
        value = _a04_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _a04_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _a04_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _a04_chat_text(payload):
    ...
    return _a04_choice_text(getattr(message, "content", None)).strip()


async def _a04_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _A04_MODELS:
        try:
            payload = await _a04_asyncio.wait_for(
                _a04_llm_chat(
                    provider=_A04_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _a04_chat_text(payload)
        if text:
            return text
    return ""


async def _a04_search(query_text):
    for provider in _A04_SEARCH_PROVIDERS:
        try:
            payload = await _a04_asyncio.wait_for(
                _a04_search_web(provider=provider, query=query_text),
                timeout=_A04_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _a04_fetch(url):
    try:
        return await _a04_asyncio.wait_for(
            _a04_fetch_page(provider="parallel", url=url),
            timeout=_A04_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _a04_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _a04_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        ...
        if len(out) >= _A04_MAX_CITES:
            break
    return out


def _a04_merge_row(citations, row):
    for position, ref in enumerate(citations):
        ...
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _A04_MAX_CITES:
        return None
    note = row["note"]
...
def _a04_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _a04_merge_row(citations, row)
        if pointer is None:
            continue
...
def _a04_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_A04_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _a04_json.dumps(output)[:_A04_ANSWER_CAP]
    except Exception:
        return str(output)[:_A04_ANSWER_CAP]


def _a04_normalize_pointers(text, n_cites):
    ...
    return _A04_POINTER_RE.sub(_keep, text)


def _a04_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_a04_normalize_pointers(text, n)[:_A04_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
...
def _a04_pointer_only(response):
    citations = _a04_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_a04_normalize_pointers(text, len(citations))[:_A04_ANSWER_CAP],
            citations=citations,
        )
    return response
...
async def _a04_audit(question, draft):
    text = await _a04_chat(
        _A04_AUDIT_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:12000],
        900,
        _A04_CHAT_TIMEOUT_S,
    )
    parsed = _a04_parse_json(text)
    if parsed is None:
        return {}
    return {
        "missing_elements": _a04_str_list(parsed.get("missing_elements"), 6),
        "unsupported_claims": _a04_str_list(parsed.get("unsupported_claims"), 6),
        "pool_incomplete": parsed.get("pool_incomplete") is True,
        "probes": _a04_str_list(parsed.get("probes"), 3),
    }


async def _a04_retrieve(question, ledger):
    probes = ledger.get("probes") or []
    if not probes:
        probes = [question[:220]]
    payloads = await _a04_asyncio.gather(
        *[_a04_search(probe) for probe in probes[:3]],
    ...
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_a04_rows_from_payload(payload, "probe"))
    return rows[:18]


async def _a04_regenerate(question, draft, ledger, rows, citations):
    board = _a04_board(rows, citations)
    if not board:
        return None
    system = (
    ...
    user = (
        "QUESTION:\n" + question
        + "\n\nOPEN ITEMS:\n" + _a04_json.dumps(ledger.get("missing_elements") or [])
        + "\n\nDRAFT:\n" + draft[:8000]
        + "\n\nBOARD:\n" + board[:22000]
    )
    parsed = _a04_parse_json(await _a04_chat(system, user, 2400, 28.0))
    if parsed is None:
        return None
    text = parsed.get("text")
    if not isinstance(text, str) or len(text.strip()) < 8:
        return None
    return _a04_rebuild(None, text.strip(), None, citations)


@entrypoint("query")
async def query(query: Query, context: ContextSnapshot) -> Response:
    started = _a04_monotonic()
    draft = await _a04_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        if _a04_monotonic() - started >= _A04_SKIP_AFTER_S:
            return _a04_pointer_only(draft)
        citations = _a04_copy_citations(draft)
        ledger = await _a04_audit(question, _a04_draft_text(draft))
        if _a04_ledger_open(ledger):
            rows = await _a04_retrieve(question, ledger)
            if rows:
                rewritten = await _a04_regenerate(
                    question, _a04_draft_text(draft), ledger, rows, citations
                )
