...
async def _a23_base_query(query: Query, context: ContextSnapshot) -> Response:
...
#   2. Rename its entrypoint `query` -> `_a23_base_query` and DELETE its
#      @entrypoint("query") decorator. Result:
#         async def _a23_base_query(query: Query) -> Response
#   3. Paste this whole block at the END of the host file.
#   4. Change _A23_TAG below to a value unique to this agent.
#
# TUNE: _A23_SKIP_AFTER_S = 245.0 assumes a ~300s wall. Lower it if the host
...
_A23_TAG = "5F-CHANGE-ME"

import asyncio as _a23_asyncio
import json as _a23_json
import re as _a23_re
from time import monotonic as _a23_monotonic

from harnyx_miner_sdk.api import fetch_page as _a23_fetch_page
from harnyx_miner_sdk.api import llm_chat as _a23_llm_chat
from harnyx_miner_sdk.api import search_web as _a23_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as _5FCitationRef
from harnyx_miner_sdk.query import CitationSlice as _5FCitationSlice
from harnyx_miner_sdk.query import Query, Response

# openrouter only -- no ai_gateway, no chutes, no custom-openai-compatible
_A23_PROVIDER = "openrouter"
_A23_MODELS = ("z-ai/glm-5.2", "openai/gpt-oss-120b", "deepseek/deepseek-v3.2")
_A23_SEARCH_PROVIDERS = ("parallel", "exa")
_A23_CHAT_TIMEOUT_S = 12.0
_A23_SEARCH_TIMEOUT_S = 12.0
_A23_FETCH_TIMEOUT_S = 14.0
_A23_SKIP_AFTER_S = 245.0
_A23_ANSWER_CAP = 60000
_A23_NOTE_CAP = 8000
_A23_MAX_CITES = 32
_A23_FENCE_RE = _a23_re.compile(r"^```(?:json)?\s*|\s*```$", _a23_re.I | _a23_re.M)
_A23_POINTER_RE = _a23_re.compile(r"\[\[(\d+)\]\]")


def _a23_parse_json(text):
    if not text:
        return None
    body = _A23_FENCE_RE.sub("", text).strip()
    try:
        value = _a23_json.loads(body)
    except ValueError:
        start = body.find("{")
        end = body.rfind("}")
        if start < 0 or end <= start:
            return None
        try:
            value = _a23_json.loads(body[start:end + 1])
        except ValueError:
            return None
    return value if isinstance(value, dict) else None


def _a23_str_list(value, cap):
    if not isinstance(value, list):
        return []
    out = []
    for item in value:
...
def _a23_chat_text(payload):
    ...
    return _a23_choice_text(getattr(message, "content", None)).strip()


async def _a23_chat(system, user, max_tokens, timeout):
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": user},
    ]
    for model in _A23_MODELS:
        try:
            payload = await _a23_asyncio.wait_for(
                _a23_llm_chat(
                    provider=_A23_PROVIDER,
                    model=model,
                    messages=messages,
                    max_output_tokens=max_tokens,
                ),
            ...
            continue
        text = _a23_chat_text(payload)
        if text:
            return text
    return ""


async def _a23_search(query_text):
    for provider in _A23_SEARCH_PROVIDERS:
        try:
            payload = await _a23_asyncio.wait_for(
                _a23_search_web(provider=provider, query=query_text),
                timeout=_A23_SEARCH_TIMEOUT_S,
            )
        except Exception:
            continue
        if payload is not None and getattr(payload, "results", None):
...
async def _a23_fetch(url):
    try:
        return await _a23_asyncio.wait_for(
            _a23_fetch_page(provider="parallel", url=url),
            timeout=_A23_FETCH_TIMEOUT_S,
        )
    except Exception:
        return None


def _a23_rows_from_payload(payload, corpus):
    if payload is None:
        return []
    receipt = str(getattr(payload, "receipt_id", "") or "")
    rows = []
...
def _a23_copy_citations(response):
    out = []
    for ref in getattr(response, "citations", None) or ():
        ...
        for slc in getattr(ref, "slices", None) or ():
            ...
            if end > start:
                slices.append(_5FCitationSlice(start=start, end=end))
        out.append(_5FCitationRef(
            receipt_id=str(getattr(ref, "receipt_id", "") or ""),
            result_id=getattr(ref, "result_id", None),
            slices=slices,
        ))
        if len(out) >= _A23_MAX_CITES:
            break
    return out


def _a23_merge_row(citations, row):
    for position, ref in enumerate(citations):
        ...
        if same_receipt and getattr(ref, "result_id", None) == row["result_id"]:
            return position + 1
    if len(citations) >= _A23_MAX_CITES:
        return None
    note = row["note"]
    citations.append(_5FCitationRef(
        receipt_id=row["receipt_id"],
        result_id=row["result_id"],
        slices=[_5FCitationSlice(start=0, end=min(len(note), 1800))],
    ))
    return len(citations)


def _a23_board(rows, citations):
    lines = []
    for row in rows:
        pointer = _a23_merge_row(citations, row)
        if pointer is None:
            continue
...
def _a23_draft_text(response):
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return text[:_A23_ANSWER_CAP]
    output = getattr(response, "output", None)
    if output is None:
        return ""
    try:
        return _a23_json.dumps(output)[:_A23_ANSWER_CAP]
    except Exception:
        return str(output)[:_A23_ANSWER_CAP]


def _a23_normalize_pointers(text, n_cites):
    ...
    return _A23_POINTER_RE.sub(_keep, text)


def _a23_rebuild(response, text, output, citations):
    n = len(citations)
    if text is not None:
        return Response(
            text=_a23_normalize_pointers(text, n)[:_A23_ANSWER_CAP],
            citations=citations,
        )
    if output is not None:
...
def _a23_pointer_only(response):
    citations = _a23_copy_citations(response)
    text = getattr(response, "text", None)
    if isinstance(text, str) and text.strip():
        return Response(
            text=_a23_normalize_pointers(text, len(citations))[:_A23_ANSWER_CAP],
            citations=citations,
        )
    return response
...
async def _a23_assess(question, draft):
    parsed = _a23_parse_json(await _a23_chat(
        _A23_POOL_SYSTEM,
        "QUESTION:\n" + question + "\n\nDRAFT:\n" + draft[:10000],
        700,
        _A23_CHAT_TIMEOUT_S,
    ...
    return {
        ...
        "probes": _a23_str_list(parsed.get("probes"), 2),
    }


async def _a23_widen(state, question):
    probes = state.get("probes") or [question[:200]]
    payloads = await _a23_asyncio.gather(
        *[_a23_search(probe) for probe in probes[:2]],
    ...
    for payload in payloads:
        if isinstance(payload, Exception):
            continue
        rows.extend(_a23_rows_from_payload(payload, "widen"))
    return rows[:14]


async def _a23_render(question, draft, rows, citations):
    board = _a23_board(rows, citations)
    if not board:
        return None
    system = (
    ...
    parsed = _a23_parse_json(await _a23_chat(system, user, 2400, 26.0))
    if parsed is None:
        return None
...
async def query(query: Query, context: ContextSnapshot) -> Response:
    started = _a23_monotonic()
    draft = await _a23_base_query(query, context)
    if bool(getattr(query, "fast", False)):
        return draft
    question = str(getattr(query, "text", "") or "")
    try:
        citations = _a23_copy_citations(draft)
        body = _a23_draft_text(draft)
        improved = None
        for _round in range(_A23_POOL_ROUNDS):
            if _a23_monotonic() - started >= _A23_SKIP_AFTER_S:
                break
            state = await _a23_assess(question, body)
            if not _a23_pool_short(state):
                break
            rows = await _a23_widen(state, question)
            if not rows:
                break
            text = await _a23_render(question, body, rows, citations)
            if text is None or text == body:
                break
            body = text
            improved = text
        if improved is not None:
            return _a23_rebuild(draft, improved, None, citations)
        return _a23_pointer_only(draft)
    except Exception:
        return draft
