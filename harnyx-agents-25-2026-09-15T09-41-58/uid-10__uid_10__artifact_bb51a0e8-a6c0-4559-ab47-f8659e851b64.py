...
import re as _qual_re
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import Query, Response

_M1_FIG_RE = _qual_re.compile(r"\d[\d,]{1,}(?:\.\d+)?")
...
_qual_fast_lane = _compose_branch34534862_a_a95_entry()

def _compose_t1_score():
    ...
    def _sc1_directive_value(value, spec, key=""):
        """Apply only mechanical output directives stated by the JSON schema."""
        if not isinstance(spec, dict):
            return value
        if isinstance(value, dict):
            props = spec.get("properties") or {}
            return {name: _sc1_directive_value(item, props.get(name) or {}, name)
                    for name, item in value.items()}
        if isinstance(value, list):
            item_spec = spec.get("items") or {}
            out = [_sc1_directive_value(item, item_spec, key) for item in value]
            description = str(spec.get("description") or "").lower()
            if "alphabetical order" in description and all(isinstance(item, str) for item in out):
                out = sorted(out, key=lambda item: item.casefold())
            return out
        ...
        if "without the word 'county'" in description or 'without the word "county"' in description:
            out = re.sub(r"\s+County\b|\bCounty\s+", " ", out, flags=re.I).strip(" ,-")
        if "without its '( map )' link text" in description:
            out = re.sub(r"\s*\(\s*map\s*\)\s*$", "", out, flags=re.I).strip()

        # Geographic headings in source tables are commonly all caps. Their field
        # values are ordinary names unless the schema explicitly asks otherwise.
        if key.lower() in ("state", "county", "city") and out.isupper():
            out = out.title()
        if "ordinary title case" in description:
            out = out.title()

        if "digits only" in description or "plain integer string without separators or units" in description:
            match = re.fullmatch(r"\s*([+-]?[0-9][0-9, ]*)\s*(?:coins?)?\s*", out, re.I)
            if match:
                out = match.group(1).replace(",", "").replace(" ", "")

        if "four-digit calendar year" in description:
            years = set(re.findall(r"(?<!\d)(?:19|20)\d{2}(?!\d)", out))
            if len(years) == 1:
                out = years.pop()

        if "percentage with one decimal place and a percent sign" in description:
            match = re.fullmatch(r"\s*([+-]?\d+(?:\.\d+)?)\s*%?\s*", out)
            if match:
                out = f"{float(match.group(1)):.1f}%"
        elif "signed percentage with one decimal place" in description:
            match = re.fullmatch(r"\s*([+-]?\d+(?:\.\d+)?)\s*%?\s*", out)
            if match:
                number = float(match.group(1))
                out = f"{number:+.1f}%"
        elif "plain number with one decimal place" in description:
            match = re.fullmatch(r"\s*([+-]?\d+(?:\.\d+)?)\s*", out)
            if match:
                out = f"{float(match.group(1)):.1f}"
    ...
    def _sc1_schema_directives(value, schema):
        try:
            return _sc1_directive_value(value, schema)
        except Exception:
            return value
    ...
    def _schema_with_computed(structured, schema):
        ...
        if structured is None:
            return None
        try:
            return _s1_clamp(_sc1_schema_directives(structured, schema), schema)
        except Exception:
            return structured
    ...
    def _do_page_grep(url: str, pattern: str, ledger: EvidenceLedger) -> str:
        ...
        win = PAGE_GREP_WINDOW if total <= K4_GREP_NARROW_AT else K4_GREP_NARROW_WINDOW
        cursors = row.setdefault("_sc1_grep_cursors", {})
        cursor_key = pat.casefold()
        start_at = min(int(cursors.get(cursor_key) or 0), total)
        selected = centres[start_at:start_at + PAGE_GREP_MAX_HITS]
        cursors[cursor_key] = start_at + len(selected)
        out, spent = [], 0
        for c in selected:
            a = max(0, c - win // 2)
            b = min(len(text), a + win)
        ...
        shown_through = start_at + len(selected)
        remaining = max(0, total - shown_through)
        head = (f"# page_grep({pat!r}) on [{n}] -> {total} match(es) of {len(text)} chars"
                f"; showing page from match {start_at + 1}, {len(out)} region(s); "
                f"{remaining} occurrence(s) remain")
        if remaining:
            head += (". This is a SAMPLE, not the full set. Repeat this exact page_grep call "
                     "to advance to the next matches, or narrow the pattern before stating "
                     "a count or an exhaustive list.")
        if win < PAGE_GREP_WINDOW:
            head += (" Windows are narrowed because there are many matches: a row shown here "
                     "may be missing the COLUMN HEADER above it, so before reporting any "
                     "numeric column read_range around one match offset and confirm which "
    ...
    async def _t1_score_entry(query: Query) -> Response:
        question = (query.text or "").strip()
        if not question:
            return Response(text="No question provided.")
        try:
    ...
    return _t1_score_entry

_t1_score_lane = _compose_t1_score()

def _compose_t1_deep():
...
_t1_deep_lane = _compose_t1_deep()

def _qual_complex_fast(query: Query) -> bool:
    if getattr(query, "output_schema", None) is not None:
        return True
    text = " ".join(str(getattr(query, "text", "") or "").lower().split())
    words = set(_qual_re.findall(r"[a-z]+", text))
    if "how many" in text or "full pool" in text:
        return True
    return bool(words.intersection({
        "compare", "compared", "comparison", "elevation", "percent",
        "percentage", "ratio", "ranking", "ranked", "checkpoint",
        "count", "total", "sum", "average", "median", "maximum",
        "minimum", "difference",
    }))
...
async def query(query: Query) -> Response:
    try:
        if not bool(getattr(query, "fast", False)):
            return await _t1_deep_lane(query)
        if _qual_complex_fast(query):
            return await _t1_score_lane(query)
        return await _qual_fast_lane(query)
    except Exception:
        return Response(text="No verifiable source-backed answer was reached for this question.")
