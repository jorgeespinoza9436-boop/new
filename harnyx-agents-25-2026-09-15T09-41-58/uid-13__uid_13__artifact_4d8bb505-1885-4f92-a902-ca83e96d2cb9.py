...
def _compose_cedar_3eec61_alpha():
...
def _compose_opal_3eec61_beta():

    from harnyx_miner_sdk.decorators import entrypoint
    from harnyx_miner_sdk.query import Query, Response


    def _compose_indigo_orchid_slot14_agent_entry():
    ...
    _indigo_orchid_slot14_agent_query_entry = _compose_indigo_orchid_slot14_agent_entry()


    def _compose_opal_willow_slot14_agent_entry():
    ...
    _opal_willow_slot14_agent_query_entry = _compose_opal_willow_slot14_agent_entry()


    def _compose_lumen_relay_slot14_agent_entry():
    ...
    _lumen_relay_slot14_agent_query_entry = _compose_lumen_relay_slot14_agent_entry()


    def _task_shape_route_label(query: Query) -> str:
        if getattr(query, "output_schema", None) is not None:
            return "IndigoOrchidSlot14Agent"
        ...
        if (
            ...
            return "OpalWillowSlot14Agent"
        return "LumenRelaySlot14Agent"


    class IndigoOrchidSlot14Agent:
        async def __call__(self, query: Query) -> Response:
            return await _indigo_orchid_slot14_agent_query_entry(query)


    class OpalWillowSlot14Agent:
        async def __call__(self, query: Query) -> Response:
            return await _opal_willow_slot14_agent_query_entry(query)


    class LumenRelaySlot14Agent:
        async def __call__(self, query: Query) -> Response:
            return await _lumen_relay_slot14_agent_query_entry(query)


    _STRUCTURED_FIELD_AGENT = IndigoOrchidSlot14Agent()
    _ANALYTICAL_FIELD_AGENT = OpalWillowSlot14Agent()
    _BROAD_FIELD_AGENT = LumenRelaySlot14Agent()
    _CANDIDATE_BRANCH_CLASS_NAMES = (
        "IndigoOrchidSlot14Agent",
        "OpalWillowSlot14Agent",
        "LumenRelaySlot14Agent",
    )
    ...
    async def query(query: Query) -> Response:
        ...
        if selected == "IndigoOrchidSlot14Agent":
            return await _STRUCTURED_FIELD_AGENT(query)
        elif selected == "OpalWillowSlot14Agent":
            return await _ANALYTICAL_FIELD_AGENT(query)
        else:
            return await _BROAD_FIELD_AGENT(query)

    return query

_LANE_A_3EEC61 = _compose_cedar_3eec61_alpha()
_LANE_B_3EEC61 = _compose_opal_3eec61_beta()
...
async def query(query: Query) -> Response:
    ...
    if selected == 'schema':
        try:
            return _contract_response(query, await _LANE_B_3EEC61(query))
        except Exception:
            pass
        ...
        try:
            return _contract_response(query, await _LANE_A_3EEC61(query))
        except Exception:
            pass
    try:
        return _contract_response(query, await _LANE_A_3EEC61(query))
