...
def _compose_cedar_cairn_slot11_agent_entry():
...
_cedar_cairn_slot11_agent_query_entry = _compose_cedar_cairn_slot11_agent_entry()


def _compose_zephyr_forge_slot11_agent_entry():
...
_zephyr_forge_slot11_agent_query_entry = _compose_zephyr_forge_slot11_agent_entry()


def _compose_juniper_finch_slot11_agent_entry():
...
_juniper_finch_slot11_agent_query_entry = _compose_juniper_finch_slot11_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "CedarCairnSlot11Agent"
    ...
    if (
        ...
        return "ZephyrForgeSlot11Agent"
    return "JuniperFinchSlot11Agent"


class CedarCairnSlot11Agent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_cairn_slot11_agent_query_entry(query)


class ZephyrForgeSlot11Agent:
    async def __call__(self, query: Query) -> Response:
        return await _zephyr_forge_slot11_agent_query_entry(query)


class JuniperFinchSlot11Agent:
    async def __call__(self, query: Query) -> Response:
        return await _juniper_finch_slot11_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = CedarCairnSlot11Agent()
_ANALYTICAL_FIELD_AGENT = ZephyrForgeSlot11Agent()
_BROAD_FIELD_AGENT = JuniperFinchSlot11Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "CedarCairnSlot11Agent",
    "ZephyrForgeSlot11Agent",
    "JuniperFinchSlot11Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "CedarCairnSlot11Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "ZephyrForgeSlot11Agent":
