...
def _compose_raven_lattice_agent_entry():
...
_raven_lattice_agent_query_entry = _compose_raven_lattice_agent_entry()


def _compose_cedar_compass_agent_entry():
...
_cedar_compass_agent_query_entry = _compose_cedar_compass_agent_entry()
...
_SHAPE_ROUTER_SEED = "a42c5c9d7ec15f56b1bd49b9"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "RavenLatticeAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "CedarCompassAgent"
    if shape == 1:
...
class RavenLatticeAgent:
    async def __call__(self, query: Query) -> Response:
        return await _raven_lattice_agent_query_entry(query)


class CedarCompassAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cedar_compass_agent_query_entry(query)
...
_SHAPE_PRIMARY_AGENT = RavenLatticeAgent()
_SHAPE_SECONDARY_AGENT = CedarCompassAgent()
_SHAPE_TERTIARY_AGENT = HarborBeaconAgent()
