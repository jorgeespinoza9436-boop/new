...
def _compose_willow_notary_slot02_agent_entry():
...
_willow_notary_slot02_agent_query_entry = _compose_willow_notary_slot02_agent_entry()


def _compose_hazel_notary_slot02_agent_entry():
...
_hazel_notary_slot02_agent_query_entry = _compose_hazel_notary_slot02_agent_entry()


def _compose_cobalt_quill_slot02_agent_entry():
    """SN67 Harnyx miner — staged research protocol agent."""
...
_cobalt_quill_slot02_agent_query_entry = _compose_cobalt_quill_slot02_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "WillowNotarySlot02Agent"
    ...
    if (
        ...
        return "HazelNotarySlot02Agent"
    return "CobaltQuillSlot02Agent"


class WillowNotarySlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_notary_slot02_agent_query_entry(query)


class HazelNotarySlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _hazel_notary_slot02_agent_query_entry(query)


class CobaltQuillSlot02Agent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_quill_slot02_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = WillowNotarySlot02Agent()
_ANALYTICAL_FIELD_AGENT = HazelNotarySlot02Agent()
_BROAD_FIELD_AGENT = CobaltQuillSlot02Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "WillowNotarySlot02Agent",
    "HazelNotarySlot02Agent",
    "CobaltQuillSlot02Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "WillowNotarySlot02Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "HazelNotarySlot02Agent":
