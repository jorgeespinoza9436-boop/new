...
_cwqnrfklwo = {'started': None, 'text': None}
_pepglphbuy = 24000
_brreuligxl = 290.0
_uxmkoklmxr = 250.0

def _pmopoeskyn() -> float:
    started = _cwqnrfklwo['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _yjcxpjgrqw() -> float:
    return _brreuligxl - _pmopoeskyn()
_ovnbnxwcxz = _hsapi.llm_chat
_pybmbhoaob = _hsapi.search_web
_rtmrhwjykz = _hsapi.fetch_page
_xenphcdrkl = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _mlqruvpkfe(*args, **kwargs):
    if _pmopoeskyn() >= _uxmkoklmxr:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _xenphcdrkl})
            kwargs['messages'] = steered
    _result = await _ovnbnxwcxz(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _piijhylrwo(_result)
    return _result

async def _jlwqownyiq(*args, **kwargs):
    if _pmopoeskyn() >= _uxmkoklmxr:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _pybmbhoaob(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _megdbxsbmo(*args, **kwargs):
    if _pmopoeskyn() >= _uxmkoklmxr:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _rtmrhwjykz(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _mlqruvpkfe
_hsapi.search_web = _jlwqownyiq
_hsapi.fetch_page = _megdbxsbmo
_uscxavalxv = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_foicehvogh = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_jmvrhnexrr = 900
_olpmappego = 2

def _ltbjpsgrwx(query: Query) -> int:
...
def _abwawtlpfg(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _ftbakijhxi(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _ltbjpsgrwx(query)
    if fields >= 3:
        return 2
    if _abwawtlpfg(lowered, _uscxavalxv):
        return 1
    if fields <= _olpmappego and len(text) <= _jmvrhnexrr:
        return 0
    if _abwawtlpfg(lowered, _foicehvogh):
        return 0
    return 1

def _piijhylrwo(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _cwqnrfklwo['text'] = text.strip()[:_pepglphbuy]
    except Exception:
        pass

def _peusuazmek(text: str):
...
def _spognzuvjy(query: Query) -> Response:
    text = _cwqnrfklwo['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_pepglphbuy]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _peusuazmek(text)
...
def _ikmtdhusyc():
    ...
    return query
_yhbthvcrtz = _yfofpmkxol()
_toytuqagbz = _eelpxkqiwf()
_kfszzxatvh = _bammrbogxx()
_eqivuaokyc = _ikmtdhusyc()
_brreuligxl = 290.0
_gwijssuogb = 250.0
_qlacxjpnsd = 90.0

async def _bjevmsuxks(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _brreuligxl - (time.monotonic() - started)
        if first:
            budget = _gwijssuogb if _gwijssuogb < remaining else remaining
            first = False
        else:
            if remaining < _qlacxjpnsd:
                break
        ...
        try:
            ...
            last_exc = exc
    return _spognzuvjy(query)
_ipilpbevvj = 235.0

async def _pisdauhqzp(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _zgqoubolzc(query, agents):
    if len(agents) < 2:
        return await _bjevmsuxks(query, agents)
    r0, r1 = await asyncio.gather(_pisdauhqzp(agents[0], query, _ipilpbevvj), _pisdauhqzp(agents[1], query, _ipilpbevvj))
    ok0 = isinstance(r0, Response) and _pqevqmjgjg(r0) != ''
    ok1 = isinstance(r1, Response) and _pqevqmjgjg(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_ovnbnxwcxz(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _cwqnrfklwo['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _bjevmsuxks(query, (_kfszzxatvh, _yhbthvcrtz, _toytuqagbz, _eqivuaokyc))
        index = _ftbakijhxi(query)
        if index == 0:
            agents = (_yhbthvcrtz, _toytuqagbz, _kfszzxatvh, _eqivuaokyc)
        elif index == 1:
            agents = (_toytuqagbz, _kfszzxatvh, _eqivuaokyc, _yhbthvcrtz)
        elif index == 2:
            agents = (_kfszzxatvh, _eqivuaokyc, _yhbthvcrtz, _toytuqagbz)
        elif index == 3:
            agents = (_eqivuaokyc, _yhbthvcrtz, _toytuqagbz, _kfszzxatvh)
        else:
            agents = (_yhbthvcrtz, _toytuqagbz, _kfszzxatvh, _eqivuaokyc)
        return await _zgqoubolzc(query, agents)
    except Exception:
        return _spognzuvjy(query)
_BUILDL_TAG_m3 = "sn45-8e6f95a1b280"
