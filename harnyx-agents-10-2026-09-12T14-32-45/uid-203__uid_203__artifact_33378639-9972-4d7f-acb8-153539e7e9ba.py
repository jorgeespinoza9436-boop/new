...
_omtkgkpayr = {'started': None, 'text': None}
_esifyjmpaf = 24000
_gpjakunqbt = 290.0
_lprzqbruva = 250.0

def _zrslftgzce() -> float:
    started = _omtkgkpayr['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _dxzctlgtwd() -> float:
    return _gpjakunqbt - _zrslftgzce()
_cnmfecbsmj = _hsapi.llm_chat
_zklxpojtfx = _hsapi.search_web
_kdhkiqosdl = _hsapi.fetch_page
_jbemqwrhrb = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _fmuflmrpfz(*args, **kwargs):
    if _zrslftgzce() >= _lprzqbruva:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _jbemqwrhrb})
            kwargs['messages'] = steered
    _result = await _cnmfecbsmj(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _vkkqkfeuft(_result)
    return _result

async def _izunlhymqw(*args, **kwargs):
    if _zrslftgzce() >= _lprzqbruva:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _zklxpojtfx(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _wlwadsfrui(*args, **kwargs):
    if _zrslftgzce() >= _lprzqbruva:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _kdhkiqosdl(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _fmuflmrpfz
_hsapi.search_web = _izunlhymqw
_hsapi.fetch_page = _wlwadsfrui
_atgpypblgd = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_cztdxpfoac = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_ylihpofrxu = 900
_hadjbnumbf = 2

def _goqdxcepli(query: Query) -> int:
...
def _ifgqctbrem(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _yvmbmywvoy(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _goqdxcepli(query)
    if fields >= 3:
        return 2
    if _ifgqctbrem(lowered, _atgpypblgd):
        return 1
    if fields <= _hadjbnumbf and len(text) <= _ylihpofrxu:
        return 0
    if _ifgqctbrem(lowered, _cztdxpfoac):
        return 0
    return 1

def _vkkqkfeuft(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _omtkgkpayr['text'] = text.strip()[:_esifyjmpaf]
    except Exception:
        pass

def _lnyzqjeydi(text: str):
...
def _rdkgotwqly(query: Query) -> Response:
    text = _omtkgkpayr['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_esifyjmpaf]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _lnyzqjeydi(text)
...
def _lfgvgusbrl():
    ...
    return query
_mjdezfuwwd = _zqwaukhlnn()
_nsobefmtir = _hikwgifucr()
_sbcqetarvf = _crichqqqts()
_rrqstmmpsz = _lfgvgusbrl()
_gpjakunqbt = 290.0
_gmnbgporku = 250.0
_trnjcyipmi = 90.0

async def _kiaetpgyse(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _gpjakunqbt - (time.monotonic() - started)
        if first:
            budget = _gmnbgporku if _gmnbgporku < remaining else remaining
            first = False
        else:
            if remaining < _trnjcyipmi:
                break
        ...
        try:
            ...
            last_exc = exc
    return _rdkgotwqly(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _omtkgkpayr['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _kiaetpgyse(query, (_sbcqetarvf, _mjdezfuwwd, _nsobefmtir, _rrqstmmpsz))
        index = _yvmbmywvoy(query)
        if index == 0:
            agents = (_mjdezfuwwd, _nsobefmtir, _sbcqetarvf, _rrqstmmpsz)
        elif index == 1:
            agents = (_nsobefmtir, _sbcqetarvf, _rrqstmmpsz, _mjdezfuwwd)
        elif index == 2:
            agents = (_sbcqetarvf, _rrqstmmpsz, _mjdezfuwwd, _nsobefmtir)
        elif index == 3:
            agents = (_rrqstmmpsz, _mjdezfuwwd, _nsobefmtir, _sbcqetarvf)
        else:
            agents = (_mjdezfuwwd, _nsobefmtir, _sbcqetarvf, _rrqstmmpsz)
        return await _kiaetpgyse(query, agents)
    except Exception:
        return _rdkgotwqly(query)
_BUILDL_TAG_m13 = "sn45-9c737b8ba7e8"
