...
_wblpgpwtyk = {'started': None, 'text': None}
_npmlfboeoo = 24000
_ziylbvmreh = 290.0
_issoiifjfo = 250.0

def _pyymjlscto() -> float:
    started = _wblpgpwtyk['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _mwucxvmjyz() -> float:
    return _ziylbvmreh - _pyymjlscto()
_eqqlenlicx = _hsapi.llm_chat
_oglwswrocy = _hsapi.search_web
_vccfidcseg = _hsapi.fetch_page
_bnfrbtpymk = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _qouocrpzno(*args, **kwargs):
    if _pyymjlscto() >= _issoiifjfo:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _bnfrbtpymk})
            kwargs['messages'] = steered
    _result = await _eqqlenlicx(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _ndipcjsney(_result)
    return _result

async def _nsdbjuuiyb(*args, **kwargs):
    if _pyymjlscto() >= _issoiifjfo:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _oglwswrocy(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _ezlkybndas(*args, **kwargs):
    if _pyymjlscto() >= _issoiifjfo:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _vccfidcseg(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _qouocrpzno
_hsapi.search_web = _nsdbjuuiyb
_hsapi.fetch_page = _ezlkybndas
_qfasfjxauz = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_niiwzfqpte = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_nnntrmqoze = 900
_burntslaca = 2

def _tcishcahem(query: Query) -> int:
...
def _wqlgerxdvn(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _agqbjjqajv(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _tcishcahem(query)
    if fields >= 3:
        return 2
    if _wqlgerxdvn(lowered, _qfasfjxauz):
        return 1
    if fields <= _burntslaca and len(text) <= _nnntrmqoze:
        return 0
    if _wqlgerxdvn(lowered, _niiwzfqpte):
        return 0
    return 1

def _ndipcjsney(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _wblpgpwtyk['text'] = text.strip()[:_npmlfboeoo]
    except Exception:
        pass

def _vrspvngouv(text: str):
...
def _qllfjejvst(query: Query) -> Response:
    text = _wblpgpwtyk['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_npmlfboeoo]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _vrspvngouv(text)
    ...
    class Grid:
        ...
        def col(self, name: str) -> int | None:
            ...
            for i, head in enumerate(heads):
                hits = len(want_words.intersection(set(head.split())))
                if hits > best_hits:
...
"""
    ...
    return query
_qswehkbbec = _yrxkjrywez()
_ajqdyapbvk = _dxflryiumu()
_xwgtghbeqi = _nmnklnworv()
_qvcsgnrdui = _jhsicybtea()
_ziylbvmreh = 290.0
_dvccypisow = 250.0
_zxhcjfqrnn = 90.0

async def _wzwgzwkoas(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ziylbvmreh - (time.monotonic() - started)
        if first:
            budget = _dvccypisow if _dvccypisow < remaining else remaining
            first = False
        else:
            if remaining < _zxhcjfqrnn:
                break
        ...
        try:
            ...
            last_exc = exc
    return _qllfjejvst(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _wblpgpwtyk['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _wzwgzwkoas(query, (_xwgtghbeqi, _qswehkbbec, _ajqdyapbvk, _qvcsgnrdui))
        index = _agqbjjqajv(query)
        if index == 0:
            agents = (_qswehkbbec, _ajqdyapbvk, _xwgtghbeqi, _qvcsgnrdui)
        elif index == 1:
            agents = (_ajqdyapbvk, _xwgtghbeqi, _qvcsgnrdui, _qswehkbbec)
        elif index == 2:
            agents = (_xwgtghbeqi, _qvcsgnrdui, _qswehkbbec, _ajqdyapbvk)
        elif index == 3:
            agents = (_qvcsgnrdui, _qswehkbbec, _ajqdyapbvk, _xwgtghbeqi)
        else:
            agents = (_qswehkbbec, _ajqdyapbvk, _xwgtghbeqi, _qvcsgnrdui)
        return await _wzwgzwkoas(query, agents)
    except Exception:
        return _qllfjejvst(query)
_BUILDK_TAG_m7 = "sn45-986b5c7edaaf"
