...
_bwkqxawwyr = {'started': None, 'text': None}
_rmromeqpjy = 24000
_ozjeaijcrj = 290.0
_alryomkncs = 250.0

def _igpkqlpafs() -> float:
    started = _bwkqxawwyr['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _hkyjbrghgx() -> float:
    return _ozjeaijcrj - _igpkqlpafs()
_aibvautnwu = _hsapi.llm_chat
_xlempjkovj = _hsapi.search_web
_gzgwjxzcwj = _hsapi.fetch_page
_sutwcqokez = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _yflawqmxue(*args, **kwargs):
    if _igpkqlpafs() >= _alryomkncs:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _sutwcqokez})
            kwargs['messages'] = steered
    _result = await _aibvautnwu(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _pagwnzvpjn(_result)
    return _result

async def _bzzylymfgt(*args, **kwargs):
    if _igpkqlpafs() >= _alryomkncs:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _xlempjkovj(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _zcygcrmtgm(*args, **kwargs):
    if _igpkqlpafs() >= _alryomkncs:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _gzgwjxzcwj(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _yflawqmxue
_hsapi.search_web = _bzzylymfgt
_hsapi.fetch_page = _zcygcrmtgm
_aqhxrswkea = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_aezyajopyl = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_lkdjvdonlu = 900
_zxynxwdfuq = 2

def _brwlpvsptg(query: Query) -> int:
...
def _mbovcjklhf(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _bflnppcvvg(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _brwlpvsptg(query)
    if fields >= 3:
        return 2
    if _mbovcjklhf(lowered, _aqhxrswkea):
        return 1
    if fields <= _zxynxwdfuq and len(text) <= _lkdjvdonlu:
        return 0
    if _mbovcjklhf(lowered, _aezyajopyl):
        return 0
    return 1

def _pagwnzvpjn(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _bwkqxawwyr['text'] = text.strip()[:_rmromeqpjy]
    except Exception:
        pass

def _kgimfugapa(text: str):
...
def _jwibbynipc(query: Query) -> Response:
    text = _bwkqxawwyr['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_rmromeqpjy]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _kgimfugapa(text)
...
def _shuxmdbqwi():
    """hybrid — agentic deep-research agent for Harnyx SN67.

ours.py's retrieval loop, claim table and select-best pool, with grid.py's
computed table answer as one more route in the pool: when the retained pages
hold a table, it is transcribed into typed cells, the comparison the question
asks for is compiled once into a closed program, Python runs it, and the
resulting verdict-first answer competes with the written ones for the ship.
...
"""
    ...
    *3,100 (footnoted); (12.4) negative; 2,567 lb with a unit; =WR and CR marks;
    ...
    async def _computed_candidate(plan: QuestionPlan, ledger: EvidenceLedger, deadline: float) -> str:
        """The grid's answer, when there is a table to compute over; otherwise ""."""
        if plan.fast or not ledger.rows or deadline - monotonic() < GRID_MIN_SECONDS + GRID_WRITE_RESERVE_S:
            return ''
        try:
            grids = await _harvest_grids(plan, ledger, deadline)
            if not grids or deadline - monotonic() <= GRID_WRITE_RESERVE_S:
                return ''
            program = await _compile_program(plan, grids, deadline)
            result = _run_program(program, grids)
            if result is None:
                return ''
            computed = _write_grid_answer(plan, result)
            if not _is_usable_answer(computed):
                return ''
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        computed = await _computed_candidate(plan, ledger, deadline)
        if computed:
            pool.insert(0, computed)
        ...
        try:
            ...
            return Response(text=text)
    return query
_yzkvclepfi = _weicmxfaad()
_vlfqtlyjjt = _soxcvixbod()
_poixxohgfw = _shuxmdbqwi()
_ozjeaijcrj = 290.0
_tqpahdepit = 250.0
_yklnybrszo = 90.0

async def _ppmlswbcgs(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ozjeaijcrj - (time.monotonic() - started)
        if first:
            budget = _tqpahdepit if _tqpahdepit < remaining else remaining
            first = False
        else:
            if remaining < _yklnybrszo:
                break
        ...
        try:
            ...
            last_exc = exc
    return _jwibbynipc(query)
_fqkbpoantf = 235.0

async def _ytmguzexdo(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _bxzogtvire(query, agents):
    if len(agents) < 2:
        return await _ppmlswbcgs(query, agents)
    r0, r1 = await asyncio.gather(_ytmguzexdo(agents[0], query, _fqkbpoantf), _ytmguzexdo(agents[1], query, _fqkbpoantf))
    ok0 = isinstance(r0, Response) and _qvbfzdgfnm(r0) != ''
    ok1 = isinstance(r1, Response) and _qvbfzdgfnm(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_aibvautnwu(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _bwkqxawwyr['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ppmlswbcgs(query, (_poixxohgfw, _yzkvclepfi, _vlfqtlyjjt))
        index = _bflnppcvvg(query)
        if index == 0:
            agents = (_yzkvclepfi, _vlfqtlyjjt, _poixxohgfw)
        elif index == 1:
            agents = (_vlfqtlyjjt, _poixxohgfw, _yzkvclepfi)
        elif index == 2:
            agents = (_poixxohgfw, _yzkvclepfi, _vlfqtlyjjt)
        else:
            agents = (_yzkvclepfi, _vlfqtlyjjt, _poixxohgfw)
        return await _bxzogtvire(query, agents)
    except Exception:
        return _jwibbynipc(query)
_BUILDL_TAG_m21 = "sn45-4ef9778d6c8f"
