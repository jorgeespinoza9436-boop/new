...
_brkjdqqpyx = {'started': None, 'text': None}
_exhpitrdri = 24000
_tcpmeaevdy = 290.0
_xghcvlmmlv = 250.0

def _nitrtivgxs() -> float:
    started = _brkjdqqpyx['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _bvjypltsmg() -> float:
    return _tcpmeaevdy - _nitrtivgxs()
_ixwpolgjcn = _hsapi.llm_chat
_ssdvdyprpv = _hsapi.search_web
_qtqxzkkqoj = _hsapi.fetch_page
_xpfzfwehqz = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _pccewwitek(*args, **kwargs):
    if _nitrtivgxs() >= _xghcvlmmlv:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _xpfzfwehqz})
            kwargs['messages'] = steered
    _result = await _ixwpolgjcn(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _gcmbwrbavm(_result)
    return _result

async def _fyomdrccde(*args, **kwargs):
    if _nitrtivgxs() >= _xghcvlmmlv:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _ssdvdyprpv(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _gsaydvxwhj(*args, **kwargs):
    if _nitrtivgxs() >= _xghcvlmmlv:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qtqxzkkqoj(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _pccewwitek
_hsapi.search_web = _fyomdrccde
_hsapi.fetch_page = _gsaydvxwhj
_uytfeoefps = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_bfsezerxow = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_fvhmzuptqj = 900
_dxhfyutrfm = 2

def _bpxlsvahpb(query: Query) -> int:
...
def _qgwikkqqbf(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _cdrwzafhug(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _bpxlsvahpb(query)
    if fields >= 3:
        return 2
    if _qgwikkqqbf(lowered, _uytfeoefps):
        return 1
    if fields <= _dxhfyutrfm and len(text) <= _fvhmzuptqj:
        return 0
    if _qgwikkqqbf(lowered, _bfsezerxow):
        return 0
    return 1

def _gcmbwrbavm(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _brkjdqqpyx['text'] = text.strip()[:_exhpitrdri]
    except Exception:
        pass

def _hkdghmftnh(text: str):
...
def _oypkjksrzt(query: Query) -> Response:
    text = _brkjdqqpyx['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_exhpitrdri]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _hkdghmftnh(text)
...
def _kvdomfmaoq():
    ...
    def _sh_shape(query_input, response):
        ...
        try:
            ...
            return response
    return query
_cyrxjriinz = _kvfvbsijni()
_vtuktxxfov = _wlrbeohecy()
_ogspvnqzti = _wpckbjymbb()
_ciohbkruyt = _kvdomfmaoq()
_tcpmeaevdy = 290.0
_khixfxgqmd = 250.0
_euvemxdote = 90.0

async def _sjxfvfjptq(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _tcpmeaevdy - (time.monotonic() - started)
        if first:
            budget = _khixfxgqmd if _khixfxgqmd < remaining else remaining
            first = False
        else:
            if remaining < _euvemxdote:
                break
        ...
        try:
            ...
            last_exc = exc
    return _oypkjksrzt(query)
_udecocnxdj = 235.0

async def _qvsdblfody(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _zpfacdcyni(query, agents):
    if len(agents) < 2:
        return await _sjxfvfjptq(query, agents)
    r0, r1 = await asyncio.gather(_qvsdblfody(agents[0], query, _udecocnxdj), _qvsdblfody(agents[1], query, _udecocnxdj))
    ok0 = isinstance(r0, Response) and _dcliiwzjkt(r0) != ''
    ok1 = isinstance(r1, Response) and _dcliiwzjkt(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_ixwpolgjcn(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _brkjdqqpyx['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _sjxfvfjptq(query, (_ogspvnqzti, _cyrxjriinz, _vtuktxxfov, _ciohbkruyt))
        index = _cdrwzafhug(query)
        if index == 0:
            agents = (_cyrxjriinz, _vtuktxxfov, _ogspvnqzti, _ciohbkruyt)
        elif index == 1:
            agents = (_vtuktxxfov, _ogspvnqzti, _ciohbkruyt, _cyrxjriinz)
        elif index == 2:
            agents = (_ogspvnqzti, _ciohbkruyt, _cyrxjriinz, _vtuktxxfov)
        elif index == 3:
            agents = (_ciohbkruyt, _cyrxjriinz, _vtuktxxfov, _ogspvnqzti)
        else:
            agents = (_cyrxjriinz, _vtuktxxfov, _ogspvnqzti, _ciohbkruyt)
        return await _zpfacdcyni(query, agents)
    except Exception:
        return _oypkjksrzt(query)
_BUILDL_TAG_m12 = "sn45-b2928b463a41"
