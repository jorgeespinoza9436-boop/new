...
_zbylxvnrls = {'started': None, 'text': None}
_mfhsaufgmq = 24000
_glxzivwvyq = 290.0
_vtrdwwzyja = 250.0

def _iehgbgwpnt() -> float:
    started = _zbylxvnrls['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _gstidazgbr() -> float:
    return _glxzivwvyq - _iehgbgwpnt()
_tceciutvwb = _hsapi.llm_chat
_jwexgzzoho = _hsapi.search_web
_xorzwvnvkf = _hsapi.fetch_page
_kdxgxojfbf = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _gogskmxvrk(*args, **kwargs):
    if _iehgbgwpnt() >= _vtrdwwzyja:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _kdxgxojfbf})
            kwargs['messages'] = steered
    _result = await _tceciutvwb(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _zkeyzbppkc(_result)
    return _result

async def _ywuezzjdfp(*args, **kwargs):
    if _iehgbgwpnt() >= _vtrdwwzyja:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _jwexgzzoho(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _gqpshkxfec(*args, **kwargs):
    if _iehgbgwpnt() >= _vtrdwwzyja:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _xorzwvnvkf(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _gogskmxvrk
_hsapi.search_web = _ywuezzjdfp
_hsapi.fetch_page = _gqpshkxfec
_fjkstfitxw = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_owzbvnluxh = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_fuutdydidg = 900
_kgwfemvxzw = 2

def _mrrpfqzqez(query: Query) -> int:
...
def _gtwfcppkpo(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _uavbmdfdrv(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _mrrpfqzqez(query)
    if fields >= 3:
        return 2
    if _gtwfcppkpo(lowered, _fjkstfitxw):
        return 1
    if fields <= _kgwfemvxzw and len(text) <= _fuutdydidg:
        return 0
    if _gtwfcppkpo(lowered, _owzbvnluxh):
        return 0
    return 1

def _zkeyzbppkc(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _zbylxvnrls['text'] = text.strip()[:_mfhsaufgmq]
    except Exception:
        pass

def _saujhloosq(text: str):
...
def _bjiwpfquvo(query: Query) -> Response:
    text = _zbylxvnrls['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_mfhsaufgmq]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _saujhloosq(text)
...
def _xymfouluru():
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return base
    return query
_iudwizsbkj = _ubyglpfytl()
_kuzsqufqdy = _nebleywtpg()
_jrxptxygwt = _ofjocgqowc()
_oxnznjxuum = _xymfouluru()
_glxzivwvyq = 290.0
_emjovfrhum = 250.0
_wgatuecihb = 90.0

async def _ggqhebjcie(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _glxzivwvyq - (time.monotonic() - started)
        if first:
            budget = _emjovfrhum if _emjovfrhum < remaining else remaining
            first = False
        else:
            if remaining < _wgatuecihb:
                break
        ...
        try:
            ...
            last_exc = exc
    return _bjiwpfquvo(query)
_dlblfxjmct = 235.0

async def _lkcnrilfxs(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _wbiihxyclz(query, agents):
    if len(agents) < 2:
        return await _ggqhebjcie(query, agents)
    r0, r1 = await asyncio.gather(_lkcnrilfxs(agents[0], query, _dlblfxjmct), _lkcnrilfxs(agents[1], query, _dlblfxjmct))
    ok0 = isinstance(r0, Response) and _tfahjipqou(r0) != ''
    ok1 = isinstance(r1, Response) and _tfahjipqou(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_tceciutvwb(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _zbylxvnrls['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ggqhebjcie(query, (_jrxptxygwt, _iudwizsbkj, _kuzsqufqdy, _oxnznjxuum))
        index = _uavbmdfdrv(query)
        if index == 0:
            agents = (_iudwizsbkj, _kuzsqufqdy, _jrxptxygwt, _oxnznjxuum)
        elif index == 1:
            agents = (_kuzsqufqdy, _jrxptxygwt, _oxnznjxuum, _iudwizsbkj)
        elif index == 2:
            agents = (_jrxptxygwt, _oxnznjxuum, _iudwizsbkj, _kuzsqufqdy)
        elif index == 3:
            agents = (_oxnznjxuum, _iudwizsbkj, _kuzsqufqdy, _jrxptxygwt)
        else:
            agents = (_iudwizsbkj, _kuzsqufqdy, _jrxptxygwt, _oxnznjxuum)
        return await _wbiihxyclz(query, agents)
    except Exception:
        return _bjiwpfquvo(query)
_BUILDL_TAG_m18 = "sn45-854b622aa8a4"
