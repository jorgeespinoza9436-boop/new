...
_wzsdsengsa = {'started': None, 'text': None}
_mgonwczkbh = 24000
_sbtnumsxim = 290.0
_qpkuyhpzuh = 250.0

def _qvsqdrqwzw() -> float:
    started = _wzsdsengsa['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _jlvwjtslqk() -> float:
    return _sbtnumsxim - _qvsqdrqwzw()
_pssgsiqope = _hsapi.llm_chat
_vgamwlrciq = _hsapi.search_web
_qthumjdttr = _hsapi.fetch_page
_tqzsbiyfvk = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _gamaqaqnql(*args, **kwargs):
    if _qvsqdrqwzw() >= _qpkuyhpzuh:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _tqzsbiyfvk})
            kwargs['messages'] = steered
    _result = await _pssgsiqope(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _qaqagrlyma(_result)
    return _result

async def _huporazesi(*args, **kwargs):
    if _qvsqdrqwzw() >= _qpkuyhpzuh:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _vgamwlrciq(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _lfvwabusux(*args, **kwargs):
    if _qvsqdrqwzw() >= _qpkuyhpzuh:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qthumjdttr(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _gamaqaqnql
_hsapi.search_web = _huporazesi
_hsapi.fetch_page = _lfvwabusux
_yizyzifcvl = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_zubglsxaps = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_tnshtdyiqg = 900
_bchxefxhah = 2

def _dnpuhrvdqd(query: Query) -> int:
...
def _vtnoqqfnek(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _abxefpvlvc(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _dnpuhrvdqd(query)
    if fields >= 3:
        return 2
    if _vtnoqqfnek(lowered, _yizyzifcvl):
        return 1
    if fields <= _bchxefxhah and len(text) <= _tnshtdyiqg:
        return 0
    if _vtnoqqfnek(lowered, _zubglsxaps):
        return 0
    return 1

def _qaqagrlyma(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _wzsdsengsa['text'] = text.strip()[:_mgonwczkbh]
    except Exception:
        pass

def _jbhcrntzld(text: str):
...
def _eyvbwkabmb(query: Query) -> Response:
    text = _wzsdsengsa['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_mgonwczkbh]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _jbhcrntzld(text)
...
"""
    ...
    """
        ...
        return response
    return query
_tuwvkkgxzn = _flrxmnbjwp()
_upwuogpuyz = _fmgkmfdkpj()
_dypndkkygp = _upghsxnqrv()
_sbtnumsxim = 290.0
_vpqicgddtj = 250.0
_lzjvgaxvss = 90.0

async def _gdeewtphzk(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _sbtnumsxim - (time.monotonic() - started)
        if first:
            budget = _vpqicgddtj if _vpqicgddtj < remaining else remaining
            first = False
        else:
            if remaining < _lzjvgaxvss:
                break
        ...
        try:
            ...
            last_exc = exc
    return _eyvbwkabmb(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _wzsdsengsa['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _gdeewtphzk(query, (_dypndkkygp, _tuwvkkgxzn, _upwuogpuyz))
        index = _abxefpvlvc(query)
        if index == 0:
            agents = (_tuwvkkgxzn, _upwuogpuyz, _dypndkkygp)
        elif index == 1:
            agents = (_upwuogpuyz, _dypndkkygp, _tuwvkkgxzn)
        elif index == 2:
            agents = (_dypndkkygp, _tuwvkkgxzn, _upwuogpuyz)
        else:
            agents = (_tuwvkkgxzn, _upwuogpuyz, _dypndkkygp)
        return await _gdeewtphzk(query, agents)
    except Exception:
        return _eyvbwkabmb(query)
_BUILDL_TAG_m23 = "sn45-415db217db12"
