...
def _compose_saffron_beacon_slot10_agent_entry():
...
_saffron_beacon_slot10_agent_query_entry = _compose_saffron_beacon_slot10_agent_entry()


def _compose_cobalt_marten_slot10_agent_entry():
...
_cobalt_marten_slot10_agent_query_entry = _compose_cobalt_marten_slot10_agent_entry()


def _compose_opal_marten_slot10_agent_entry():
    """SN67 Harnyx miner — staged research protocol agent."""
...
_opal_marten_slot10_agent_query_entry = _compose_opal_marten_slot10_agent_entry()


def _task_shape_route_label(query: Query) -> str:
    if getattr(query, "output_schema", None) is not None:
        return "SaffronBeaconSlot10Agent"
    ...
    if (
        ...
        return "CobaltMartenSlot10Agent"
    return "OpalMartenSlot10Agent"


class SaffronBeaconSlot10Agent:
    async def __call__(self, query: Query) -> Response:
        return await _saffron_beacon_slot10_agent_query_entry(query)


class CobaltMartenSlot10Agent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_marten_slot10_agent_query_entry(query)


class OpalMartenSlot10Agent:
    async def __call__(self, query: Query) -> Response:
        return await _opal_marten_slot10_agent_query_entry(query)


_STRUCTURED_FIELD_AGENT = SaffronBeaconSlot10Agent()
_ANALYTICAL_FIELD_AGENT = CobaltMartenSlot10Agent()
_BROAD_FIELD_AGENT = OpalMartenSlot10Agent()
_CANDIDATE_BRANCH_CLASS_NAMES = (
    "SaffronBeaconSlot10Agent",
    "CobaltMartenSlot10Agent",
    "OpalMartenSlot10Agent",
)
...
async def query(query: Query) -> Response:
    ...
    if selected == "SaffronBeaconSlot10Agent":
        branch = _STRUCTURED_FIELD_AGENT
    elif selected == "CobaltMartenSlot10Agent":
