...
_SHAPE_ROUTER_SEED = "daa5448f6c50fb94aa7c896f"
...
def _balanced_route_label(query: Query) -> str:
    if getattr(query, "fast", False):
        return "IvoryQuillAgent"
    text = (getattr(query, "text", "") or "").strip()
    shape = _shape_class(query)
    if shape == 0:
        return "CobaltPrismAgent"
    if shape == 1:
...
_SHAPE_PRIMARY_AGENT = IvoryQuillAgent()
_SHAPE_SECONDARY_AGENT = CobaltPrismAgent()
_SHAPE_TERTIARY_AGENT = JuniperLatticeAgent()
