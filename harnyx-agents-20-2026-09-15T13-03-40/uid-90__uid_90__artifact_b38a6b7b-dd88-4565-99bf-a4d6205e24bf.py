...
def _compose_branch0_b405_c_a_e_c72_b_entry():
...
_branch0_b405_c_a_e_c72_b_query_entry = _compose_branch0_b405_c_a_e_c72_b_entry()
...
def _compose_branch7038560_c85_d1_entry():
    ...
    async def _w4_baseline_query(query: Query, budget: float | None = None) -> Response:
    ...
    """
        ...
        _MAIN_DEAD.clear()
        schema = getattr(query, "output_schema", None)
        total = TASK_TOTAL_BUDGET_SECONDS if budget is None else budget
        if schema is None:
            return await _plain_query_with_cold_retry(query, total)
        try:
            drafted = await _plain_query_with_cold_retry(
                query, total - STRUCTURED_RESERVE_SECONDS)
        except Exception:
    ...
    async def _w4_research_or_salvage(query_input: Query, budget: float | None = None) -> Response:
        """Stage 2 - the research stage, held so no failure inside it can escape.
    ...
    """
        try:
            return await _w4_baseline_query(query_input, budget)
        except Exception:
            return Response(text="No verifiable source-backed answer was reached for this question.")


    # ── claim board ──────────────────────────────────────────────────────────
    # From here the answer is organised by what it OWES, not by what the research
    # stage happened to gather. The question is resolved into claim slots; rows are
    # retrieved and bound to the claims they can support; a claim nothing supports
    # sends retrieval back out; and the delivered answer is written claim by claim
    # off that board, carrying the markers of the rows that back it. The research
    # stage's own answer is kept only as the fallback for a board that cannot be
    # built, written, or backed.
    CB_MAX = 6
    CB_MIN_LEFT_S = 34.0
    CB_PLAN_MIN_LEFT_S = 90.0
    CB_PROBE_MAX = 2
    CB_GATHER_MAX = 3
    CB_PLAN_TIMEOUT_S = 18.0
    CB_WRITE_TIMEOUT_S = 60.0
    CB_WRITE_RESERVE_S = 8.0
    CB_STAGE_RESERVE_S = 58.0
    CB_BIND_RATIO = 0.6
    CB_KEEP_PCT = 55
    CB_ROW_CHARS = 1200
    CB_ROW_TEXT_CHARS = 4000
    CB_BOARD_CHARS = 44000
    CB_MIN_KEYS = 2
    CB_ROWS_PER_SLOT = 4
    CB_SEARCH_RESULTS = 8
    CB_CITATION_CAP = 24
    CB_MIN_ANSWER_CHARS = 200
    CB_ROUTE_MIN_DEMANDS = 2
    CB_ROUTE_MIN_CHARS = 120

    _CB_TOKEN_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9'’&.\-]*")
    _CB_NUM_RE = re.compile(r"\d[\d,.]*")
    _CB_NAME_RE = re.compile(r"[A-Z][A-Za-z0-9&'’.\-]{2,}")
    _CB_MARK_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
    _CB_DOUBLE_RE = re.compile(r"\[\[(\d[\d,\s\-]*)\]\]")
    _CB_PART_RE = re.compile(r"(?<=[.!?])\s+|\n+|;")
    _CB_DEMAND_RE = re.compile(
        r"\b(how many|how much|which|what|who|when|where|list|name|state|give|"
        r"compare|identify)\b")
    _CB_ROUTE_SIGNALS = ("list all", "for each", "both", "respectively", "as well as",
                         "in addition to", "along with", "and also", "together with")
    _CB_TOOL_RE = re.compile(r"<\s*/?\s*(tool_call|function_call|invoke|parameter)\b", re.I)
    _CB_STOP = frozenset(
        "the a an and or of for to in on at by with from that this these those "
        "is are was were be been being do does did have has had will would can "
        "what which who whom whose when where why how many much list name give "
        "state provide return include answer question task using based each "
        "every all both only also please your their its his her them they it "
        "must should need needs into over under than then there here about "
        "output format json text line lines word words".split())


    def _cb_keys(claim: str) -> list[str]:
        """The words that decide whether a gathered row speaks to this claim."""
        keys: list[str] = []
        for match in _CB_TOKEN_RE.finditer((claim or "").lower()):
            token = match.group(0).strip(".-'’")
            if len(token) >= 4 and token not in _CB_STOP and token not in keys:
                keys.append(token)
        return keys[:8]


    def _cb_seeds(question: str) -> list[str]:
        """Claim slots read deterministically off the question itself."""
        out: list[str] = []
        for part in _CB_PART_RE.split(question or ""):
            text = " ".join(part.split())
            if len(text) < 12 or len(_cb_keys(text)) < CB_MIN_KEYS:
                continue
            if any(text.lower() == other.lower() for other in out):
                continue
            out.append(text[:180])
            if len(out) >= CB_MAX:
                break
    ...
    def _cb_demands(text: str) -> int:
        """How many separate statements the question owes."""
        lowered = (text or "").casefold()
        count = 0
        for part in re.split(r"(?<=[.!?])\s+|;|\n", lowered):
            piece = part.strip()
            if len(piece) >= 12 and _CB_DEMAND_RE.search(piece):
                count += 1
        if any(signal in lowered for signal in _CB_ROUTE_SIGNALS):
            count += 1
        return count


    def _cb_worth_board(query_input: object, question: str) -> bool:
        """The board earns its cost only on questions that owe several claims.

    A structured task is answered through `output`, which the board does not
    produce, and a fast task is scored by component F1 — where a proof block the
    question did not ask for counts as an excessive component. Both stay on the
    research stage's own path.
    """
        try:
            if getattr(query_input, "output_schema", None) is not None:
                return False
            if bool(getattr(query_input, "fast", False)):
                return False
            text = getattr(query_input, "text", None) or question or ""
            if len(text) < CB_ROUTE_MIN_CHARS:
                return False
            return _cb_demands(text) >= CB_ROUTE_MIN_DEMANDS
        except Exception:
            return False


    def _cb_baseline_budget(board: bool) -> float:
        """The research stage's budget, with the board's slice held back."""
        total = _w4_total_budget_seconds()
        if not board:
            return total
        return max(total * 0.5, total - CB_STAGE_RESERVE_S)


    async def _cb_chat(system: str, user: str, *, timeout: float) -> str:
        if timeout <= 0:
            return ""
        messages = [{"role": "system", "content": system}, {"role": "user", "content": user}]
        return await _w4_chat(messages, timeout=timeout, temperature=0.15)


    async def _cb_slots(question: str, deadline: float) -> list[str]:
        """Resolve what the answer owes before any of it is written."""
        slots = _cb_seeds(question)
        left = _w4_remaining(deadline)
        if left < CB_PLAN_MIN_LEFT_S:
            return slots[:CB_MAX]
        probe = (
            "Break the question into the distinct CLAIMS an answer must state: one "
            "short line per claim, each naming what has to be established — a "
            "figure, a name, a date, a comparison, a membership list. No preamble. "
            "JSON only: a list of strings, at most 6.\n\nQuestion:\n"
            + (question or "")[:3500])
        try:
            raw = await _cb_chat(
                "Research planner. Name the claims to establish. JSON list only.",
                probe, timeout=max(6.0, min(CB_PLAN_TIMEOUT_S, left - CB_MIN_LEFT_S)))
            raw = re.sub(r"^```(?:json)?\s*|\s*```$", "", (raw or "").strip(), flags=re.I | re.M)
        ...
        if isinstance(parsed, list):
            for item in parsed:
                ...
                if len(text) < 8 or len(_cb_keys(text)) < CB_MIN_KEYS:
                    continue
                if not any(text.lower() == slot.lower() for slot in slots):
                    slots.append(text[:180])
        return slots[:CB_MAX]


    async def _cb_search(query_text: str, rows: list[dict]) -> bool:
        """One bounded search, recorded as citable rows for the board."""
        if not (query_text or "").strip():
            return False
        try:
            payload = await search_web(query_text, provider="parallel",
        ...
        if not receipt or not results:
            return False
        seen = {row.get("result_id") for row in rows}
        added = False
        for item in results[:CB_SEARCH_RESULTS]:
    ...
    async def _cb_gather(question: str, slots: list[str], rows: list[dict],
                         deadline: float) -> None:
        """First pass of retrieval, aimed at the claims rather than at the page."""
        tail = " ".join(word for word in (question or "").split()
                        if word.lower() not in _CB_STOP)[:90]
        fired = 0
        for claim in slots:
            if fired >= CB_GATHER_MAX or _w4_remaining(deadline) < CB_MIN_LEFT_S:
                break
            probe = " ".join(_cb_keys(claim)[:6])
            if not probe.strip():
                continue
            await _cb_search((probe + " " + tail).strip(), rows)
            fired += 1


    def _cb_row_blob(row: dict) -> str:
        parts = []
    ...
    def _cb_row_text(row: dict, cap: int = CB_ROW_CHARS) -> str:
        return (row.get("text") or "").strip()[:cap]


    def _cb_bind(slots: list[str], rows: list[dict]) -> dict[int, list[int]]:
        """Bind every gathered row to the claims it can support.

    This mapping, not the result list, is the run's evidence state: rows are
    addressed through the claim they answer, and a claim with no row is an open
    research target rather than a formatting problem.
    """
        blobs = [(_cb_row_blob(row) or "").lower() for row in rows]
        bound: dict[int, list[int]] = {}
        for index, claim in enumerate(slots):
            keys = _cb_keys(claim)
            hits: list[int] = []
            if keys:
                for number, blob in enumerate(blobs, start=1):
                    ...
                    for key in keys:
                        if key in blob:
                            found += 1
                    if float(found) / float(len(keys)) >= CB_BIND_RATIO:
                        hits.append(number)
            bound[index] = hits[:CB_ROWS_PER_SLOT]
        return bound


    def _cb_open(slots: list[str], bound: dict[int, list[int]]) -> list[str]:
        return [claim for index, claim in enumerate(slots) if not bound.get(index)]


    async def _cb_fill(question: str, open_claims: list[str], rows: list[dict],
                       deadline: float) -> bool:
        """Send retrieval back out for the claims nothing supports yet."""
        tail = " ".join(word for word in (question or "").split()
                        if word.lower() not in _CB_STOP)[:90]
        fired = False
        for claim in open_claims[:CB_PROBE_MAX]:
            if _w4_remaining(deadline) < CB_MIN_LEFT_S:
                break
            probe = " ".join(_cb_keys(claim)[:6])
            if not probe.strip():
                continue
            if await _cb_search((probe + " " + tail).strip(), rows):
                fired = True
        return fired


    def _cb_board(slots: list[str], bound: dict[int, list[int]], rows: list[dict]) -> str:
        """The board handed to the writer: evidence indexed by claim."""
        lines = ["CLAIM BOARD — each claim with the evidence bound to it. "
                 "Cite by the [n] shown."]
        spent = 0
        for index, claim in enumerate(slots):
            numbers = bound.get(index) or []
            lines.append("")
            lines.append("CLAIM %d: %s" % (index + 1, claim))
            if not numbers:
                lines.append("  (nothing gathered supports this claim — say so plainly "
                             "in the answer and state what was checked)")
                continue
            for number in numbers:
                if number < 1 or number > len(rows):
                    continue
                row = rows[number - 1]
                body = _cb_row_text(row).strip()
                if not body:
                    continue
                block = "  [%d] %s (%s)\n    %s" % (
                    number, row.get("title") or "", row.get("url") or "", body)
                if spent + len(block) > CB_BOARD_CHARS:
                    break
                spent += len(block)
                lines.append(block)
        return "\n".join(lines)
    ...
    _CB_FILTER_RE = re.compile(
        r"\b(exclud\w+|excluding|not appear\w*|do(?:es)? not appear|only those|"
        r"but not|other than|apart from|omit\w*|ignore|disregard|leaving out|"
        r"still (?:operating|active|listed)|no longer|removed from|dropped from|"
        r"restrict\w* to|limited to|qualif\w+)\b", re.I)

    def _cb_filtered_pool(question: str) -> bool:
        """Does the question define its set by exclusion?

    A question that keeps some rows and drops others is the one a judge marks
    wrong when the answer shows only the survivors: the work of filtering is
    invisible, so a correct verdict reads as a bare assertion.
    """
        return bool(_CB_FILTER_RE.search(question or ""))


    async def _cb_write(question: str, board: str, deadline: float) -> str:
        """Write the answer claim by claim, straight off the board."""
        left = _w4_remaining(deadline) - CB_WRITE_RESERVE_S
        if left <= 10.0 or not board.strip():
            return ""
        system = ("You write the FINAL answer from a claim board. Work claim by "
                  "claim: state what each claim establishes, in the order the "
                  "question asks for, and put the [n] marker of the bound row on "
                  "the sentence that carries the figure or name. A claim the board "
                  "shows as unsupported is reported as unestablished, naming what "
                  "was checked — never invented. No preamble, no process narration, "
                  "no headings unless the question asks for them: the answer only, "
                  "in the shape the question requires.")
        if _cb_filtered_pool(question):
            system += (
                " This question defines its set by exclusion. Before the answer "
                "proper, name the pool you swept and the items you dropped with "
                "the reason each was dropped, each carrying the [n] that shows "
                "it -- an answer that lists only the survivors leaves the judge "
                "nothing to check the filtering against. Keep it to one compact "
                "block; the answer the question asks for still follows, in the "
                "required shape.")
        user = "QUESTION:\n" + (question or "")[:3500] + "\n\n" + board
        try:
            text = await _cb_chat(system, user,
                                  timeout=min(CB_WRITE_TIMEOUT_S, left))
        except Exception:
            return ""
        return (text or "").strip()


    def _cb_facts(text: str) -> set[str]:
        body = _CB_MARK_RE.sub(" ", text or "")
        out: set[str] = set()
        for match in _CB_NUM_RE.finditer(body):
            out.add("n:" + match.group(0).replace(",", "").rstrip("."))
        for match in _CB_NAME_RE.finditer(body):
            out.add("e:" + " ".join(match.group(0).split()).lower())
        return out


    def _cb_keeps_facts(draft: str, written: str) -> bool:
        before = _cb_facts(draft)
        if not before:
            return True
        kept = len(before & _cb_facts(written))
        return kept * 100 >= len(before) * CB_KEEP_PCT


    def _cb_usable(text: str) -> bool:
        body = (text or "").strip()
        if len(body) < CB_MIN_ANSWER_CHARS:
            return False
        if _CB_TOOL_RE.search(body):
            return False
        return True


    def _cb_flatten_markers(text: str) -> str:
        """Bring every marker down to the single-bracket form the board hands out.

    The evaluator resolves `[[n]]` against the delivered citation list and fails
    the whole response on an index it cannot resolve. The board's numbering is
    its own, so no marker it wrote is allowed to reach the platform in the form
    the evaluator checks; the repointing below is what makes them resolvable.
    """
        return _CB_DOUBLE_RE.sub(lambda m: "[" + m.group(1) + "]", text or "")


    def _cb_marker_numbers(blob: str, top: int) -> list[int]:
        out: list[int] = []
        for chunk in (blob or "").split(","):
            ...
            if span:
                ...
                for number in range(low, min(high, low + 16) + 1):
                    if 1 <= number <= top and number not in out:
                        out.append(number)
            elif piece.isdigit():
                number = int(piece)
                if 1 <= number <= top and number not in out:
    ...
    def _cb_deliver(answer: str, rows: list[dict]) -> tuple[str, list[CitationRef]]:
        """Deliver exactly the rows the answer cites, with the markers repointed."""
        refs: list[CitationRef] = []
        position: dict[int, int] = {}
        for match in _CB_MARK_RE.finditer(answer or ""):
            for number in _cb_marker_numbers(match.group(0)[1:-1], len(rows)):
                if number in position or len(refs) >= CB_CITATION_CAP:
                    continue
                row = rows[number - 1]
                ...
                if not receipt or not result:
                    continue
                try:
                    refs.append(CitationRef(receipt_id=receipt, result_id=result))
                except Exception:
                    continue
                position[number] = len(refs)

        def _repoint(match: "re.Match[str]") -> str:
            kept = [position[n] for n in _cb_marker_numbers(match.group(0)[1:-1], len(rows))
                    if n in position]
            if not kept:
                return ""
            return "[" + ",".join(str(n) for n in kept) + "]"

        text = _CB_MARK_RE.sub(_repoint, answer or "")
    ...
    async def _cb_stage(question: str, response: object, deadline: float) -> object:
        """Stage 4 — the board: the answer rebuilt around what it owes.

    Held so that nothing here can cost the run the answer it already has: every
    exit that is not a written, marked, fact-keeping, citable board falls back to
    the response the earlier stages produced.
    """
        try:
            if getattr(response, "output", None) is not None:
                return response
            if _w4_remaining(deadline) < CB_MIN_LEFT_S:
                return response
            draft = _w4_response_text(response)
            slots = await _cb_slots(question, deadline)
            if not slots:
                return response
            rows: list[dict] = []
            await _cb_gather(question, slots, rows, deadline)
            if not rows:
                return response
            bound = _cb_bind(slots, rows)
            open_claims = _cb_open(slots, bound)
            if open_claims and await _cb_fill(question, open_claims, rows, deadline):
                bound = _cb_bind(slots, rows)
            if not any(bound.get(index) for index in range(len(slots))):
                return response
            board = _cb_board(slots, bound, rows)
            written = _cb_flatten_markers(await _cb_write(question, board, deadline))
            if not _cb_usable(written) or not _CB_MARK_RE.search(written):
                return response
            if draft.strip() and not _cb_keeps_facts(draft, written):
                return response
            text, refs = _cb_deliver(written, rows)
            if not refs or not _cb_usable(text):
                return response
            try:
    ...
    """
        ...
        question = getattr(query, "text", "") or ""
        schema = getattr(query, "output_schema", None)
        board = _cb_worth_board(query, question)

        contract = await _w4_build_answer_contract(question, schema, deadline=deadline)
        response = await _w4_research_or_salvage(query, _cb_baseline_budget(board))
        ...
        if board:
            response = await _cb_stage(question, response, deadline)
...
_branch7038560_c85_d1_query_entry = _compose_branch7038560_c85_d1_entry()
...
class Branch0B405CAEC72B:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch0_b405_c_a_e_c72_b_query_entry(query=query)
...
class Branch7038560C85D1:
    async def __call__(self, query: Query, context: ContextSnapshot) -> Response:
        return await _branch7038560_c85_d1_query_entry(query=query)


_MODE_SHAPE_PRIMARY_AGENT = Branch0B405CAEC72B()
_MODE_SHAPE_SECONDARY_AGENT = Branch24ACBE3AF69D()
_MODE_SHAPE_TERTIARY_AGENT = Branch7038560C85D1()
_CANDIDATE_BRANCH_CLASS_NAMES = ("Branch0B405CAEC72B", "Branch24ACBE3AF69D", "Branch7038560C85D1")
