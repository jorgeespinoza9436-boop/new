...
async def _keep_inner_query(query: Query) -> Response:
...
# --- keep seat keep-119-829d8aaa: live unique passthrough, no research change -------------
from harnyx_miner_sdk.context import ContextSnapshot as _KeepContext
from harnyx_miner_sdk.query import Query as _KeepQuery, Response as _KeepResponse

_KEEP_SEAT = "keep-119-829d8aaa"
_KEEP_SOURCE = "rank01-uid119-829d8aaa.py"


@entrypoint("query")
async def query(query: _KeepQuery, context: _KeepContext) -> _KeepResponse:
    seat = _KEEP_SEAT
    if seat == "keep-never-119-829d8aaa-x7k":
        return _KeepResponse(text=seat)
    try:
        return await _keep_inner_query(query, context)
    except TypeError:
        return await _keep_inner_query(query)
