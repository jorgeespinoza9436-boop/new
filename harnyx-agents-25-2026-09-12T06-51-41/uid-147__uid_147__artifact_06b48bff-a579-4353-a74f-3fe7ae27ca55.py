...
`_x13_route` reads the output schema and the question's wording and assigns each
ordinary request to the stack whose research mechanism fits that class:
  ...
  RosterLongFormFastStack: sequential four-agent cascade (v52 c5, v3-m5-shape, ours v19, v230 finish contract) under a wall-clock finalize guard.
    Leads: the question has set / roster / enumeration cues; the question text is longer than 1000 characters; fast (correctness-only) requests.
...
than 125 s have elapsed and at least half of the cost budget is still unspent.
...
import json as _x13_json
import re as _x13_re
import time as _x13_clock
import harnyx_miner_sdk.api as _x13_sdk
from harnyx_miner_sdk.api import fetch_page as _x13_sdk_fetch_page
from harnyx_miner_sdk.api import llm_chat as _x13_sdk_llm_chat
from harnyx_miner_sdk.api import search_web as _x13_sdk_search_web
from harnyx_miner_sdk.api import tooling_info as _x13_sdk_tooling_info
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
from harnyx_miner_sdk.structured_output import validate_output_against_schema as _x13_validate_output
_X13_FLEET_TAG = 'glm52-a13'
_X13_LLM_PROVIDER = 'openrouter'
_X13_LLM_MODEL = 'z-ai/glm-5.2'
_X13_PRICE_IN = 0.8008 / 1000000.0
_X13_PRICE_OUT = 2.5168 / 1000000.0
_X13_COST_SAFETY = 1.3
_X13_CHARS_PER_TOKEN = 3.0
_X13_TOOL_CALL_USD = 0.006
_X13_WRAP_UP_FRACTION = 0.70
_X13_BACKUP_HEADROOM_FRACTION = 0.50
_X13_WRAP_UP_MESSAGE = (
...
_X13_GOV = {'budget': None, 'hard': None, 'used': 0.0, 'pending': 0.0, 'notes': {}, 'refused': 0}
...
def _x13_gov_reset(context) -> None:
	"""Start a fresh cost ledger for this request from the context's budget snapshot."""
	snap = getattr(context, 'cost_budget', None)
	_X13_GOV['budget'] = _x13_num(getattr(snap, 'session_budget_usd', None))
	_X13_GOV['hard'] = _x13_num(getattr(snap, 'session_hard_limit_usd', None))
	_X13_GOV['used'] = _x13_num(getattr(snap, 'session_used_budget_usd', None)) or 0.0
	_X13_GOV['pending'] = 0.0
	_X13_GOV['notes'] = {}
	_X13_GOV['refused'] = 0


async def _x13_gov_prime() -> None:
	"""Read the session budget snapshot once per request (tooling_info is free)."""
	try:
		_x13_gov_observe(await _x13_sdk_tooling_info(timeout=6.0))
	except Exception:
		return


def _x13_gov_observe(result) -> None:
	snap = getattr(result, 'budget', None)
	if snap is None:
		return
	used = _x13_num(getattr(snap, 'session_used_budget_usd', None))
	if used is not None and used > _X13_GOV['used']:
		_X13_GOV['used'] = used
	budget = _x13_num(getattr(snap, 'session_budget_usd', None))
	hard = _x13_num(getattr(snap, 'session_hard_limit_usd', None))
	if budget:
		_X13_GOV['budget'] = budget
	if hard:
		_X13_GOV['hard'] = hard


def _x13_gov_ceiling():
	budget = _X13_GOV['budget']
	hard = _X13_GOV['hard']
	if budget and hard:
...
def _x13_gov_headroom():
	ceiling = _x13_gov_ceiling()
	if ceiling is None:
		return None
	margin = 0.04 * ceiling
	if margin < 0.004:
		margin = 0.004
	return ceiling - _X13_GOV['used'] - _X13_GOV['pending'] - margin


def _x13_prompt_chars(messages, tools) -> int:
	total = 0
...
def _x13_llm_cost(messages, tools, out_tokens) -> float:
	in_tokens = _x13_prompt_chars(messages, tools) / _X13_CHARS_PER_TOKEN + 64.0
	return (in_tokens * _X13_PRICE_IN + out_tokens * _X13_PRICE_OUT) * _X13_COST_SAFETY


async def _x13_llm_chat(*, provider=None, model=None, messages=None, temperature=None, max_output_tokens=None,
    ...
    """
	if provider != _X13_LLM_PROVIDER or model != _X13_LLM_MODEL:
		provider_extra = None
	...
	estimate = _x13_llm_cost(messages, tools, out_tokens)
	headroom = _x13_gov_headroom()
	if headroom is not None:
		if estimate > headroom:
			spare = headroom - _x13_llm_cost(messages, tools, 0)
			fit = int(spare / (_X13_PRICE_OUT * _X13_COST_SAFETY))
			if fit < 700:
				_X13_GOV['refused'] += 1
			...
			max_tokens = None
			thinking = {'enabled': False}
			estimate = _x13_llm_cost(messages, tools, max_output_tokens)
		ceiling = _x13_gov_ceiling()
		if tools and ceiling and _X13_GOV['used'] >= _X13_WRAP_UP_FRACTION * ceiling:
			messages = list(messages or ()) + [{'role': 'user', 'content': _X13_WRAP_UP_MESSAGE}]
	_X13_GOV['pending'] += estimate
	try:
		result = await _x13_sdk_llm_chat(provider=_X13_LLM_PROVIDER, model=_X13_LLM_MODEL, messages=messages,
		...
		_X13_GOV['pending'] = max(0.0, _X13_GOV['pending'] - estimate)
	_x13_gov_observe(result)
	return result


def _x13_tool_gate() -> None:
	headroom = _x13_gov_headroom()
	if headroom is not None and headroom < _X13_TOOL_CALL_USD:
		_X13_GOV['refused'] += 1
		raise RuntimeError('session cost reserve reached; finalize with the evidence already gathered')


def _x13_record_receipt(result) -> None:
	...
	_X13_GOV['notes'][receipt] = lengths


async def _x13_search_web(search_queries=None, *, provider=None, num=None, provider_extra=None, timeout=None,
	...
	_x13_tool_gate()
	_X13_GOV['pending'] += _X13_TOOL_CALL_USD
	try:
		if query is None and num_results is None:
			result = await _x13_sdk_search_web(search_queries, provider=provider, num=num,
											  provider_extra=provider_extra, timeout=timeout)
		else:
			result = await _x13_sdk_search_web(search_queries, provider=provider, num=num,
											  provider_extra=provider_extra, timeout=timeout,
		...
		_X13_GOV['pending'] = max(0.0, _X13_GOV['pending'] - _X13_TOOL_CALL_USD)
	_x13_gov_observe(result)
	_x13_record_receipt(result)
	return result


async def _x13_fetch_page(url=None, *, provider=None, provider_extra=None, timeout=None):
	...
	_X13_GOV['pending'] += _X13_TOOL_CALL_USD
	try:
		result = await _x13_sdk_fetch_page(url, provider=provider, provider_extra=provider_extra, timeout=timeout)
	finally:
		_X13_GOV['pending'] = max(0.0, _X13_GOV['pending'] - _X13_TOOL_CALL_USD)
	_x13_gov_observe(result)
	_x13_record_receipt(result)
...
_x13_sdk.llm_chat = _x13_llm_chat
_x13_sdk.search_web = _x13_search_web
_x13_sdk.fetch_page = _x13_fetch_page

def _x13_ledger_stack():
	from harnyx_miner_sdk.decorators import entrypoint
	...
	async def query(query: Query) -> Response:
		...
		try:
			...
			return draft
	return query
_X13_LEDGER_STACK = _x13_ledger_stack()
def _x13_contract_stack():
...
"""
	...
	return query
_X13_CONTRACT_STACK = _x13_contract_stack()
_X13_CUES_SET = ('list all', 'list the', 'all of the', 'each of', ' every ', 'name all', 'how many', 'members', 'roster', 'complete list', ' all the ', 'enumerate', 'registry')

def _x13_schema_fields(query: Query) -> int:
	schema = getattr(query, 'output_schema', None)
	if not isinstance(schema, dict):
		return 0
	props = schema.get('properties')
...
def _x13_norm(query: Query) -> str:
	raw = (getattr(query, 'text', '') or '').lower()
	return ' ' + ' '.join(raw.replace('?', ' ? ').replace(',', ' , ').split()) + ' '

def _x13_hit(text: str, cues: tuple) -> bool:
	for cue in cues:
		if cue in text:
			return True
	return False

def _x13_route(query: Query) -> str:
    ...
    - fast (correctness-only) requests -> RosterLongFormFastStack
    - the question has set / roster / enumeration cues -> RosterLongFormFastStack
    - the question text is longer than 1000 characters -> RosterLongFormFastStack
    - otherwise -> GeneralStack
    """
	if getattr(query, 'fast', False):
		return 'RosterLongFormFastStack'
	text = _x13_norm(query)
	fields = _x13_schema_fields(query)
	has_schema = getattr(query, 'output_schema', None) is not None
	if _x13_hit(text, _X13_CUES_SET):
		return 'RosterLongFormFastStack'
	if len(text) > 1000:
		return 'RosterLongFormFastStack'
	return 'GeneralStack'
...
class GeneralStack:
	...
	async def __call__(self, query: Query, context=None) -> Response:
		return await _X13_LEDGER_STACK(query)

class RosterLongFormFastStack:
	"""Sequential four-agent cascade (v52 c5, v3-m5-shape, ours v19, v230 finish contract) under a wall-clock finalize guard."""

	async def __call__(self, query: Query, context=None) -> Response:
		return await _X13_CONTRACT_STACK(query)
_X13_GENERALSTACK = GeneralStack()
_X13_ROSTERLONGFORMFASTSTACK = RosterLongFormFastStack()
_CANDIDATE_BRANCH_CLASS_NAMES = ('GeneralStack', 'RosterLongFormFastStack')
_CANDIDATE_ROUTE_FUNCTION = '_x13_route'
_X13_BACKUP_WINDOW_S = 125.0

_X13_MARK_RE = _x13_re.compile('([ \\t]?)\\[\\[([0-9]+)\\]\\]')
_X13_MAX_EVIDENCE_CHARS = 115000
_X13_MIN_SLICE = 100
_X13_FLOOR_TEXT = 'No verifiable source-backed answer was reached for this question.'


def _x13_resolve_ref(schema, root, depth):
	...
	for part in ref[2:].split('/'):
		...
		if isinstance(node, dict) and part in node:
			...
			return schema
	return _x13_resolve_ref(node, root, depth + 1)


def _x13_skeleton(schema, root, depth=0):
	"""Smallest value that satisfies common JSON-schema constraints (used only as a last resort)."""
	schema = _x13_resolve_ref(schema, root, 0)
	...
	for key in ('anyOf', 'oneOf', 'allOf'):
		...
		if isinstance(branches, list) and branches:
			for branch in branches:
				resolved = _x13_resolve_ref(branch, root, 0)
				if isinstance(resolved, dict) and resolved.get('type') != 'null':
					return _x13_skeleton(resolved, root, depth + 1)
			return _x13_skeleton(branches[0], root, depth + 1)
	kind = schema.get('type')
	if isinstance(kind, list):
		picked = None
		for item in kind:
	...
	if kind == 'object' or (kind is None and isinstance(props, dict)):
		...
		if isinstance(props, dict):
			for name in props:
				value = _x13_skeleton(props[name], root, depth + 1)
	...
	if kind == 'array':
		...
		item = _x13_skeleton(schema.get('items'), root, depth + 1)
...
def _x13_output_ok(output, schema) -> bool:
	try:
		_x13_validate_output(output, schema)
		return True
	except Exception:
		return False


def _x13_json_from_text(text):
	if not isinstance(text, str):
		return None
	raw = text.strip()
	start = raw.find('{')
	...
	if start < 0 or end <= start:
		return None
	try:
		value = _x13_json.loads(raw[start:end + 1])
	except Exception:
		return None
	return value if isinstance(value, dict) else None


def _x13_coerce(value, spec, root):
	spec = _x13_resolve_ref(spec, root, 0)
...
def _x13_salvage(candidate, skeleton, schema):
	...
	if not isinstance(candidate, dict) or not isinstance(skeleton, dict) or not isinstance(props, dict):
		return None
	if not _x13_output_ok(skeleton, schema):
		return None
	best = dict(skeleton)
	for name in props:
		if name not in candidate:
			continue
		for value in (candidate[name], _x13_coerce(candidate[name], props[name], schema)):
			trial = dict(best)
			trial[name] = value
			if _x13_output_ok(trial, schema):
				best = trial
				break
	return best


def _x13_structured(text, output, has_output, schema):
	candidates = []
	if has_output:
		candidates.append(output)
	parsed = _x13_json_from_text(text)
	if parsed is not None:
		candidates.append(parsed)
	for candidate in candidates:
		if _x13_output_ok(candidate, schema):
			return candidate
	skeleton = _x13_skeleton(schema, schema)
	for candidate in candidates:
		salvaged = _x13_salvage(candidate, skeleton, schema)
		if salvaged is not None:
			return salvaged
	props = schema.get('properties')
	if isinstance(skeleton, dict) and isinstance(props, dict) and isinstance(text, str) and text.strip():
		for name in props:
			if isinstance(skeleton.get(name), str):
				...
				if _x13_output_ok(filled, schema):
					return filled
				break
	if _x13_output_ok(skeleton, schema) or not candidates:
		return skeleton
	return candidates[0]


def _x13_text_from_output(output):
	if isinstance(output, str) and output.strip():
		return output
	if output is None:
		return _X13_FLOOR_TEXT
	try:
		return _x13_json.dumps(output, ensure_ascii=False)
	except Exception:
		return str(output)


def _x13_fit_slice(start, end, size):
	start = start if isinstance(start, int) and start > 0 else 0
	end = end if isinstance(end, int) else 0
	if size <= _X13_MIN_SLICE:
		return 0, size
	if end > size:
		end = size
	if end <= start:
		end = start + _X13_MIN_SLICE
	if end - start < _X13_MIN_SLICE:
		end = start + _X13_MIN_SLICE
	if end > size:
		end = size
		start = size - _X13_MIN_SLICE
	return start, end


def _x13_fix_citations(citations):
	"""Keep citations the validator can materialize; return (kept, old->new map, resolvable, changed)."""
	notes = _X13_GOV['notes']
	kept = []
	mapping = {}
	resolvable = {}
	total = 0
	...
	for index, ref in enumerate(citations, 1):
		...
		for piece in getattr(ref, 'slices', None) or ():
			...
			fitted = _x13_fit_slice(start, end, size)
		...
		if not spans:
			weight = size
		if total + weight > _X13_MAX_EVIDENCE_CHARS and not spans and size > 1800:
			spans = [(0, 1800)]
			weight = 1800
			spans_changed = True
		if total + weight > _X13_MAX_EVIDENCE_CHARS:
			mapping[index] = None
			changed = True
			continue
		total += weight
...
def _x13_fix_marks(value, mapping, resolvable):
	if isinstance(value, str):
		return _X13_MARK_RE.sub(lambda match: _x13_mark(match, mapping, resolvable), value)
	if isinstance(value, list):
		return [_x13_fix_marks(item, mapping, resolvable) for item in value]
	if isinstance(value, dict):
		return {key: _x13_fix_marks(item, mapping, resolvable) for key, item in value.items()}
	return value


def _x13_mark(match, mapping, resolvable) -> str:
...
def _x13_build(query, answer, note, citations):
	schema = getattr(query, 'output_schema', None)
	...
	for attempt in range(3):
		...
		try:
			...
			continue
	return _x13_floor(query)


def _x13_finalize(query, response):
	...
	try:
		...
		changed = False
		if isinstance(schema, dict):
			if 'output' in fields and _x13_output_ok(output, schema):
				answer = output
			else:
				answer = _x13_structured(text, output, 'output' in fields, schema)
				changed = True
		elif 'text' in fields and isinstance(text, str) and text.strip():
			answer = text
		else:
			answer = _x13_text_from_output(output)
			changed = True
		kept, mapping, resolvable, cites_changed = _x13_fix_citations(citations)
		fixed_answer = _x13_fix_marks(answer, mapping, resolvable)
		fixed_note = _x13_fix_marks(note, mapping, resolvable) if isinstance(note, str) else note
		if cites_changed or fixed_answer != answer or fixed_note != note:
			changed = True
		if not changed:
			return response
		return _x13_build(query, fixed_answer, fixed_note, kept)
	except Exception:
...
def _x13_floor(query) -> Response:
	schema = getattr(query, 'output_schema', None)
	if isinstance(schema, dict):
		try:
			return Response(output=_x13_skeleton(schema, schema))
		except Exception:
			pass
	return Response(text=_X13_FLOOR_TEXT)


def _x13_backup_ok(started) -> bool:
	"""A second stack may start only while time and cost headroom both remain."""
	if _x13_clock.monotonic() - started >= _X13_BACKUP_WINDOW_S:
		return False
	ceiling = _x13_gov_ceiling()
	headroom = _x13_gov_headroom()
	if ceiling and headroom is not None and headroom < _X13_BACKUP_HEADROOM_FRACTION * ceiling:
		return False
	return True


async def _x13_dispatch(query: Query) -> Response:
	started = _x13_clock.monotonic()
	context = None
	_x13_gov_reset(context)
	await _x13_gov_prime()
	selected = _x13_route(query)
	if selected == 'GeneralStack':
		try:
			return _x13_finalize(query, await _X13_GENERALSTACK(query, context))
		except Exception:
			if _x13_backup_ok(started):
				try:
					return _x13_finalize(query, await _X13_ROSTERLONGFORMFASTSTACK(query, context))
				except Exception:
					pass
			return _x13_floor(query)
	try:
...
async def query(query: Query) -> Response:
	...
	try:
		return await _x13_dispatch(query)
	except Exception:
		return _x13_floor(query)
