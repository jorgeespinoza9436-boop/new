...
def _compose_meadow_beacon_agent_entry():
...
_meadow_beacon_agent_query_entry = _compose_meadow_beacon_agent_entry()


_SHAPE_ROUTER_SEED = "d868f3277b89cc2d5c4f418c"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if shape == 0:
        return "CobaltAnvilAgent"
    if shape == 1:
        return "MeadowBeaconAgent"
...
class MeadowBeaconAgent:
    async def __call__(self, query: Query) -> Response:
        return await _meadow_beacon_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = BasaltRelayAgent()
_SHAPE_SECONDARY_AGENT = CobaltAnvilAgent()
_SHAPE_TERTIARY_AGENT = MeadowBeaconAgent()
