...
`_x15_route` reads the output schema and the question's wording and assigns each
ordinary request to the stack whose research mechanism fits that class:

  GeneralStack: schema/analytical router over a staged-protocol agent and two w4 answer-contract tool loops (v260-14, v270-12).
    Leads: every other ordinary request.
  FieldRosterFastStack: sequential four-agent cascade (v52 c5, v260-14, ours v16, v52) under a wall-clock finalize guard.
    Leads: the output_schema has 2 or more fields; the question has set / roster / enumeration cues; fast (correctness-only) requests.
...
import json as _x15_json
import re as _x15_re
import time as _x15_clock
import harnyx_miner_sdk.api as _x15_sdk
from harnyx_miner_sdk.api import fetch_page as _x15_sdk_fetch_page
from harnyx_miner_sdk.api import llm_chat as _x15_sdk_llm_chat
from harnyx_miner_sdk.api import search_web as _x15_sdk_search_web
from harnyx_miner_sdk.api import tooling_info as _x15_sdk_tooling_info
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef, CitationSlice, Query, Response
from harnyx_miner_sdk.structured_output import validate_output_against_schema as _x15_validate_output
_X15_FLEET_TAG = 'glm52-a15'
_X15_LLM_PROVIDER = 'openrouter'
_X15_LLM_MODEL = 'z-ai/glm-5.2'
_X15_PRICE_IN = 0.8008 / 1000000.0
_X15_PRICE_OUT = 2.5168 / 1000000.0
_X15_COST_SAFETY = 1.3
_X15_CHARS_PER_TOKEN = 3.0
_X15_TOOL_CALL_USD = 0.006
_X15_WRAP_UP_FRACTION = 0.70
_X15_BACKUP_HEADROOM_FRACTION = 0.50
_X15_WRAP_UP_MESSAGE = (
...
_X15_GOV = {'budget': None, 'hard': None, 'used': 0.0, 'pending': 0.0, 'notes': {}, 'refused': 0}
...
def _x15_gov_reset(context) -> None:
	"""Start a fresh cost ledger for this request from the context's budget snapshot."""
	snap = getattr(context, 'cost_budget', None)
	_X15_GOV['budget'] = _x15_num(getattr(snap, 'session_budget_usd', None))
	_X15_GOV['hard'] = _x15_num(getattr(snap, 'session_hard_limit_usd', None))
	_X15_GOV['used'] = _x15_num(getattr(snap, 'session_used_budget_usd', None)) or 0.0
	_X15_GOV['pending'] = 0.0
	_X15_GOV['notes'] = {}
	_X15_GOV['refused'] = 0


async def _x15_gov_prime() -> None:
	"""Read the session budget snapshot once per request (tooling_info is free)."""
	try:
		_x15_gov_observe(await _x15_sdk_tooling_info(timeout=6.0))
	except Exception:
		return


def _x15_gov_observe(result) -> None:
	snap = getattr(result, 'budget', None)
	if snap is None:
		return
	used = _x15_num(getattr(snap, 'session_used_budget_usd', None))
	if used is not None and used > _X15_GOV['used']:
		_X15_GOV['used'] = used
	budget = _x15_num(getattr(snap, 'session_budget_usd', None))
	hard = _x15_num(getattr(snap, 'session_hard_limit_usd', None))
	if budget:
		_X15_GOV['budget'] = budget
	if hard:
		_X15_GOV['hard'] = hard


def _x15_gov_ceiling():
	budget = _X15_GOV['budget']
	hard = _X15_GOV['hard']
	if budget and hard:
...
def _x15_gov_headroom():
	ceiling = _x15_gov_ceiling()
	if ceiling is None:
		return None
	margin = 0.04 * ceiling
	if margin < 0.004:
		margin = 0.004
	return ceiling - _X15_GOV['used'] - _X15_GOV['pending'] - margin


def _x15_prompt_chars(messages, tools) -> int:
	total = 0
...
def _x15_llm_cost(messages, tools, out_tokens) -> float:
	in_tokens = _x15_prompt_chars(messages, tools) / _X15_CHARS_PER_TOKEN + 64.0
	return (in_tokens * _X15_PRICE_IN + out_tokens * _X15_PRICE_OUT) * _X15_COST_SAFETY


async def _x15_llm_chat(*, provider=None, model=None, messages=None, temperature=None, max_output_tokens=None,
    ...
    """
	if provider != _X15_LLM_PROVIDER or model != _X15_LLM_MODEL:
		provider_extra = None
	...
	estimate = _x15_llm_cost(messages, tools, out_tokens)
	headroom = _x15_gov_headroom()
	if headroom is not None:
		if estimate > headroom:
			spare = headroom - _x15_llm_cost(messages, tools, 0)
			fit = int(spare / (_X15_PRICE_OUT * _X15_COST_SAFETY))
			if fit < 700:
				_X15_GOV['refused'] += 1
			...
			max_tokens = None
			thinking = {'enabled': False}
			estimate = _x15_llm_cost(messages, tools, max_output_tokens)
		ceiling = _x15_gov_ceiling()
		if tools and ceiling and _X15_GOV['used'] >= _X15_WRAP_UP_FRACTION * ceiling:
			messages = list(messages or ()) + [{'role': 'user', 'content': _X15_WRAP_UP_MESSAGE}]
	_X15_GOV['pending'] += estimate
	try:
		result = await _x15_sdk_llm_chat(provider=_X15_LLM_PROVIDER, model=_X15_LLM_MODEL, messages=messages,
		...
		_X15_GOV['pending'] = max(0.0, _X15_GOV['pending'] - estimate)
	_x15_gov_observe(result)
	return result


def _x15_tool_gate() -> None:
	headroom = _x15_gov_headroom()
	if headroom is not None and headroom < _X15_TOOL_CALL_USD:
		_X15_GOV['refused'] += 1
		raise RuntimeError('session cost reserve reached; finalize with the evidence already gathered')


def _x15_record_receipt(result) -> None:
	...
	_X15_GOV['notes'][receipt] = lengths


async def _x15_search_web(search_queries=None, *, provider=None, num=None, provider_extra=None, timeout=None,
	...
	_x15_tool_gate()
	_X15_GOV['pending'] += _X15_TOOL_CALL_USD
	try:
		if query is None and num_results is None:
			result = await _x15_sdk_search_web(search_queries, provider=provider, num=num,
											  provider_extra=provider_extra, timeout=timeout)
		else:
			result = await _x15_sdk_search_web(search_queries, provider=provider, num=num,
											  provider_extra=provider_extra, timeout=timeout,
		...
		_X15_GOV['pending'] = max(0.0, _X15_GOV['pending'] - _X15_TOOL_CALL_USD)
	_x15_gov_observe(result)
	_x15_record_receipt(result)
	return result


async def _x15_fetch_page(url=None, *, provider=None, provider_extra=None, timeout=None):
	...
	_X15_GOV['pending'] += _X15_TOOL_CALL_USD
	try:
		result = await _x15_sdk_fetch_page(url, provider=provider, provider_extra=provider_extra, timeout=timeout)
	finally:
		_X15_GOV['pending'] = max(0.0, _X15_GOV['pending'] - _X15_TOOL_CALL_USD)
	_x15_gov_observe(result)
	_x15_record_receipt(result)
...
_x15_sdk.llm_chat = _x15_llm_chat
_x15_sdk.search_web = _x15_search_web
_x15_sdk.fetch_page = _x15_fetch_page

def _x15_analytic_stack():
	from harnyx_miner_sdk.decorators import entrypoint
	...
	return query
_X15_ANALYTIC_STACK = _x15_analytic_stack()
def _x15_relay_stack():
...
"""
	...
	return query
_X15_RELAY_STACK = _x15_relay_stack()
_X15_CUES_SET = ('list all', 'all of the', 'which of the', 'each of', 'how many', 'members', 'complete list', ' all the ', 'which countries', 'table of', 'registry')

def _x15_schema_fields(query: Query) -> int:
	schema = getattr(query, 'output_schema', None)
	if not isinstance(schema, dict):
		return 0
	props = schema.get('properties')
...
def _x15_norm(query: Query) -> str:
	raw = (getattr(query, 'text', '') or '').lower()
	return ' ' + ' '.join(raw.replace('?', ' ? ').replace(',', ' , ').split()) + ' '

def _x15_hit(text: str, cues: tuple) -> bool:
	for cue in cues:
		if cue in text:
			return True
	return False

def _x15_route(query: Query) -> str:
    ...
    - fast (correctness-only) requests -> FieldRosterFastStack
    - the output_schema has 2 or more fields -> FieldRosterFastStack
    - the question has set / roster / enumeration cues -> FieldRosterFastStack
    - otherwise -> GeneralStack
    """
	if getattr(query, 'fast', False):
		return 'FieldRosterFastStack'
	text = _x15_norm(query)
	fields = _x15_schema_fields(query)
	has_schema = getattr(query, 'output_schema', None) is not None
	if fields >= 2:
		return 'FieldRosterFastStack'
	if _x15_hit(text, _X15_CUES_SET):
		return 'FieldRosterFastStack'
	return 'GeneralStack'

class GeneralStack:
	"""Schema/analytical router over a staged-protocol agent and two w4 answer-contract tool loops (v260-14, v270-12)."""

	async def __call__(self, query: Query, context=None) -> Response:
		return await _X15_ANALYTIC_STACK(query)

class FieldRosterFastStack:
	"""Sequential four-agent cascade (v52 c5, v260-14, ours v16, v52) under a wall-clock finalize guard."""

	async def __call__(self, query: Query, context=None) -> Response:
		return await _X15_RELAY_STACK(query)
_X15_GENERALSTACK = GeneralStack()
_X15_FIELDROSTERFASTSTACK = FieldRosterFastStack()
_CANDIDATE_BRANCH_CLASS_NAMES = ('GeneralStack', 'FieldRosterFastStack')
_CANDIDATE_ROUTE_FUNCTION = '_x15_route'
_X15_BACKUP_WINDOW_S = 115.0

_X15_MARK_RE = _x15_re.compile('([ \\t]?)\\[\\[([0-9]+)\\]\\]')
_X15_MAX_EVIDENCE_CHARS = 115000
_X15_MIN_SLICE = 100
_X15_FLOOR_TEXT = 'No verifiable source-backed answer was reached for this question.'


def _x15_resolve_ref(schema, root, depth):
	...
	for part in ref[2:].split('/'):
		...
		if isinstance(node, dict) and part in node:
			...
			return schema
	return _x15_resolve_ref(node, root, depth + 1)


def _x15_skeleton(schema, root, depth=0):
	"""Smallest value that satisfies common JSON-schema constraints (used only as a last resort)."""
	schema = _x15_resolve_ref(schema, root, 0)
	...
	for key in ('anyOf', 'oneOf', 'allOf'):
		...
		if isinstance(branches, list) and branches:
			for branch in branches:
				resolved = _x15_resolve_ref(branch, root, 0)
				if isinstance(resolved, dict) and resolved.get('type') != 'null':
					return _x15_skeleton(resolved, root, depth + 1)
			return _x15_skeleton(branches[0], root, depth + 1)
	kind = schema.get('type')
	if isinstance(kind, list):
		picked = None
		for item in kind:
	...
	if kind == 'object' or (kind is None and isinstance(props, dict)):
		...
		if isinstance(props, dict):
			for name in props:
				value = _x15_skeleton(props[name], root, depth + 1)
	...
	if kind == 'array':
		...
		item = _x15_skeleton(schema.get('items'), root, depth + 1)
...
def _x15_output_ok(output, schema) -> bool:
	try:
		_x15_validate_output(output, schema)
		return True
	except Exception:
		return False


def _x15_json_from_text(text):
	...
	if start < 0 or end <= start:
		return None
	try:
		value = _x15_json.loads(raw[start:end + 1])
	except Exception:
		return None
	return value if isinstance(value, dict) else None


def _x15_coerce(value, spec, root):
	spec = _x15_resolve_ref(spec, root, 0)
...
def _x15_salvage(candidate, skeleton, schema):
	...
	if not isinstance(candidate, dict) or not isinstance(skeleton, dict) or not isinstance(props, dict):
		return None
	if not _x15_output_ok(skeleton, schema):
		return None
	best = dict(skeleton)
	for name in props:
		if name not in candidate:
			continue
		for value in (candidate[name], _x15_coerce(candidate[name], props[name], schema)):
			trial = dict(best)
			trial[name] = value
			if _x15_output_ok(trial, schema):
				best = trial
				break
	return best


def _x15_structured(text, output, has_output, schema):
	candidates = []
	if has_output:
		candidates.append(output)
	parsed = _x15_json_from_text(text)
	if parsed is not None:
		candidates.append(parsed)
	for candidate in candidates:
		if _x15_output_ok(candidate, schema):
			return candidate
	skeleton = _x15_skeleton(schema, schema)
	for candidate in candidates:
		salvaged = _x15_salvage(candidate, skeleton, schema)
		if salvaged is not None:
			return salvaged
	props = schema.get('properties')
	if isinstance(skeleton, dict) and isinstance(props, dict) and isinstance(text, str) and text.strip():
		for name in props:
			if isinstance(skeleton.get(name), str):
				...
				if _x15_output_ok(filled, schema):
					return filled
				break
	if _x15_output_ok(skeleton, schema) or not candidates:
		return skeleton
	return candidates[0]


def _x15_text_from_output(output):
	if isinstance(output, str) and output.strip():
		return output
	if output is None:
		return _X15_FLOOR_TEXT
	try:
		return _x15_json.dumps(output, ensure_ascii=False)
	except Exception:
		return str(output)


def _x15_fit_slice(start, end, size):
	start = start if isinstance(start, int) and start > 0 else 0
	end = end if isinstance(end, int) else 0
	if size <= _X15_MIN_SLICE:
		return 0, size
	if end > size:
		end = size
	if end <= start:
		end = start + _X15_MIN_SLICE
	if end - start < _X15_MIN_SLICE:
		end = start + _X15_MIN_SLICE
	if end > size:
		end = size
		start = size - _X15_MIN_SLICE
	return start, end


def _x15_fix_citations(citations):
	"""Keep citations the validator can materialize; return (kept, old->new map, resolvable, changed)."""
	notes = _X15_GOV['notes']
	kept = []
	mapping = {}
	resolvable = {}
	total = 0
	...
	for index, ref in enumerate(citations, 1):
		...
		for piece in getattr(ref, 'slices', None) or ():
			...
			fitted = _x15_fit_slice(start, end, size)
		...
		if not spans:
			weight = size
		if total + weight > _X15_MAX_EVIDENCE_CHARS and not spans and size > 1800:
			spans = [(0, 1800)]
			weight = 1800
			spans_changed = True
		if total + weight > _X15_MAX_EVIDENCE_CHARS:
			mapping[index] = None
			changed = True
			continue
		total += weight
...
def _x15_fix_marks(value, mapping, resolvable):
	if isinstance(value, str):
		return _X15_MARK_RE.sub(lambda match: _x15_mark(match, mapping, resolvable), value)
	if isinstance(value, list):
		return [_x15_fix_marks(item, mapping, resolvable) for item in value]
	if isinstance(value, dict):
		return {key: _x15_fix_marks(item, mapping, resolvable) for key, item in value.items()}
	return value


def _x15_mark(match, mapping, resolvable) -> str:
...
def _x15_build(query, answer, note, citations):
	schema = getattr(query, 'output_schema', None)
	...
	for attempt in range(3):
		...
		try:
			...
			continue
	return _x15_floor(query)


def _x15_finalize(query, response):
	...
	try:
		...
		changed = False
		if isinstance(schema, dict):
			if 'output' in fields and _x15_output_ok(output, schema):
				answer = output
			else:
				answer = _x15_structured(text, output, 'output' in fields, schema)
				changed = True
		elif 'text' in fields and isinstance(text, str) and text.strip():
			answer = text
		else:
			answer = _x15_text_from_output(output)
			changed = True
		kept, mapping, resolvable, cites_changed = _x15_fix_citations(citations)
		fixed_answer = _x15_fix_marks(answer, mapping, resolvable)
		fixed_note = _x15_fix_marks(note, mapping, resolvable) if isinstance(note, str) else note
		if cites_changed or fixed_answer != answer or fixed_note != note:
			changed = True
		if not changed:
			return response
		return _x15_build(query, fixed_answer, fixed_note, kept)
	except Exception:
...
def _x15_floor(query) -> Response:
	schema = getattr(query, 'output_schema', None)
	if isinstance(schema, dict):
		try:
			return Response(output=_x15_skeleton(schema, schema))
		except Exception:
			pass
	return Response(text=_X15_FLOOR_TEXT)


def _x15_backup_ok(started) -> bool:
	"""A second stack may start only while time and cost headroom both remain."""
	if _x15_clock.monotonic() - started >= _X15_BACKUP_WINDOW_S:
		return False
	ceiling = _x15_gov_ceiling()
	headroom = _x15_gov_headroom()
	if ceiling and headroom is not None and headroom < _X15_BACKUP_HEADROOM_FRACTION * ceiling:
		return False
	return True


async def _x15_dispatch(query: Query) -> Response:
	started = _x15_clock.monotonic()
	context = None
	_x15_gov_reset(context)
	await _x15_gov_prime()
	selected = _x15_route(query)
	if selected == 'GeneralStack':
		try:
			return _x15_finalize(query, await _X15_GENERALSTACK(query, context))
		except Exception:
			if _x15_backup_ok(started):
				try:
					return _x15_finalize(query, await _X15_FIELDROSTERFASTSTACK(query, context))
				except Exception:
					pass
			return _x15_floor(query)
	try:
...
async def query(query: Query) -> Response:
	...
	try:
		return await _x15_dispatch(query)
	except Exception:
		return _x15_floor(query)
