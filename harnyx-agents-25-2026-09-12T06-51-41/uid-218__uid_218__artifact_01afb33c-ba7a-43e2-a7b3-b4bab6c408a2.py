...
async def _ns_base_query(query: Query) -> Response:
...
# --- ns repair: roster-completeness fill (start) ---
# Ordinary-path stage around the base entrypoint (now _ns_base_query):
#   base research -> draft
#   -> collapsed-extract repair when every structured field is the same token
#   -> roster-completeness fill gate on text drafts only
#   -> otherwise the draft is returned unchanged
# Timeouts and exceptions fail open to the base draft.
import json as _ns_json
import re as _ns_re
from time import monotonic as _ns_monotonic
from harnyx_miner_sdk.api import llm_chat as _ns_llm_chat
from harnyx_miner_sdk.api import search_web as _ns_search_web
from harnyx_miner_sdk.decorators import entrypoint
from harnyx_miner_sdk.query import CitationRef as _NSCitationRef
from harnyx_miner_sdk.query import CitationSlice as _NSCitationSlice
from harnyx_miner_sdk.query import Query as _NSQuery
from harnyx_miner_sdk.query import Response as _NSResponse

_NS_TAG = "nosing"
NOSING_VERSION = "v2-nosing-uid8-roster-repair"
_NS_SKIP_AFTER_S = 90.0
_NS_HARD_DEADLINE_S = 258.0
_NS_LLM_PROVIDER = "openrouter"
_NS_REPAIR_MODEL = "z-ai/glm-5.3-flash"
_NS_SEARCH_PROVIDER = "parallel"
_NS_THINKING = {"enabled": True, "effort": "low"}
_NS_SAME_TOKENS = frozenset((
    "", "cambridge", "n/a", "na", "none", "unknown", "null", "-", "tbd",
))
_NS_PTR_RE = _ns_re.compile(r"\[\[(\d+)\]\]")
_NS_NUM_RE = _ns_re.compile(r"\d+(?:[.,]\d+)*")


class _NSGap:
    """Research gap the roster-completeness fill gate decided is still open."""

    def __init__(self, kind: str, query_text: str) -> None:
        self.kind = kind
        self.query_text = query_text

    def is_open(self) -> bool:
        return bool(self.kind) and bool(self.query_text)


def _ns_llm_text(result) -> str:
    if result is None:
        return ""
    resp = getattr(result, "llm", None) or getattr(result, "response", None) or result
...
def _ns_parse_json(text: str) -> dict:
    if not text:
        return {}
    stripped = text.strip()
    fenced = _ns_re.search(r"```(?:json)?\s*(\{.*\})\s*```", stripped, _ns_re.S)
    if fenced:
        stripped = fenced.group(1)
    try:
        parsed = _ns_json.loads(stripped)
        ...
        if start < 0 or end <= start:
            return {}
        try:
            parsed = _ns_json.loads(stripped[start:end + 1])
            return parsed if isinstance(parsed, dict) else {}
        except Exception:
            return {}


def _ns_collapsed(output) -> bool:
    """True when every structured field is empty or the same dummy token."""
    if not isinstance(output, dict) or len(output) < 2:
        return False
    values = []
    ...
    if len(uniq) == 1:
        return True
    if all(item in _NS_SAME_TOKENS for item in values):
        return True
    return False


def _ns_schema_keys(query) -> list:
    schema = getattr(query, "output_schema", None)
...
async def _ns_chat(system: str, user: str, *, max_tokens: int, timeout: float) -> str:
    if timeout <= 3.0:
        return ""
    messages = [{"role": "system", "content": system},
                {"role": "user", "content": user}]
    try:
        result = await _ns_llm_chat(
            provider=_NS_LLM_PROVIDER, model=_NS_REPAIR_MODEL, messages=messages,
            temperature=0.0, max_output_tokens=max_tokens, timeout=timeout,
            thinking=_NS_THINKING)
    except Exception:
        return ""
    return _ns_llm_text(result)


async def _ns_repair_output(query, response, deadline: float):
    output = getattr(response, "output", None)
    if not _ns_collapsed(output):
        return response
    keys = _ns_schema_keys(query) or list(output.keys())
    question = str(getattr(query, "text", "") or "")
    timeout = min(12.0, deadline - _ns_monotonic())
    raw = await _ns_chat(
        "Fill a structured research extract. Return JSON only, one key per "
        "requested field. Use distinct values taken from the question when "
        "they are stated there. Never copy the same token into every field. "
        "If a field is unknown, use an empty string for that field only.",
        "Question:\n" + question[:4000]
        + "\n\nCollapsed extract (do not repeat this collapse):\n"
        + _ns_json.dumps(output)[:2000]
        + "\n\nRequired keys: " + ", ".join(keys[:24]),
        max_tokens=900, timeout=timeout,
    )
    payload = _ns_parse_json(raw)
    if not payload or _ns_collapsed(payload):
        return response
    filled = {}
    for key in keys:
        value = payload.get(key, output.get(key))
        filled[key] = value
    if _ns_collapsed(filled):
        return response
    try:
        return _NSResponse(output=filled, citations=getattr(response, "citations", None))
    except Exception:
        return response



_NS_ROSTER_RE = _ns_re.compile(
    r"\b(?:each|every|all (?:of )?(?:the )?(?:items|entries|members|states|"
    r"countries|agencies|names)|list (?:the|all|every)|roster|enumerate|"
    r"name (?:the|each|every))\b", _ns_re.I)
_NS_ITEM_RE = _ns_re.compile(r"(?:^|\n)\s*(?:[-*]|\d+\.)\s+\S")


def _ns_asked_count(question: str):
    match = _ns_re.search(r"\b(\d{1,2})\b", question or "")
    if not match:
        return None
...
def _ns_find_gap(question: str, draft: str):
    """Open when a roster question returns fewer items than asked."""
    if not _NS_ROSTER_RE.search(question):
        return None
    asked = _ns_asked_count(question)
    items = _NS_ITEM_RE.findall(draft)
    if asked is not None and len(items) >= asked:
        return None
    if asked is None and len(items) >= 3:
        return None
    return _NSGap("roster_short", question[:240])


def _ns_maybe_rewrite(question: str, draft: str, gap, offset: int, total: int) -> str:
    if total <= offset or _NS_PTR_RE.search(draft):
        return ""
    return draft.rstrip() + " [[" + str(offset + 1) + "]]"



async def _ns_search_cite(query_text: str, timeout: float):
    q = (query_text or "").strip()[:300]
    if not q or timeout <= 3.0:
        return []
    try:
        packet = await _ns_search_web(
            q, provider=_NS_SEARCH_PROVIDER, num=4, timeout=min(8.0, timeout))
    except Exception:
        return []
    ...
    for row in rows[:4]:
        ...
        slices = []
        if note.strip():
            slices.append(_NSCitationSlice(start=0, end=min(len(note), 520)))
        try:
            cite = _NSCitationRef(receipt_id=receipt_id, result_id=result_id, slices=slices)
        except Exception:
            cite = None
        if cite is not None:
            cites.append(cite)
...
def _ns_merge_citations(existing, added):
    merged = []
    seen = set()
    for cite in list(existing or []) + list(added or []):
        ...
        seen.add(key)
        merged.append(cite)
    return merged or list(existing or [])


async def _ns_cycle(query, response, started: float):
    output = getattr(response, "output", None)
    deadline = started + _NS_HARD_DEADLINE_S
    if deadline - _ns_monotonic() < 10.0:
        return response
    if output is not None:
        try:
            return await _ns_repair_output(query, response, deadline)
        except Exception:
            return response
    ...
    if not question or not draft:
        return response
    gap = _ns_find_gap(question, draft)
    if gap is None or not gap.is_open():
        return response
    new_cites = await _ns_search_cite(gap.query_text, deadline - _ns_monotonic())
    if not new_cites:
        return response
    existing = list(getattr(response, "citations", None) or [])
    merged = _ns_merge_citations(existing, new_cites)
    new_text = _ns_maybe_rewrite(question, draft, gap, len(existing), len(merged))
    text = new_text or draft
    try:
        return _NSResponse(text=text, citations=merged)
    except Exception:
        return response


@entrypoint("query")
async def query(query: _NSQuery) -> _NSResponse:
    _ns_started = _ns_monotonic()
    response = await _ns_base_query(query)
    try:
        if bool(getattr(query, "fast", False)):
            return response
        if _ns_monotonic() - _ns_started >= _NS_SKIP_AFTER_S:
            return response
        return await _ns_cycle(query, response, _ns_started)
    except Exception:
        return response
# --- ns repair: roster-completeness fill (end) ---
