...
def _compose_cobalt_quill_agent_entry():
...
_cobalt_quill_agent_query_entry = _compose_cobalt_quill_agent_entry()


def _compose_willow_lantern_agent_entry():
...
_willow_lantern_agent_query_entry = _compose_willow_lantern_agent_entry()


_SHAPE_ROUTER_SEED = "f80632dd8a1df10028c32f5e"
...
def _balanced_route_label(query: Query) -> str:
    ...
    if shape == 0:
        return "CobaltQuillAgent"
    if shape == 1:
        return "WillowLanternAgent"
...
class CobaltQuillAgent:
    async def __call__(self, query: Query) -> Response:
        return await _cobalt_quill_agent_query_entry(query)


class WillowLanternAgent:
    async def __call__(self, query: Query) -> Response:
        return await _willow_lantern_agent_query_entry(query)


_SHAPE_PRIMARY_AGENT = SaffronLanternAgent()
_SHAPE_SECONDARY_AGENT = CobaltQuillAgent()
_SHAPE_TERTIARY_AGENT = WillowLanternAgent()
