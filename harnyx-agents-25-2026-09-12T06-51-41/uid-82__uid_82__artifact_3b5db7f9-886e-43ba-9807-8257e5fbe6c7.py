...
def _zbgvqvvvra():
    ...
    _CB_SINGLE_RE = re.compile(r"(?<!\[)\[(\d[\d,\s\-]*)\](?!\])")

    def _cb_valid_slots(corpo: str, entregues: int) -> list:
        mantidos = []
        for pedaco in (corpo or "").split(","):
            item = pedaco.strip()
            faixa = re.fullmatch(r"(\d{1,4})\s*-\s*(\d{1,4})", item)
            if faixa:
                lo, hi = int(faixa.group(1)), int(faixa.group(2))
                for k in range(lo, min(hi, lo + 16) + 1):
                    if 1 <= k <= entregues:
                        mantidos.append(k)
            elif item.isdigit() and 1 <= int(item) <= entregues:
                mantidos.append(int(item))
        return list(dict.fromkeys(mantidos))

    def _cb_prune_citations(texto: str, entregues: int) -> str:
        """Remove every marker the delivered citation list cannot back.

        A single invalid citation index now fails the whole response, and
        `_repoint` leaves a marker untouched whenever it has no slot — dropped
        source, citation cap or evidence budget. Those are pruned here.
        """
        if not texto:
            return texto
        def _dupla(m):
            slots = _cb_valid_slots(m.group(1), entregues)
            return "".join("[[%d]]" % k for k in slots)
        limpo = _CB_DOUBLE_RE.sub(_dupla, texto)
    ...
    def _cb_prune_output(valor, entregues: int, profundidade: int = 0):
        """Same pruning, applied through a structured output.

        The validator walks `text`, `output` and `note`, so a marker carried
        into a schema value fails the response just as one in the prose does.
        """
        if profundidade > 6:
            return valor
        if isinstance(valor, str):
            return _cb_prune_citations(valor, entregues)
        if isinstance(valor, list):
            return [_cb_prune_output(v, entregues, profundidade + 1) for v in valor]
        if isinstance(valor, tuple):
            return tuple(_cb_prune_output(v, entregues, profundidade + 1) for v in valor)
        if isinstance(valor, dict):
            return {k: _cb_prune_output(v, entregues, profundidade + 1) for k, v in valor.items()}
        return valor
    ...
    async def _temporal_recheck(question: str, answer: str, messages: list,
        ...
        try:
            ...
            messages.append({"role": "system", "content": (
                "COVERAGE SWEEP (required documents): research is NOT complete. No gathered "
    ...
    CLAIM_PLAN_MIN_LEFT_S = 150.0
    CLAIM_FILL_MIN_LEFT_S = 55.0
    CLAIM_SCHEMA_RESERVE_S = 34.0
    CLAIM_TEXT_RESERVE_S = 14.0
    CLAIM_ROUTE_MIN_DEMANDS = 2
    CLAIM_ROUTE_MIN_CHARS = 120
    _CLAIM_DEMAND_RE = re.compile(
        r"\b(how many|how much|which|what|who|when|where|list|name|state|give|compare|identify)\b")
    _CLAIM_ROUTE_SIGNALS = ('list all', 'for each', 'both', 'respectively', 'as well as',
                            'in addition to', 'along with', 'and also', 'together with')

    def _claim_demands(text: str) -> int:
        """How many separate statements the question owes."""
        lowered = (text or "").casefold()
        contagem = 0
        for parte in re.split(r"(?<=[.!?])\s+|;|\n", lowered):
            trecho = parte.strip()
            if len(trecho) >= 12 and _CLAIM_DEMAND_RE.search(trecho):
                contagem += 1
        if any(sinal in lowered for sinal in _CLAIM_ROUTE_SIGNALS):
            contagem += 1
        return contagem

    def _claim_worth_board(query, question: str) -> bool:
        ...
        if bool(getattr(query, "fast", False)):
            return False
        texto = getattr(query, "text", None) or question or ""
        if len(texto) < CLAIM_ROUTE_MIN_CHARS:
            return False
        return _claim_demands(texto) >= CLAIM_ROUTE_MIN_DEMANDS

    # ── claim board ──────────────────────────────────────────────────────
    # From here the run is organised by what the ANSWER OWES, not by what the
    # loop happened to gather. The question is resolved into claim slots; every
    # gathered row is bound to the slots it can support; slots nothing supports
    # send retrieval back out; and the delivered answer is written slot by slot
    # off that board. The loop's own draft is kept only as the fallback for a
    # board that cannot be built or written.
    CLAIM_MAX = 6
    CLAIM_MIN_LEFT_S = 30.0
    CLAIM_PROBE_MAX = 2
    CLAIM_PLAN_MAX_TOKENS = 320
    CLAIM_PLAN_TIMEOUT_S = 20.0
    CLAIM_WRITE_MAX_TOKENS = 2600
    CLAIM_WRITE_TIMEOUT_S = 70.0
    CLAIM_WRITE_RESERVE_S = 25.0
    CLAIM_BIND_RATIO = 0.6
    CLAIM_KEEP_PCT = 55
    CLAIM_ROW_CHARS = 1200
    CLAIM_BOARD_CHARS = 44000
    CLAIM_MIN_KEYS = 2
    CLAIM_ROWS_PER_SLOT = 4
    _CLAIM_TOKEN_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9'’&.\-]*")
    _CLAIM_NUM_RE = re.compile(r"\d[\d,.]*")
    _CLAIM_NAME_RE = re.compile(r"[A-Z][A-Za-z0-9&'’.\-]{2,}")
    _CLAIM_MARK_RE = re.compile(r"\[[0-9][0-9,\s\-]*\]")
    _CLAIM_PART_RE = re.compile(r"(?<=[.!?])\s+|\n+|;")
    ...
    def _claim_keys(claim: str) -> list:
        """The words that decide whether a gathered row speaks to this claim."""
        keys: list = []
        for match in _CLAIM_TOKEN_RE.finditer((claim or "").lower()):
            token = match.group(0).strip(".-'’")
            if len(token) >= 4 and token not in _CLAIM_STOP and token not in keys:
                keys.append(token)
        return keys[:8]

    def _claim_seeds(question: str) -> list:
        """Claim slots read deterministically off the question itself."""
        out: list = []
        for part in _CLAIM_PART_RE.split(question or ""):
            text = " ".join(part.split())
            if len(text) < 12 or len(_claim_keys(text)) < CLAIM_MIN_KEYS:
                continue
    ...
    async def _claim_slots(question: str, deadline: float) -> list:
        """Resolve what the answer owes before any of it is written."""
        slots = _claim_seeds(question)
        left = deadline - monotonic()
        if left < CLAIM_PLAN_MIN_LEFT_S:
            return slots[:CLAIM_MAX]
        probe = ("Break the question into the distinct CLAIMS an answer must "
                 "state: one short line per claim, each naming what has to be "
                 "established — a figure, a name, a date, a comparison, a "
                 "membership list. No preamble. JSON only: a list of strings, "
                 "at most 6.\n\nQuestion:\n" + (question or "")[:3500])
        try:
            raw = await _chat_simple(
                LLM_LANE_A, AUDIT_MODEL,
                "Research planner. Name the claims to establish. JSON list only.",
                probe, max_tokens=CLAIM_PLAN_MAX_TOKENS,
                timeout=max(6.0, min(CLAIM_PLAN_TIMEOUT_S, left - 60.0)))
        ...
        if isinstance(parsed, list):
            for item in parsed:
                ...
                if len(text) < 8 or len(_claim_keys(text)) < CLAIM_MIN_KEYS:
                    continue
                if not any(text.lower() == slot.lower() for slot in slots):
                    slots.append(text[:180])
        return slots[:CLAIM_MAX]

    def _claim_row_blob(row) -> str:
        parts = []
        for field in ("title", "url", "preview", "text"):
            blob = row.get(field) or ""
            if blob:
                parts.append(blob)
        return " ".join(parts)

    def _claim_bind(slots: list, ledger) -> dict:
        """Bind every gathered row to the claims it can support.

        This mapping, not the page list, is the run's evidence state: rows are
        addressed through the claim they answer, and a claim with no row is an
        open research target rather than a formatting problem.
        """
        rows = getattr(ledger, "rows", []) or []
        blobs = [(_claim_row_blob(row) or "").lower() for row in rows]
        bound: dict = {}
            ...
            if keys:
                for number, blob in enumerate(blobs, start=1):
                    ...
                    for key in keys:
                        if key in blob:
                            found += 1
                    if float(found) / float(len(keys)) >= CLAIM_BIND_RATIO:
                        hits.append(number)
            bound[index] = hits[:CLAIM_ROWS_PER_SLOT]
        return bound

    def _claim_open(slots: list, bound: dict) -> list:
        return [claim for index, claim in enumerate(slots) if not bound.get(index)]

    async def _claim_fill(question: str, open_claims: list, ledger,
                          deadline: float) -> bool:
        """Send retrieval back out for the claims nothing supports yet."""
        tail = " ".join(word for word in (question or "").split()
                        if word.lower() not in _CLAIM_STOP)[:90]
        fired = False
        for claim in open_claims[:CLAIM_PROBE_MAX]:
            if deadline - monotonic() < CLAIM_MIN_LEFT_S:
                break
            probe = " ".join(_claim_keys(claim)[:6])
            if not probe.strip():
                continue
            try:
                await _do_search((probe + " " + tail).strip(), ledger)
                fired = True
            except Exception:
                pass
        return fired

    def _claim_board(slots: list, bound: dict, ledger) -> str:
        ...
        for index, claim in enumerate(slots):
            ...
            lines.append("")
            lines.append(f"CLAIM {index + 1}: {claim}")
            if not numbers:
                lines.append("  (nothing gathered supports this claim — say so "
                             "plainly in the answer and state what was checked)")
                continue
            for number in numbers:
                if number < 1 or number > len(rows):
                    continue
                row = rows[number - 1]
                body = _row_evidence_text(row, CLAIM_ROW_CHARS).strip()
                if not body:
                    continue
                block = (f"  [{number}] {row.get('title') or ''} "
                         f"({row.get('url') or ''})\n    {body}")
                if spent + len(block) > CLAIM_BOARD_CHARS:
                    break
                spent += len(block)
                lines.append(block)
        return "\n".join(lines)

    _CLAIM_FILTER_RE = re.compile(
    ...
    def _claim_filtered_pool(question: str) -> bool:
        """Does the question define its set by exclusion?

        Cross-edition questions that keep some rows and drop others are the ones
        a judge marks wrong when the answer shows only the survivors: the work of
        filtering is invisible, so a correct verdict reads as a bare assertion.
        """
        return bool(_CLAIM_FILTER_RE.search(question or ""))

    async def _claim_write(question: str, board: str, deadline: float) -> str:
        """Write the answer claim by claim, straight off the board."""
        left = deadline - monotonic() - CLAIM_WRITE_RESERVE_S
        if left <= 12.0 or not board.strip():
            return ""
        system = ("You write the FINAL answer from a claim board. Work claim by "
                  ...
                  "question asks for, and put the [n] marker of the bound row "
                  "on the sentence that carries the figure or name. A claim the "
                  "board shows as unsupported is reported as unestablished, "
                  "naming what was checked — never invented. No preamble, no "
                  "process narration, no headings unless the question asks for "
                  "them: the answer only, in the shape the question requires.")
        if _claim_filtered_pool(question):
            system += (
        ...
        user = ("QUESTION:\n" + (question or "")[:3500] + "\n\n" + board)
        try:
            text = await _chat_simple(LLM_LANE_A, LOOP_MODEL_A, system, user,
                                      max_tokens=CLAIM_WRITE_MAX_TOKENS,
                                      timeout=min(CLAIM_WRITE_TIMEOUT_S, left))
        except Exception:
            return ""
        return (text or "").strip()

    def _claim_facts(text: str) -> set:
        body = _CLAIM_MARK_RE.sub(" ", text or "")
        out = set()
        for match in _CLAIM_NUM_RE.finditer(body):
            out.add("n:" + match.group(0).replace(",", "").rstrip("."))
        for match in _CLAIM_NAME_RE.finditer(body):
            out.add("e:" + " ".join(match.group(0).split()).lower())
        return out

    def _claim_keeps_facts(draft: str, written: str) -> bool:
        before = _claim_facts(draft)
        if not before:
            return True
        kept = len(before & _claim_facts(written))
        return kept * 100 >= len(before) * CLAIM_KEEP_PCT

    async def _claim_pipeline(question: str, draft: str, ledger,
                              deadline: float) -> str:
        """Claims → binding → targeted retrieval → the written answer."""
        try:
            if not getattr(ledger, "rows", None):
                return ""
            if deadline - monotonic() < CLAIM_MIN_LEFT_S:
                return ""
            slots = await _claim_slots(question, deadline)
            if not slots:
                return ""
            bound = _claim_bind(slots, ledger)
            open_claims = _claim_open(slots, bound)
            if open_claims and _spend_left() >= AUDIT_MIN_USD:
                if await _claim_fill(question, open_claims, ledger, deadline):
                    bound = _claim_bind(slots, ledger)
            if not any(bound.get(index) for index in range(len(slots))):
                return ""
            board = _claim_board(slots, bound, ledger)
            written = await _claim_write(question, board, deadline)
            if not _is_usable_answer(written):
                return ""
            if not _CLAIM_MARK_RE.search(written):
                return ""
            if _is_usable_answer(draft) and not _claim_keeps_facts(draft, written):
                return ""
            return written
        except Exception:
            return ""
    ...
    async def _solve(query: Query, question: str) -> Response:
        ...
        _cb_on = False
        try:
            _cb_on = _claim_worth_board(query, question)
        except Exception:
            _cb_on = False
        _cb_slots: list = []
        if _cb_on:
            try:
                _cb_slots = await _claim_slots(question, deadline)
            except Exception:
                _cb_slots = []
        answer = ''
        messages: list[dict] = []
        ...
        if _cb_on:
            _cb_answer = ''
            try:
                _cb_answer = await _claim_pipeline(question, answer, ledger,
                                                   deadline, _cb_slots,
                                                   CLAIM_TEXT_RESERVE_S)
            except Exception:
                _cb_answer = ''
            if _is_usable_answer(_cb_answer):
                answer = _cb_answer
        ...
        text = _cb_prune_citations(text, len(citations))
        ...
        if query.output_schema is not None:
            ...
            if structured is not None:
                ...
                if structured is not None:
                    structured = _cb_prune_output(structured, len(citations))
                    try:
            ...
            if basis is not answer:
                ...
                if salvaged is not None:
                    ...
                    if salvaged is not None:
                        salvaged = _cb_prune_output(salvaged, len(citations))
                        try:
