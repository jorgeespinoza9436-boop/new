...
_fnxdkcpihj = {'started': None, 'text': None}
_tjqovrgvmn = 24000
_roxhgksrfw = 290.0
_sszpursjzc = 250.0

def _yyfnidlvfj() -> float:
    started = _fnxdkcpihj['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _nrlhplxram() -> float:
    return _roxhgksrfw - _yyfnidlvfj()
_azfbkbjurn = _hsapi.llm_chat
_qcbvjcacni = _hsapi.search_web
_syiodgycvi = _hsapi.fetch_page
_exgcyvlsfk = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _vunqjcjdsy(*args, **kwargs):
    if _yyfnidlvfj() >= _sszpursjzc:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _exgcyvlsfk})
            kwargs['messages'] = steered
    _result = await _azfbkbjurn(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _xcfxovlsom(_result)
    return _result

async def _ztjafrwwhf(*args, **kwargs):
    if _yyfnidlvfj() >= _sszpursjzc:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _qcbvjcacni(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _oottydizyz(*args, **kwargs):
    if _yyfnidlvfj() >= _sszpursjzc:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _syiodgycvi(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _vunqjcjdsy
_hsapi.search_web = _ztjafrwwhf
_hsapi.fetch_page = _oottydizyz
_nngedguxoq = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_ofxmwyvnwl = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_mwidcpajww = 900
_rookxjymrg = 2

def _pjezfvwpnw(query: Query) -> int:
...
def _mjlitwsobz(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _fudhxwrxor(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _pjezfvwpnw(query)
    if fields >= 3:
        return 2
    if _mjlitwsobz(lowered, _nngedguxoq):
        return 1
    if fields <= _rookxjymrg and len(text) <= _mwidcpajww:
        return 0
    if _mjlitwsobz(lowered, _ofxmwyvnwl):
        return 0
    return 1

def _xcfxovlsom(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _fnxdkcpihj['text'] = text.strip()[:_tjqovrgvmn]
    except Exception:
        pass

def _rxcsiiwsjk(text: str):
...
def _locythazkp(query: Query) -> Response:
    text = _fnxdkcpihj['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_tjqovrgvmn]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _rxcsiiwsjk(text)
...
def _rwkaxtohga():
    ...
    async def query(query: Query) -> Response:
        ...
        try:
            ...
            return base
    return query
_bbcflghsql = _mtoujrhklv()
_accghczlnw = _onpbmvizfn()
_vyytiqevjf = _owkyvyndsh()
_rjscdzutlh = _rwkaxtohga()
_roxhgksrfw = 290.0
_xvsvphmeyv = 250.0
_itwubldypa = 90.0

async def _myzswiacrg(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _roxhgksrfw - (time.monotonic() - started)
        if first:
            budget = _xvsvphmeyv if _xvsvphmeyv < remaining else remaining
            first = False
        else:
            if remaining < _itwubldypa:
                break
        ...
        try:
            ...
            last_exc = exc
    return _locythazkp(query)
_jywluvlaui = 235.0

async def _bdobbnoaub(agent, query, budget):
    try:
        return await asyncio.wait_for(agent(query), timeout=budget)
    except Exception:
        return None
...
async def _uqhsucntnw(query, agents):
    if len(agents) < 2:
        return await _myzswiacrg(query, agents)
    r0, r1 = await asyncio.gather(_bdobbnoaub(agents[0], query, _jywluvlaui), _bdobbnoaub(agents[1], query, _jywluvlaui))
    ok0 = isinstance(r0, Response) and _lviulevsxo(r0) != ''
    ok1 = isinstance(r1, Response) and _lviulevsxo(r1) != ''
    if ok0 and (not ok1):
        return r0
    if ok1 and (not ok0):
        return r1
    ...
    try:
        ...
        jr = await asyncio.wait_for(_azfbkbjurn(provider='openrouter', messages=msgs, model='openai/gpt-oss-120b', temperature=0.0, max_output_tokens=8, timeout=30.0), timeout=32.0)
        jt = ''
...
@entrypoint('query')
async def query(query: Query) -> Response:
    _fnxdkcpihj['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _myzswiacrg(query, (_vyytiqevjf, _bbcflghsql, _accghczlnw, _rjscdzutlh))
        index = _fudhxwrxor(query)
        if index == 0:
            agents = (_bbcflghsql, _accghczlnw, _vyytiqevjf, _rjscdzutlh)
        elif index == 1:
            agents = (_accghczlnw, _vyytiqevjf, _rjscdzutlh, _bbcflghsql)
        elif index == 2:
            agents = (_vyytiqevjf, _rjscdzutlh, _bbcflghsql, _accghczlnw)
        elif index == 3:
            agents = (_rjscdzutlh, _bbcflghsql, _accghczlnw, _vyytiqevjf)
        else:
            agents = (_bbcflghsql, _accghczlnw, _vyytiqevjf, _rjscdzutlh)
        return await _uqhsucntnw(query, agents)
    except Exception:
        return _locythazkp(query)
_BUILDM_TAG_m20 = "sn45-6ed1d3bf3052"
