...
_aabyzvgham = {'started': None, 'text': None}
_ekpkourjen = 24000
_nukwmmjgne = 290.0
_mmaxckfukd = 250.0

def _eljykixxyj() -> float:
    started = _aabyzvgham['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _gwlffwlhpd() -> float:
    return _nukwmmjgne - _eljykixxyj()
_zfwgzysrkq = _hsapi.llm_chat
_quyunbmntf = _hsapi.search_web
_kooeknxbae = _hsapi.fetch_page
_huazmlscmo = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _bivmiubzca(*args, **kwargs):
    if _eljykixxyj() >= _mmaxckfukd:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _huazmlscmo})
            kwargs['messages'] = steered
    _result = await _zfwgzysrkq(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _tubbyicgck(_result)
    return _result

async def _vbcflusoan(*args, **kwargs):
    if _eljykixxyj() >= _mmaxckfukd:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _quyunbmntf(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _gbnesiwtpr(*args, **kwargs):
    if _eljykixxyj() >= _mmaxckfukd:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _kooeknxbae(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _bivmiubzca
_hsapi.search_web = _vbcflusoan
_hsapi.fetch_page = _gbnesiwtpr
_vpeipdgtaf = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_odrlnwywhl = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_ksuvobxfgm = 900
_shatjqsqxb = 2

def _gztcxrxung(query: Query) -> int:
...
def _nioanyshwg(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _vefqttnfou(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _gztcxrxung(query)
    if fields >= 3:
        return 2
    if _nioanyshwg(lowered, _vpeipdgtaf):
        return 1
    if fields <= _shatjqsqxb and len(text) <= _ksuvobxfgm:
        return 0
    if _nioanyshwg(lowered, _odrlnwywhl):
        return 0
    return 1

def _tubbyicgck(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _aabyzvgham['text'] = text.strip()[:_ekpkourjen]
    except Exception:
        pass

def _obkljtsfay(text: str):
...
def _hiiqvslewo(query: Query) -> Response:
    text = _aabyzvgham['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_ekpkourjen]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _obkljtsfay(text)
...
def _mcielllbwt():
    ...
    return query
_lggqfgrmvb = _xxydbtfpyz()
_wekfyrykmb = _qvunnblnhh()
_fxibfzanxf = _ugsldhuchu()
_ooebigyhpl = _mcielllbwt()
_nukwmmjgne = 290.0
_bpjicoprib = 250.0
_skoslzgzvs = 90.0

async def _ourbmcmitw(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _nukwmmjgne - (time.monotonic() - started)
        if first:
            budget = _bpjicoprib if _bpjicoprib < remaining else remaining
            first = False
        else:
            if remaining < _skoslzgzvs:
                break
        ...
        try:
            ...
            last_exc = exc
    return _hiiqvslewo(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _aabyzvgham['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ourbmcmitw(query, (_fxibfzanxf, _lggqfgrmvb, _wekfyrykmb, _ooebigyhpl))
        index = _vefqttnfou(query)
        if index == 0:
            agents = (_lggqfgrmvb, _wekfyrykmb, _fxibfzanxf, _ooebigyhpl)
        elif index == 1:
            agents = (_wekfyrykmb, _fxibfzanxf, _ooebigyhpl, _lggqfgrmvb)
        elif index == 2:
            agents = (_fxibfzanxf, _ooebigyhpl, _lggqfgrmvb, _wekfyrykmb)
        elif index == 3:
            agents = (_ooebigyhpl, _lggqfgrmvb, _wekfyrykmb, _fxibfzanxf)
        else:
            agents = (_lggqfgrmvb, _wekfyrykmb, _fxibfzanxf, _ooebigyhpl)
        return await _ourbmcmitw(query, agents)
    except Exception:
        return _hiiqvslewo(query)
_BUILDK_TAG_m4 = "sn45-327cdb0b86b2"
