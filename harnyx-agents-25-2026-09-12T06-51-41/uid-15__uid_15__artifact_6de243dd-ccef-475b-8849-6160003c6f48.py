...
_rkpisxzpjs = {'started': None, 'text': None}
_bwfaismyzd = 24000
_ixjldcjmlo = 290.0
_qincrtolak = 250.0

def _ggprxyelnj() -> float:
    started = _rkpisxzpjs['started']
    if started is None:
        return 0.0
    return max(0.0, time.monotonic() - started)

def _rbfgobnawb() -> float:
    return _ixjldcjmlo - _ggprxyelnj()
_swrabduzvs = _hsapi.llm_chat
_xyyjzxhmfi = _hsapi.search_web
_keigdvhcyk = _hsapi.fetch_page
_mnmmhvadhi = 'The research time budget is now exhausted. Do NOT request any more search or fetch tools. Using only the information already gathered in this conversation, produce your COMPLETE final answer now, including every field the requested output schema requires. If a finish/submit tool is available, call it now with that complete answer.'

async def _wobicwxwbf(*args, **kwargs):
    if _ggprxyelnj() >= _qincrtolak:
        messages = kwargs.get('messages')
        if messages is not None:
            steered = list(messages)
            steered.append({'role': 'user', 'content': _mnmmhvadhi})
            kwargs['messages'] = steered
    _result = await _swrabduzvs(provider=kwargs.get('provider'), messages=kwargs.get('messages'), model=kwargs.get('model'), temperature=kwargs.get('temperature'), max_output_tokens=kwargs.get('max_output_tokens'), max_tokens=kwargs.get('max_tokens'), tools=kwargs.get('tools'), tool_choice=kwargs.get('tool_choice'), parallel_tool_calls=kwargs.get('parallel_tool_calls'), thinking=kwargs.get('thinking'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
    _jwlluiqycl(_result)
    return _result

async def _soefndjaab(*args, **kwargs):
    if _ggprxyelnj() >= _qincrtolak:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _xyyjzxhmfi(*args, provider=kwargs.get('provider'), num=kwargs.get('num'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))

async def _oiidquqayt(*args, **kwargs):
    if _ggprxyelnj() >= _qincrtolak:
        raise TimeoutError('research cutoff reached; finalize with gathered evidence')
    return await _keigdvhcyk(*args, provider=kwargs.get('provider'), provider_extra=kwargs.get('provider_extra'), timeout=kwargs.get('timeout'))
_hsapi.llm_chat = _wobicwxwbf
_hsapi.search_web = _soefndjaab
_hsapi.fetch_page = _oiidquqayt
_xvpuwwvstc = ('compare', 'difference', 'calculate', 'ratio', 'how many', 'how much', ' vs ', 'versus')
_cskvuzmrin = ('who is', 'what is', 'when did', 'where is', 'which', 'name the', 'identify', 'list the')
_sutijkwika = 900
_usgcbhqqgj = 2

def _tbwwiurowz(query: Query) -> int:
...
def _vohhecvgxz(text: str, terms: tuple) -> bool:
    for term in terms:
        if term in text:
            return True
    return False

def _ckqbekjqdw(query: Query) -> int:
    text = (getattr(query, 'text', '') or '').strip()
    lowered = text.lower()
    fields = _tbwwiurowz(query)
    if fields >= 3:
        return 2
    if _vohhecvgxz(lowered, _xvpuwwvstc):
        return 1
    if fields <= _usgcbhqqgj and len(text) <= _sutijkwika:
        return 0
    if _vohhecvgxz(lowered, _cskvuzmrin):
        return 0
    return 1

def _jwlluiqycl(result: object) -> None:
    try:
        ...
        if text and text.strip():
            _rkpisxzpjs['text'] = text.strip()[:_bwfaismyzd]
    except Exception:
        pass

def _bmlxiddhef(text: str):
...
def _ebzgftiyix(query: Query) -> Response:
    text = _rkpisxzpjs['text']
    if not text or not text.strip():
        text = 'A complete answer could not be produced within the available time budget.'
    text = text.strip()[:_bwfaismyzd]
    schema = getattr(query, 'output_schema', None)
    if schema is not None:
        parsed = _bmlxiddhef(text)
...
def _jafghopjmx():
    ...
    return query
_owpavnvvtp = _hetdvckzso()
_tuuiewdajt = _lhkbakilzv()
_wxpqhowgby = _fesjzwdkpn()
_mibjofcems = _jafghopjmx()
_ixjldcjmlo = 290.0
_yufxwavilb = 250.0
_ldqchjggsw = 90.0

async def _ocawsnweif(query: Query, agents: tuple) -> Response:
    started = time.monotonic()
    last_exc = None
    first = True
    for agent in agents:
        remaining = _ixjldcjmlo - (time.monotonic() - started)
        if first:
            budget = _yufxwavilb if _yufxwavilb < remaining else remaining
            first = False
        else:
            if remaining < _ldqchjggsw:
                break
        ...
        try:
            ...
            last_exc = exc
    return _ebzgftiyix(query)

@entrypoint('query')
async def query(query: Query) -> Response:
    _rkpisxzpjs['started'] = time.monotonic()
    try:
        if getattr(query, 'fast', False):
            return await _ocawsnweif(query, (_wxpqhowgby, _owpavnvvtp, _tuuiewdajt, _mibjofcems))
        index = _ckqbekjqdw(query)
        if index == 0:
            agents = (_owpavnvvtp, _tuuiewdajt, _wxpqhowgby, _mibjofcems)
        elif index == 1:
            agents = (_tuuiewdajt, _wxpqhowgby, _mibjofcems, _owpavnvvtp)
        elif index == 2:
            agents = (_wxpqhowgby, _mibjofcems, _owpavnvvtp, _tuuiewdajt)
        elif index == 3:
            agents = (_mibjofcems, _owpavnvvtp, _tuuiewdajt, _wxpqhowgby)
        else:
            agents = (_owpavnvvtp, _tuuiewdajt, _wxpqhowgby, _mibjofcems)
        return await _ocawsnweif(query, agents)
    except Exception:
        return _ebzgftiyix(query)
_BUILDL_TAG_m22 = "sn45-91cacd27445e"
