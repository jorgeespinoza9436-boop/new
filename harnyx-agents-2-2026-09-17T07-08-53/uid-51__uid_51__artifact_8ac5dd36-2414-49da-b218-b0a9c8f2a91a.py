...
_c2b_fast_query_entry = _compose_c2b_fast_entry()

def _compose_c2b_efficient_entry():
...
_c2b_efficient_query_entry = _compose_c2b_efficient_entry()

def _compose_c2b_balanced_entry():
...
_c2b_balanced_query_entry = _compose_c2b_balanced_entry()

def _compose_c2b_complete_entry():
...
_c2b_complete_query_entry = _compose_c2b_complete_entry()
...
import re as _frontier_re

_M1_FIG_RE = _frontier_re.compile(r"\d[\d,]{1,}(?:\.\d+)?")


def _c2b_efficient_prose(query: Query) -> bool:
    if bool(getattr(query, "fast", False)) or getattr(query, "output_schema", None) is not None:
        return False
    text = " ".join(str(getattr(query, "text", "") or "").lower().split())
    words = set(_frontier_re.findall(r"[a-z]+", text))
    current_status = (
    ...
    quarterly_filter = (
        "quarterly" in words and ("totals" in words or "breakdown" in words)
        and ("threshold" in words or "greater" in words or "less" in words
             or "lowest" in words or "filter" in words)
    )
    denomination_sum = (
        "denomination" in text and "table" in text
        and ("combined" in words or "sum" in words or "total" in words)
    )
    return current_status or quarterly_filter or denomination_sum


@entrypoint("query")
async def query(query: Query) -> Response:
    if bool(getattr(query, "fast", False)):
        return await _c2b_fast_query_entry(query)
    # The completeness lane improves treaty reconciliation while the cheaper
    # lane retains the runtime door on periodic approvals.
    if _frontier_treaty_shape(query):
        try:
            return await _c2b_complete_query_entry(query)
        except Exception:
            pass
    if _c2b_efficient_prose(query):
        try:
            return await _c2b_efficient_query_entry(query)
        except Exception:
            pass
    return await _c2b_balanced_query_entry(query)
