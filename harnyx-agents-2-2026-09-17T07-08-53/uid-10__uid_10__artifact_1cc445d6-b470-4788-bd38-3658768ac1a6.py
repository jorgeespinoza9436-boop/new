...
async def _c1_score_query(query: Query) -> Response:
...
_c1_fast_query_entry = _compose_c1_fast_entry()

def _compose_c1_balanced_entry():
 import asyncio as _repair_asyncio
 import json as _repair_json
 import re as _repair_re
 import time as _repair_time
...
_c1_balanced_query_entry = _compose_c1_balanced_entry()

def _compose_c1_deep_entry():
...
_c1_deep_query_entry = _compose_c1_deep_entry()


def _c1_words(query: Query):
    text = " ".join(str(getattr(query, "text", "") or "").lower().split())
    return text, set(_repair_re.findall(r"[a-z]+", text))


def _c1_route(query: Query) -> str:
    """Route reusable evidence shapes to the lane that led them in 4126558c."""
    if bool(getattr(query, "fast", False)):
        return "fast"
    if getattr(query, "output_schema", None) is not None:
        return "score"
    text, words = _c1_words(query)
    current_status = (
        ("current" in words or "later" in words)
        and ("withdraw" in text or "no longer" in text)
        and ("authorisation" in text or "authorization" in text or "status" in words)
    )
    periodic_approvals = (
        ("announcement" in words or "announcements" in words)
        and ("approval" in text or "title" in words)
        and ("after each" in text or "chronological" in words)
    )
    if current_status or periodic_approvals:
        return "deep"
    statistical_tables = (
        "statistical tables" in text
        and ("comparison" in words or "comparing" in words or "totals" in words)
    )
    station_catalogue = (
        ("station catalogue" in text or "station catalog" in text)
        and ("latitude" in words or "operational" in words or "coordinates" in words)
    )
    data_dictionary = (
        ("flat-file" in text or "data dictionary" in text)
        and ("column" in words or "columns" in words or "catalog" in words
             or "catalogs" in words)
    )
    if statistical_tables or station_catalogue or data_dictionary:
        return "balanced"
    return "score"


async def _c1a_query(query: Query) -> Response:
    route = _c1_route(query)
    if route == "fast":
        try:
            return await _c1_fast_query_entry(query)
        except Exception:
            pass
    elif route == "balanced":
        try:
            return await _c1_balanced_query_entry(query)
        except Exception:
            pass
    elif route == "deep":
        try:
            return await _c1_deep_query_entry(query)
        except Exception:
            pass
    return await _c1_score_query(query)

def _compose_c1b_complete_entry():
...
_c1b_complete_query_entry = _compose_c1b_complete_entry()


def _frontier_treaty_shape(query: Query) -> bool:
    if bool(getattr(query, "fast", False)) or getattr(query, "output_schema", None) is not None:
        return False
    text = " ".join(str(getattr(query, "text", "") or "").lower().split())
    return (
        ("depositary" in text or "status of treaties" in text)
        and ("cutoff" in text or "footer date" in text)
        and ("contracting state" in text or "entry into force" in text)
    )


def _frontier_periodic_approvals(query: Query) -> bool:
    if bool(getattr(query, "fast", False)) or getattr(query, "output_schema", None) is not None:
        return False
    text = " ".join(str(getattr(query, "text", "") or "").lower().split())
    return (
        ("announcement" in text or "announcements" in text)
        and ("approval" in text or "title" in text)
        and ("after each" in text or "chronological" in text)
    )


def _frontier_complete_shape(query: Query) -> bool:
    """Cross-record reconciliation where completeness decides the score."""
    return _frontier_treaty_shape(query) or _frontier_periodic_approvals(query)

@entrypoint("query")
async def query(query: Query) -> Response:
    # The new lane did not beat C1's score floor on the live treaty probe.
    if _frontier_periodic_approvals(query):
        try:
            return await _c1b_complete_query_entry(query)
        except Exception:
            pass
    return await _c1a_query(query)
